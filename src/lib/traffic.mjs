 ...
 * flush 간격.
 *
 * 🔴 2026-08-22 (5번) — 10분이었고, 그 때문에 **하루치가 통째로 샜다.** 그날 R2 의 마지막
 *   갱신이 오전 10:26 이었다(어제까지는 매일 23:5x). 한 서버가 네 집을 내는데 여섯 자리가
 *   오후 내내 배포했고, 배포는 컨테이너를 새로 띄운다. **10분이 차기 전에 죽으면 다 사라진다.**
 * ⭐ 셋으로 줄인다. 20분에 한 번 배포하는 날에도 대부분이 남는다.
 * ⚠ 흘려 쓸 때마다 R2 를 한 번 읽고 한 번 쓴다. 우리 규모(하루 요청 수천)에서는 값이 없다 —
 *   그리고 `server.mjs` 가 종료 신호에서도 한 번 더 흘려 쓴다. 둘이 함께 막는다.
 */
...
/**
 * 🔴🔴 [2026-09-17 · 5번] **여기서 하루가 통째로 «덮여» 사라지고 있었다.**
 * ─────────────────────────────────────────────────────────────────────────────
 * 30일을 세어 보니 **사흘이 비어 있었다** — 09-05 는 요청 3건, 09-10 은 12건, 09-15 는 2건.
 * 옆날들이 2~5만 건인데 그날만 한 자릿수다. 「그날 손님이 없었다」가 아니라 «지워진» 것이다.
 *
 * 까닭은 flush 안의 `catch` 한 줄이었다. 「파일이 없다(404)」와 「못 읽었다(시간초과·5xx·
 * 네트워크)」를 **같이 받아** 누적을 빈 것으로 두고, 그 위에 지금 통만 얹어 put 했다.
 * ⇒ R2 를 한 번 못 읽은 그 순간, **그날 쌓인 것이 통째로 덮여 사라진다.**
 * (`store.mjs` 의 getRemote 는 404 면 null 을 내고 그 밖의 실패는 «던진다» — 갈라져 있었다)
 *
 * ✅ 못 읽었으면 **쓰지 않는다.** 통을 그대로 들고 다음 판에 다시 시도한다 —
 *   덮어써서 잃는 것보다 3분 늦는 쪽이 낫다.
 * ⭐ 판정을 함수로 떼어 낸다. 안에 박아 두면 죽어도 모른다(회사 강령 넷째).
 *
 * @returns {Promise<{ok:true, 누적:object} | {ok:false, 왜:string}>}
 */
export async function 쌓인것읽기(읽기, key) {
  let 옛;
  try {
    옛 = await 읽기(key);
  } catch (e) {
    /* ⛔ 「없다」로 읽지 않는다. 모르면 모르는 것이고, 모를 때는 손대지 않는다 */
    return { ok: false, 왜: '못 읽었다: ' + String(e?.message ?? e).slice(0, 110) };
  }
  if (!옛) return { ok: true, 누적: {} };        /* 없는 것은 정상이다 — 그날 첫 판이다 */
  try {
    return { ok: true, 누적: JSON.parse(String(옛))?.집계 ?? {} };
  } catch (e) {
    /* 읽기는 됐는데 «깨졌다». 이때도 덮어쓰지 않는다 — 덮으면 깨진 것마저 못 되살린다 */
    return { ok: false, 왜: '깨졌다: ' + String(e?.message ?? e).slice(0, 110) };
  }
}
...
export async function flush(지금 = new Date()) {
  ...
  try {
    /**
     * 🔴🔴 2026-08-22 (5번이 찾음 · 2번이 정함) — 컨테이너가 UTC 로 돈다. 그래서 `날` 을
     *   그냥 `지금`에서 뽑으면 하루 파일의 경계가 09:00~09:00 KST 가 된다 — 달력 하루가 아니다.
     *   여섯 자리가 이 파일로 사장님 9월 목표를 재므로 **KST 달력 하루**로 오늘 옮긴다.
     *   ⚠ 옮기는 날 하루는 15시간짜리로 짧게 잡힌다(자정 KST 이전 데이터가 없어서다) — 한 번뿐이다.
     *   그 뒤로는 계속 KST 자정~자정이 하루다. traffic-report.mjs 는 이미 로컬(KST) PC 에서
     *   돌아 그대로 맞다 — 거기는 고칠 것이 없다.
     */
    const KST날짜 = new Date(지금.getTime() + 9 * 3600 * 1000);
    const 날 = `${KST날짜.getFullYear()}${String(KST날짜.getMonth() + 1).padStart(2, '0')}${String(KST날짜.getDate()).padStart(2, '0')}`;
    const key = 일별키(날);

    /* 읽어서 → 합치고 → 쓴다.
     * ⚠ 프로세스가 재시작되면 메모리 집계가 0 이 된다. 그래서 **쌓인 것을 먼저 읽는다** —
     *   안 그러면 재배포할 때마다 그날치가 날아간다. */
    /* 🔴🔴 [2026-09-17 · 5번] **여기서 하루가 통째로 사라지고 있었다.**
     * ─────────────────────────────────────────────────────────────────────────
     * 30일을 세어 보니 **사흘이 비어 있었다** — 09-05 는 요청 3건, 09-10 은 12건, 09-15 는 2건.
     * 옆날들이 2~5만 건인데 그날만 한 자릿수다. 「그날 손님이 없었다」가 아니라 «지워진» 것이다.
     *
     * 까닭은 이 `catch` 한 줄이었다. 「없다(404)」와 「못 읽었다(시간초과·5xx·네트워크)」를
     * 같이 받아 `누적 = {}` 로 두고, 바로 아래에서 **그 빈 것 위에 지금 통만 얹어 put 한다.**
     * ⇒ R2 를 한 번 못 읽은 그 순간, **그날 쌓인 것이 통째로 덮여 사라진다.**
     * (`store.mjs` 의 getRemote 는 404 면 null 을 내고 그 밖의 실패는 «던진다» — 갈라져 있었다)
     *
     * ✅ 못 읽었으면 **쓰지 않는다.** 통을 그대로 들고 다음 판에 다시 시도한다.
     *   덮어써서 잃는 것보다 3분 늦는 쪽이 낫다. 아래 「성공했을 때만 비운다」와 같은 생각이다.
     * ⛔ 「없는 게 정상이다」를 「못 읽은 것도 정상이다」로 넓혀 읽지 않는다. */
    const 읽은것 = await 쌓인것읽기(get, key);
    if (!읽은것.ok) {
      /* ⚠ 통을 안 비우고 그대로 나간다. 아래 finally 가 flush중 을 푼다 */
      return { skipped: true, 못읽음: 읽은것.왜 };
    }
    const 누적 = 읽은것.누적;
    ...
    /**
     * 🔴 2026-08-22 (5번) — `갱신` 이 **한국어 꼴인데 값은 UTC** 였다. 그래서 읽는 사람이
     *   KST 로 읽고 아홉 시간을 잃은 것으로 착각한다. 내가 그렇게 착각해서 전 유닛에 잘못 알렸다.
     * ⭐ 값을 안 바꾸고(읽는 자들이 이 꼴을 파싱한다) **어느 시계인지 옆에 적는다.**
     *   그리고 KST 로 읽고 싶은 사람을 위해 `갱신KST` 를 따로 준다 — 새 칸은 아무것도 안 깨뜨린다.
     */
    const KST = new Date(지금.getTime() + 9 * 3600 * 1000);
    const 몸 = JSON.stringify({
      날짜: 날,
      갱신: 지금.toLocaleString('ko-KR'),
      갱신시계: '위 갱신 값은 서버 시계다. 이 컨테이너에는 TZ 설정이 없어 UTC 로 돈다',
      갱신KST: KST.toISOString().replace('T', ' ').slice(0, 19),
      하루의범위: '이 파일의 «하루» 는 서버 시계(UTC) 기준이다 — 한국 시각으로는 09:00 부터 다음 날 09:00 까지다',
