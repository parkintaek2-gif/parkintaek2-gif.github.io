...
export async function flush(지금 = new Date()) {
  ...
  try {
    /**
     * 🔴🔴 2026-08-22 (5번) — **이 주석이 틀렸다.** 「이 서버는 KST 로 돈다」고 적혀 있었는데
     *   운영 컨테이너에 TZ 설정이 없어 **UTC** 로 돈다(라이브 응답 헤더 `Date: … GMT` 로 확인).
     *   그래서 아래 `날` 은 UTC 날짜이고, **하루치 파일의 「하루」가 09:00~09:00 KST** 다.
     *   나는 이것을 모르고 「오전 10:26」을 KST 로 읽어 「계측이 아홉 시간 샜다」고 잘못 알렸다.
     *
     * ⚠ 이 칸을 KST 로 바꾸면 **모든 유닛의 하루 수가 한 번 어긋난다**(하루가 15시간이 되는 날이
     *   생긴다). 여섯 자리가 이 파일을 읽고 사장님 9월 목표를 이 수로 재고 있으므로,
     *   **혼자 바꾸지 않는다** — 2번과 상의해서 한 번에 옮긴다. 그때까지 사실만 적어 둔다.
     * ⭐ 지금 이 수를 읽는 사람이 알아야 할 것 하나: **「8월 22일치」는 8/22 09:00 KST 부터
     *   8/23 09:00 KST 까지**다. 달력 하루가 아니다.
     */
