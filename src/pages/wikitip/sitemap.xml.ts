...
export const GET: APIRoute = async () => {
  const entries: Entry[] = [
    ...
    /* 🔴 지면을 만들면 «같은 커밋에서» 이 목록에 넣는다 — 9/04 에 한 번 잊었다 */
    ...
    /* 🔴 [2026-09-10] 가장 많이 읽힌 K팝 대상이 실재하지 않는다는 것을 잰 지면.
       ⛔ 창을 «둘» 낸다 — 12개월로는 가상이 이기고 마지막 달로는 진다.
         하나만 내면 유리한 창을 고른 것이 된다. */
    { path: '/fictional-vs-real', priority: '0.9', changefreq: 'weekly' },
    ...
    /* 🔴 [2026-09-07] 오늘 낸 지면 셋. **하루 늦게 넣었다** —
       빌드되고 링크도 걸렸는데 사이트맵에 없어서 구글에 알린 적이 없었다.
       ⛔ 새 지면을 내면 이 목록에 «같은 커밋에서» 넣는다. 나중에 넣으면 그 사이 날은 없다.
       ⚠ 오늘 이 셋은 canonical 도 함께 빠져 있었다(레이아웃이 page= 를 버렸다).
    ...
    { path: '/what-countries-read', priority: '0.9', changefreq: 'weekly' },
    ...
    /* 자료 지면. 2026-09-10 — 한국 연예 데스크가 하루에 이름을 쓴 사람이 «영어권에서 찾아지나».
       ⭐ 씨앗은 우리가 «매일 받는» 신문제목 아카이브다(소급 불가라 남이 못 만든다).
       ⛔ 제목 본문을 옮기지 않는다 — 쓰는 것은 「그 이름이 그날 거기 있었다」 하나다.
       ⛔ 합계로만 말하지 않는다 — 배우가 두 배 많아서 합계가 두 배인 것을 1인당으로 갈라 냈다 */
    { path: '/newsdesk-abroad', priority: '0.9', changefreq: 'weekly' },
    /* 134편째 자료 지면. 2026-09-08 — 「1세대·4세대·6세대」라는 선이 자료에 있나.
       ⭐ 씨앗은 오늘 스타뉴스 「6세대 걸그룹」. 그 말이 매일 쓰이는데 경계를 대는 곳이 없다.
       ⛔ 「몇 세대가 최고」로 안 쓴다. 잰 것은 «그 선에서 무엇이 달라지나» 하나다.
       ⛔ 5세대 멤버몫 2.5% 를 팬 행동으로 읽지 않는다 — 문서 나이다. 화면에 적어 뒀다 */
    { path: '/generations', priority: '0.9', changefreq: 'weekly' },
    /* 135편째 자료 지면. 2026-09-08 — 774팀 사이에서 관심이 어떤 모양으로 퍼져 있나.
       ⭐ 씨앗은 인벤 「비인기 아이돌은 행사할때마다 적자라고 말하는 이유」(댓 20).
