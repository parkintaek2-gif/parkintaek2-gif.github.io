# 채널 문안 — Rank Korean musicians by look-ups and IU is not first — the top name is there for acting

> ⛔ 계정이 없어 **아직 안 올렸다.** 「계정 열면 오늘 올라감」 상태다.
> ⛔ 셋은 서로 다른 글이다. 복사해 붙인 것이 아니다.
> 기사: https://www.kculturewire.com/article/kpop-attention-top-is-actors

## X
```
232 of the 1,550 individuals in our K-pop panel also appear in Korean titles that reached a Netflix Top 10.

https://www.kculturewire.com/article/kpop-attention-top-is-actors
```
⚠ 132자 (한도 280)

## Threads
```
Rank Korean musicians by look-ups and IU is not first — the top name is there for acting

They are 15% of the names and 41% of the attention. Removing them does not correct the list.

How we counted it, and what it cannot say: https://www.kculturewire.com/article/kpop-attention-top-is-actors
```

## Instagram
```
Rank Korean musicians by look-ups and IU is not first — the top name is there for acting

· 232 of the 1,550 individuals in our K-pop panel also appear in Korean titles that reached a Netflix Top 10.
· They are 15% of the names and 41% of the attention.
· Removing them does not correct the list.

We publish the working, the thresholds we chose, and the questions this
data cannot answer. Link in bio → https://www.kculturewire.com/article/kpop-attention-top-is-actors

#Kpop #Koreandrama #Attention #Netflix #Wikipedia #Measurement #Korea #KCultureWire
```

