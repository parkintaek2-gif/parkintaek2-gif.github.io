...
```
⬜ docs/6번-할일.md 의 «2단계»(비밀번호 → 계정 → 「내가 산 것」 목록) — 마감 9/18 21:00,
   **아직 시작 전이다.** 볼륨·복제·복구 시험을 먼저 끝내고 손님 자료를 담아야 한다
   (KLifeMap 이 볼륨 없이 DB 를 컨테이너에 둬 통째로 잃은 사고를 SeoulMarkets 에서
   되풀이하지 않는 것이 이 항목의 핵심 위험이다)
⛔ 정리(구조조정) 시각이 내일 아침으로 잡혀 있어 이 마감을 6번이 못 넘긴다. **2번이
   이어받는다.** 「1단계(이메일 확보)」만 끝난 상태로 넘긴다.
```
