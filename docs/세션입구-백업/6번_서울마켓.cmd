@echo off
chcp 65001 >nul
title 6번 SeoulMarkets (seoulmarkets.com)
REM ────────────────────────────────────────────────────────────
REM  6번 SeoulMarkets (seoulmarkets.com)
REM  금융·경제. 2번 지휘.
REM
...
claude --resume b78d5fbe-be59-4cee-b798-ca9c15c28eb5
if not errorlevel 1 goto :eof
...
set SID=
for /f "usebackq delims=" %%i in (`node "%~dp0_세션ID-찾기.mjs" --id 6`) do set SID=%%i
if not defined SID goto :fresh
if /i "%SID%"=="b78d5fbe-be59-4cee-b798-ca9c15c28eb5" goto :eof
...
:fresh
echo.
echo  [!] 6번 세션이 없어졌습니다. 새 대화를 엽니다.
echo      붙여 넣을 것은 없습니다. 창이 열리면 스스로 6번인 것을 압니다.
echo.
