@echo off
chcp 65001 >nul
title 1번 KLifeMap (klifemap.ai)
REM ────────────────────────────────────────────────────────────
REM  1번 KLifeMap (klifemap.ai)
REM  사장님 직할. 매출이 나는 서비스다.
REM
...
claude --resume 51145360-18ae-41bb-b9a6-7ec1838915c4
if not errorlevel 1 goto :eof
...
set SID=
for /f "usebackq delims=" %%i in (`node "%~dp0_세션ID-찾기.mjs" --id 1`) do set SID=%%i
if not defined SID goto :fresh
if /i "%SID%"=="51145360-18ae-41bb-b9a6-7ec1838915c4" goto :eof
...
:fresh
echo.
echo  [!] 1번 세션이 없어졌습니다. 새 대화를 엽니다.
echo      붙여 넣을 것은 없습니다. 창이 열리면 스스로 1번인 것을 압니다.
echo.
