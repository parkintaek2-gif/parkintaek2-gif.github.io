...
/**
 * 🔴 [2026-09-15 19:0x · 사장님] **36시간 → 30일로 넓혔다.**
 *
 * > 「너와 3번은 원래 창이 열리는 것 같은데, **다른 세션은 아예 새로 시작한다**...
 * >  다른 세션들도 너와 3번처럼 **원래 창이 열리게 해라**, 다시 열어」
 *
 * 그날 17:55 에 PC 가 재시작했고, 다시 열렸을 때 셋으로 갈렸다. 재서 까닭이 나왔다 —
 * ```
 *   3번  마지막 대화 09-13 07:00 → 재시작 때 «55시간» 전           ← 그런데 이어졌다(*)
 *   5번  마지막 대화가 계속 살아 있었음                              ← 이어졌다
 *   1번  마지막 대화 09-13 07:00 → 59시간 전  → 36시간을 넘겨 «잘림» ← 새로 열림
 *   6번  마지막 대화 09-13 18:13 → 48시간 전  → 36시간을 넘겨 «잘림» ← 새로 열림
 *   2번  마지막 대화 09-11      → 한참 넘김                        ← 새로 열림
 * ```
 * ⇒ **자리가 게을러서가 아니라 이 «36」이라는 수가 갈랐다.**
 *
 * ⚠ 앞 판의 걱정은 이것이었다 — 「오래된 것을 이어 열면 유닛이 그때로 되돌아간다」.
 *   그 걱정 자체는 틀리지 않다. 다만 사장님이 저울을 정하셨다 —
 *   **새로 시작해서 하루치 맥락을 통째로 잃는 쪽이 더 나쁘다.**
 *   되돌아가는 것은 자리가 CLAUDE.md·메모 꼬리·git log 를 읽어 스스로 따라잡을 수 있지만,
 *   잃은 창은 아무도 못 되살린다.
 *
 * ⛔ 그러니 이 수를 「안전하게」 다시 줄이지 않는다. 줄이면 사장님 지시를 되돌리는 것이다.
 * ✅ 「빈 껍데기는 안 고른다」(최소크기)는 그대로 둔다 — 그것은 시간이 아니라 «알맹이»를 보는 자다.
 */
export const 이어열참는시간 = 30 * 24 * 3600 * 1000;   /* 30일 */
...
function 자가시험() {
  ...
  /* 🔴 [2026-09-15] 이 시험은 «뒤집혔다». 앞 판은 아흐레 전 것을 막았고, 그 막음이
     09-15 재시작 때 1·2·6번의 창을 통째로 새로 열게 했다. 사장님이 바로잡으셨다 —
     「다른 세션들도 너와 3번처럼 원래 창이 열리게 해라」
     ⇒ 이제 아흐레 전 것은 «이어 연다». 자리는 CLAUDE.md 와 메모로 지금을 따라잡는다. */
  재다('🔴 이어열: 아흐레 전 것도 «이어 연다» — 새로 여는 것보다 낫다 (사장님 09-15)', (() => {
    const 읽기 = 디스크({ 'projects': ['A'], '/A': [U1] });
    const r = 이어열수있나(3, 이제, 'X', 읽기, 잼표({ [U1]: { size: 500 * 1024, mtimeMs: 아흐레전 } }));
    return r.된다 === true && r.고른것.id.startsWith('11111111');
  })());

  재다('⛔ 이어열: 서른날을 넘긴 것은 그래도 안 된다 — 선은 있어야 한다', (() => {
    const 읽기 = 디스크({ 'projects': ['A'], '/A': [U1] });
    const 두달전 = 이제 - 60 * 24 * 3600 * 1000;
    const r = 이어열수있나(3, 이제, 'X', 읽기, 잼표({ [U1]: { size: 500 * 1024, mtimeMs: 두달전 } }));
    return r.된다 === false && /되돌아간다/.test(r.까닭);
  })());

  재다('참는 시간이 30일이다', 이어열참는시간 === 30 * 24 * 3600 * 1000);
