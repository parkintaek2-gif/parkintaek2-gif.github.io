...
async function 본일(몇명, 적나) {
  ...
  for (const 사람 of 명단) {
    ...
    낸것.push({
      ...
      으뜸이본지면인가: !!으뜸?.본지면인가,
      으뜸조회: 으뜸?.조회 ?? null,
      /* ⭐ [2026-09-05 07:1x] 제목별 내역을 «남긴다» — 앞판은 합계와 으뜸만 남겼다.
         한 사람이 영어권에서 «몇 개의 이름»으로 불리나를 재려면 내역이 있어야 한다.
         ⚠ 이미 받아 온 값이다. 다시 부르지 않는다 — 우물에 두 번 묻는 것이 가장 게으른 짓이다. */
      제목별: 줄들.map((x) => ({ 제목: 읽는이름(x.제목), 조회: x.조회, 본지면인가: x.본지면인가 }))
        .sort((a, b) => (b.조회 ?? 0) - (a.조회 ?? 0)),
    });
