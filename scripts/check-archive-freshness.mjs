...
export const 갈래들 = [
  ...
  { 길: 'archive/raw/krx', 이름: 'KRX 일별시세(내부 검산용 · 팔 파일에 넣지 않는다)', 몫: '6번', 참는날: 3, 거래일: true, 무늬: /-(\d{8})\.json$/ },
...
if (나 && process.argv.includes('--자가시험')) {
  ...
  const krx갈래 = 갈래들.find((x) => x.길 === 'archive/raw/krx');
  검('⛔ KRX 갈래 이름에 «팔 파일에 넣지 않는다»가 적혀 있다 — 이름이 규칙을 말한다',
    /팔 파일에 넣지 않는다/.test(krx갈래?.이름 ?? ''));
