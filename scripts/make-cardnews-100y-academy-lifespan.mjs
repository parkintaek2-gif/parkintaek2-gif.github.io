/**
 * make-cardnews-100y-academy-lifespan.mjs — 카드뉴스 「학원 종류별 사업장 수명」 5장
 *
 * 소스 지면 — /academy-lifespan (국민연금 가입 사업장 내역, 학원 종류별)
 * ⛔ 이 카드의 수는 전부 그 지면에 있는 수다. 새로 재지 않는다.
 *
 * 쓰는 법  node scripts/make-cardnews-100y-academy-lifespan.mjs [--selftest]
 */
...
const 자료 = JSON.parse(fs.readFileSync(path.join(ROOT, 'src/data/100yearmap/academy-lifespan-by-type.json'), 'utf8'));

export const 갈곳 = '100yearmap.com/academy-lifespan';
export const 바닥 = `${자료.출처.이름} · 학원 종류별 사업장 수명`;

const 표 = [...자료.자료].sort((a, b) => b.수명중앙값 - a.수명중앙값);
const 가장긺 = 표[0];
const 가장짧음 = 표[표.length - 1];
const 배수 = Math.round((가장긺.수명중앙값 / 가장짧음.수명중앙값) * 10) / 10;
const 일반교과 = 표.find((r) => r.업종 === '일반 교과학원');
  ...
  return [
    {
      꼴: '표지',
      큰수: `${가장짧음.수명중앙값}년`,
      줄들: [`${가장짧음.업종}`, '사업장 수명 중앙값입니다. (국민연금 자료)'],
    },
    {
      머리: '학원 종류마다 다릅니다',
...
if (process.argv.includes('--selftest')) {
  ...
  본다('② 표지에 큰 수가 있다', 장들[0].꼴 === '표지' && 장들[0].큰수.includes(String(가장짧음.수명중앙값)));
  본다('③ 모든 장에 데려갈 주소가 있다',
...
/* 🔴 여기부터가 «부르면 도는 몸»이다 */
const 내가직접불렸나 = !!process.argv[1] && path.basename(process.argv[1]) === 'make-cardnews-100y-academy-lifespan.mjs';
if (내가직접불렸나) {
  ...
  for (let i = 0; i < 장들.length; i++) {
    ...
    await sharp(Buffer.from(svg)).png().toFile(path.join(낼방, `학원생존기간-${i + 1}.png`));
    console.log('✅', `학원생존기간-${i + 1}.png`);
  }
