...
/**
 * 🔴 [2026-09-09 05:5x · 5번] **이 자와 `check-utc-today.mjs` 가 서로의 시험 견본을 잡고 있었다.**
 *   이 자는 같은 줄에 `검(` 이 있으면 건너뛰었지만, 견본이 두 줄로 나뉘면 못 걸렀다.
 *   ⇒ 오늘 톱니가 13 → 18 로 «헛으로» 늘었다. 새로 걸린 5곳이 전부 남의 시험 견본이었다.
 * ✅ 그래서 줄 단위 판정을 `scripts/lib/live-code.mjs` 한 곳에 두고 둘이 같이 쓴다.
 *   ⛔ 파일 이름으로 빼지 않는다 — 그러면 진짜 결함이 생겨도 영영 안 잡힌다.
 */
export function 어긋난자리찾기(글) {
  const 걸린 = [];
  /**
   * ⭐ 주석 줄을 버리고, «따옴표 안»에 든 무늬를 `<시험견본>` 으로 바꾼 글을 본다.
   *   ⛔ 아래의 「같은 줄에 `검(` 이 있으면 건너뛴다」만으로는 모자랐다 —
   *     견본이 두 줄로 나뉘면 그 줄만 봐서는 시험인지 알 수 없다.
   *   ⚠ 줄 수는 그대로 유지된다(살아있는글 이 줄을 지우지 않고 비운다). 그래서
   *     아래에서 내는 «줄 번호»가 원본과 어긋나지 않는다.
   */
  const 줄들 = 살아있는글(String(글 ?? '')).split(/\r?\n/);
 ...
 */
const 오늘까지봐주는수 = 9;   /* 🔴 [2026-09-09 · 5번] 13 → 9 로 내렸다. 5번(KCW) 몫 2곳을 KST 로 고치고,
                                두 검사기가 서로의 «시험 견본»을 잡던 헛경보를 lib/live-code.mjs 로 없앴다.
                                ⛔ 이 수는 «내려가기»만 한다. 올려서 통과시키지 않는다. */
/*
