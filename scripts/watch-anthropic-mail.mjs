...
/**
 * Gmail 목록의 한글 때를 읽는다 — 「2026년 9월 23일 (수) 오후 2:49」.
 * ⛔ 못 읽으면 null. 못 읽은 것을 「오래됐다」로 치지 않는다.
 */
export function 한글때읽기(글) {
  const s = String(글 ?? '');
  /* ⚠ 「일」과 시각 사이에 「(수)」가 낀다. 그 사이를 탐욕적으로 먹으면 오전/오후를
     같이 삼켜 «오후 2:49» 가 «2시 49분» 이 된다 — 그래서 오전/오후는 따로 찾는다. */
  const m = s.match(/(\d{4})년\s*(\d{1,2})월\s*(\d{1,2})일[^0-9]*?(\d{1,2}):(\d{2})/);
  if (!m) return null;
  const 낮밤 = (s.match(/(오전|오후)/) || [])[1] ?? null;
  let 시 = Number(m[4]);
  if (낮밤 === '오후' && 시 < 12) 시 += 12;
  if (낮밤 === '오전' && 시 === 12) 시 = 0;
  return new Date(Number(m[1]), Number(m[2]) - 1, Number(m[3]), 시, Number(m[5])).getTime();
}

/**
 * 우리가 보낸 제목과 «정확히» 같고, 그 발송이 막 전이면 내 것이다.
 *
 * 🔴🔴 [2026-09-23 15:0x] **이 규칙이 «진짜 답장»을 가렸다.**
 * 14:44 에 보낸 편지에 Fin 이 14:49 에 답했는데, 제목이 같고 2시간 안이라 걸러졌다.
 * 사장님이 「클로드 메일 옴」이라고 두 번째로 알려 주셔야 알았다 — 이 자를 만든 목적이
 * 바로 그 수고를 없애는 것이었는데 내가 오늘 아침에 넣은 그물이 그것을 막았다.
 *
 * ⇒ **스레드의 때를 본다.** 우리가 보낸 뒤에 스레드가 «더 움직였으면» 답장이 온 것이다.
 *   제목이 같아도, 2시간 안이어도, 그때는 거르지 않는다.
 * ⛔ 때를 못 읽으면 «거르지 않는다» — 놓치는 쪽보다 한 번 더 보는 쪽이 낫다.
 */
export function 내가방금보낸것인가(줄, 우리가보낸것, 이제 = Date.now()) {
  const 제목 = String(줄?.제목 || '').trim();
  if (!제목) return false;
  const 스레드때 = 한글때읽기(줄?.때);
  return (우리가보낸것 ?? []).some((x) => {
    if (String(x?.제목 || '').trim() !== 제목) return false;
    const 때 = Number(x?.때);
    if (!Number.isFinite(때)) return false;
    if (!(이제 - 때 >= 0 && 이제 - 때 < 막보낸시간)) return false;
    /* 🔴 우리가 보낸 뒤에 스레드가 더 움직였으면 답장이 온 것이다 — 거르지 않는다.
       ⚠ 1분은 우리 발송이 편지함에 찍히는 틈이다. 그보다 더 뒤면 남이 쓴 것이다. */
    if (스레드때 !== null && 스레드때 > 때 + 60000) return false;
    return true;
  });
}
...
function 자가시험() {
  ...
  /* 🔴🔴 [2026-09-23 15:0x] 위 그물이 «진짜 답장»을 가렸다. 14:44 에 보냈고 14:49 에 답이 왔다 */
  잰다('한글 때를 읽는다 — 오후',
    한글때읽기('2026년 9월 23일 (수) 오후 2:49'), new Date(2026, 8, 23, 14, 49).getTime());
  잰다('한글 때를 읽는다 — 오전',
    한글때읽기('2026년 9월 23일 (수) 오전 9:05'), new Date(2026, 8, 23, 9, 5).getTime());
  잰다('오후 12시는 12시다', 한글때읽기('2026년 9월 23일 오후 12:30'), new Date(2026, 8, 23, 12, 30).getTime());
  잰다('오전 12시는 0시다', 한글때읽기('2026년 9월 23일 오전 12:30'), new Date(2026, 8, 23, 0, 30).getTime());
  잰다('⛔ 못 읽으면 null', 한글때읽기('어제'), null);

  const 보낸때 = new Date(2026, 8, 23, 14, 44).getTime();
  const 대장2 = [{ 때: 보낸때, 제목: 'Conv 215476021858427 was closed' }];
  잰다('🔴 내가 보낸 뒤 스레드가 더 움직였으면 답장이다 — 거르지 않는다',
    내가방금보낸것인가({ 제목: 'Conv 215476021858427 was closed', 때: '2026년 9월 23일 (수) 오후 2:49' },
      대장2, new Date(2026, 8, 23, 14, 56).getTime()), false);
  잰다('⚠ 내 발송 그 시각 그대로면 아직 내 것이다',
    내가방금보낸것인가({ 제목: 'Conv 215476021858427 was closed', 때: '2026년 9월 23일 (수) 오후 2:44' },
      대장2, new Date(2026, 8, 23, 14, 46).getTime()), true);
  잰다('⛔ 스레드 때를 못 읽으면 거르지 않는다 — 놓치는 쪽보다 한 번 더 보는 쪽이 낫다',
    내가방금보낸것인가({ 제목: 'Conv 215476021858427 was closed', 때: '어제' },
      대장2, new Date(2026, 8, 23, 14, 46).getTime()), true);
