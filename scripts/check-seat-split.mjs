...
/* 사장님이 「그대로 두라」 하신 자리 — 흠으로 세지 않고 사실로만 적는다.
   2026-09-01 사장님 — 「admin@klifedesign.net >> 이건 내자리다」
                      「1번만 내 개인계정을 썼다」 「2번은 냅둬」
   ⚠ 한 곳에만 적는다. 두 군데 적으면 반드시 갈라진다.

   🔴 [2026-09-17] 사장님 — 「**2번과 6번을 맞바꿔**」
     그날 계정을 맞바꿨다. 2번 ↔ 6번 — admin 자리가 2번에서 **6번**으로 옮겨 갔다.
     ⇒ 허락된 자리도 함께 옮긴다. 안 옮기면 사장님이 «시키신 대로» 해 놓고
       이 자가 그것을 흠이라고 매시 빨간불을 켠다. **일을 옮기면 그것을 보는 자까지 따라간다.**
     ⚠ 왜 2번을 뺐나 — 2번은 이제 u6@klifedesign.net 이라 사장님 계정이 아니다.
       목록에 남겨 둬도 걸릴 일이 없지만, 남겨 두면 다음 사람이 「2번이 아직 admin 인가」로 읽는다. */
const 허락된자리 = ['6', '7'];
...
export function 진단(표) {
  ...
  /* ③ 사장님 계정을 유닛이 쓰고 있는가 — primary_owner 는 사람 자리다.
     ⚠ 허락된자리(2·7번)는 사장님이 그대로 두라 하신 것이라 흠이 아니다. */
  const 사장님계정 = 산것.filter(([번호, v]) => v.역할 === 'primary_owner' && !허락된자리.includes(번호));
  const 허락된것 = 산것.filter(([번호, v]) => v.역할 === 'primary_owner' && 허락된자리.includes(번호));

  /* ④ 추가 사용량이 조직 차원에서 막혀 있는가 */
  const 추가막힘 = 산것.length > 0 && 산것.every(([, v]) => !v.추가사용);

  return { 산것, 못잰것, 겹친계정, 최고좌석, 낮은것, 모르는좌석, 사장님계정, 허락된것, 추가막힘 };
}
...
function 낸다() {
  ...
  /* 사장님이 그대로 두라 하신 것은 흠이 아니라 «사실»로만 적는다 */
  for (const [번호, v] of (d.허락된것 || [])) {
    console.log(`⚪ ${번호}번은 사장님 자리(${v.계정})를 그대로 쓴다 — 사장님 「2번은 냅둬」. 흠으로 세지 않는다`);
  }
  for (const [계정, 번호들] of d.겹친계정) {
    if (번호들.every((x) => 허락된자리.includes(x))) {
      console.log(`⚪ ${번호들.map((x) => x + '번').join('·')} 의 계정 겹침은 사장님이 그대로 두라 하신 것이다`);
      continue;
    }
