...
/* 🔴 [2026-09-19] 도는 자리·접힌 자리 목록은 한 곳에서 읽는다 — 여기 또 적지 않는다 */
import { 도는자리 as 도는자리목록, 접힌자리 as 접힌자리목록, 설정뿌리 } from './lib/도는자리.mjs';

/* 유닛 번호 → 설정 뿌리. 뿌리를 «안 거는» 유닛은 기본 `.claude` 를 쓴다 —
   그것이 바로 이번 사고의 씨앗이었다(1번·7번이 사장님 계정을 쓰게 된 경로). */
const 기본뿌리 = '.claude';
/* 🔴 [2026-09-19] **도는 자리는 1·2·5번 셋뿐이다. 3·4·6번은 닫혔다.**
 *
 * 사장님: 「**내일부터 1, 2, 5번 세션만 보고해라**」(2026-09-18)
 *
 * [실측 2026-09-19 12:30]
 *   6번 — 스스로 「자리 없음, 실요청 대기로 전환」(9/18 15:09) → 「무응답 전환」(9/18 20:25).
 *         오늘 커밋 0건. 6번 몫이던 SeoulMarkets 결제 점검은 5번이 넘겨받았다(9/18 인계)
 *   3번·4번 — 마지막 커밋 9/18. 오늘 0건
 *
 * ⛔ 닫힌 자리를 계속 재면 «거짓 빨강»이 켜진다. 실제로 그랬다 —
 *   「2번·6번이 계정 하나를 나눠 쓴다」가 흠으로 올라왔는데, 6번은 돌지도 않으므로
 *   한도를 나눠 쓰는 일이 없다. 그 빨간불이 진짜 흠을 가린다.
 * ⚠ 그렇다고 «없는 것»으로 지우지 않는다 — 아래 닫힌자리 에 남겨, 자격증명이 아직
 *   붙어 있는지는 계속 본다. 남은 로그인은 언제든 다시 도는 길이다.
 */
/* ⛔ 목록을 여기 다시 적지 않는다 — scripts/lib/도는자리.mjs 가 정본이다.
   7번은 설정 뿌리가 없다(자리가 아니라 기본 뿌리에 붙은 이름이었다) — 뿌리가 있는 것만 살핀다. */
const 닫힌자리 = Object.fromEntries(접힌자리목록
  .map((자리) => [Number(String(자리).replace('번', '')), 설정뿌리[자리]])
  .filter(([, 뿌리]) => !!뿌리));

const 유닛뿌리 = {
  /* [2026-09-01 20:3x] 사장님이 u1@ 을 팀에 넣어 주셔서 1번을 전용 뿌리로 옮겼다.
     ⛔ [2026-09-19] 여기 「1·2·5」를 손으로 적어 두었다가, 자리를 줄인 뒤에도 다른 자들과
        갈라졌다. 목록은 scripts/lib/도는자리.mjs 한 곳에서 읽는다. */
  ...Object.fromEntries(도는자리목록
    .map((자리) => [Number(String(자리).replace('번', '')), 설정뿌리[자리]])
    .filter(([, 뿌리]) => !!뿌리)),
  /* 🔴 [2026-09-17] **「7번」을 뺐다 — 자리가 아니었다.**
   * 사장님: 「**7번은 뭐야? 난 6번까지 있는 지 알았지**」
   * 재 보니 맞는 말씀이다 —
   *   · 전체 이력에서 「7번」 이름으로 커밋한 것이 **1건**
   *   · `docs/업무분장*.md` 어디에도 없다
   *   · 그저 «기본 설정 뿌리»(.claude)에 붙여 둔 이름이었다
   * ⇒ 자리가 아닌 것을 자리로 세니 사장님이 모르는 유닛이 화면에 떴고,
   *   좌석 어긋남·계정 겹침 경고도 그만큼 부풀었다.
   * ⛔ 그렇다고 기본 뿌리를 «안 보는» 것은 아니다 — 아래 `기본뿌리살피기()` 가
   *   여전히 본다. 거기서 뭔가 돌면 그것은 사장님 계정으로 도는 것이라 알려야 한다.
   *   다만 «유닛»으로는 안 센다. 유닛은 1~6번이다. */
};
...
/* 사장님이 「그대로 두라」 하신 자리 — 흠으로 세지 않고 사실로만 적는다.
   ...
   🔴 [2026-09-19] **admin 자리가 다시 2번으로 왔다.**
     사장님이 직접 그 계정으로 2번을 열어 두시고 알려 주셨다 —
       「**로그인 됨. 일하라고 했고. 담당업무와 2번임을 알려줬다**」
     그리고 같은 날 도는 자리가 **1·2·5 셋**으로 줄었다(보고 대상도 그 셋뿐).
     6번·7번은 이제 없는 자리라 목록에 두면 «아무도 안 걸리는 이름»만 남는다.
     ⇒ 허락된 자리를 2번으로 옮긴다.
     ⛔ 이 자가 빨갛다고 사장님이 손수 해 두신 로그인을 「고치십시오」로 올리지 않는다.
       사장님이 아시고 정하신 것이다 — 값(사장님 몫이 깎이는 것)은 아래에서 사실로만 적는다. */
const 허락된자리 = ['2'];
...
/**
 * 기본 뿌리(`.claude`)를 «유닛이 아니라 사실»로 살핀다.
 *
 * 🔴 [2026-09-17] 여기 있던 것을 「7번」이라 부르며 유닛으로 세고 있었다.
 *   사장님: 「7번은 뭐야? 난 6번까지 있는 지 알았지」 — 자리가 아니었다(커밋 1건·업무분장 없음).
 * ⛔ 그렇다고 «안 보면» 안 된다. 여기에 로그인이 남아 있으면 세션이 뿌리를 안 걸었을 때
 *   **사장님 계정으로 도는 길**이 열려 있는 것이고, 그것이 2026-09-01 사고의 씨앗이었다.
 * ⇒ 유닛 표에서는 빼되, 아래 한 줄로 «무엇이 붙어 있는지»는 계속 말한다.
 */
export function 기본뿌리살피기(읽기 = 계정읽기) {
  const v = 읽기(기본뿌리);
  if (v.못잼) {
    console.log(`⬜ 기본 뿌리(${기본뿌리}) — 못 쟀다: ${v.못잼}. 「비어 있다」가 아니다`);
    return { 붙었나: null };
  }
  console.log(`⚪ 기본 뿌리(${기본뿌리}) 에 ${v.계정} 이 붙어 있다 — **유닛이 아니다.** 도는 유닛은 1·2·5번이다`);
  console.log('   세션이 자기 뿌리를 안 걸면 이 계정으로 돈다. 사장님 계정이면 사장님 몫이 깎인다');
  return { 붙었나: true, 계정: v.계정 };
}

/**
 * 닫힌 자리(3·4·6번)에 «로그인이 아직 남아 있는지»를 사실로만 적는다.
 * ⛔ 흠으로 세지 않는다 — 안 도는 자리는 한도를 나눠 쓰지 않는다.
 * ⚠ 그래도 본다 — 남은 자격증명은 언제든 다시 도는 길이고, 그 자리가 사장님 계정이면
 *   누군가 그 창을 열었을 때 사장님 몫이 깎인다.
 */
export function 닫힌자리살피기(읽기 = 계정읽기) {
  const 남은것 = [];
  for (const [번호, 뿌리] of Object.entries(닫힌자리)) {
    const v = 읽기(뿌리);
    if (v.못잼) continue;
    남은것.push({ 번호, 뿌리, 계정: v.계정 });
  }
  if (!남은것.length) return { 남은것 };
  console.log('');
  console.log('⚪ 닫힌 자리 — 사장님 「내일부터 1, 2, 5번 세션만 보고해라」(2026-09-18)');
  for (const x of 남은것) {
    console.log(`   ${x.번호}번 (${x.뿌리}) 에 ${x.계정} 로그인이 아직 남아 있다 — 흠은 아니다. 안 돌기 때문이다`);
  }
  console.log('   ⚠ 그 창을 누가 열면 그 계정으로 돈다. 사장님 계정이면 사장님 몫이 깎인다');
  return { 남은것 };
}
...
function 낸다() {
  ...
  /* 🔴 [2026-09-17] 좌석 등급 어긋남은 **더 이상 흠이 아니다.**
   *   사장님: 「**6번 안바꿔도 돼..스탠다드로 해도 돼**」
   *   ⇒ 사장님이 «그렇게 하기로 정하신 것»을 이 자가 매시 빨간불로 켜면,
   *     사람이 빨간불 자체를 안 보게 되고 그때 진짜 고장이 묻힌다.
   *   ⛔ 그렇다고 지우지 않는다 — 사실은 그대로 적는다. 흠으로만 안 센다.
   *   ⚠ 되살리려면 아래 console.log 를 잡는다() 로 바꾸면 된다. 한 줄이다. */
  if (d.낮은것.length) {
    console.log(`⚪ 좌석 등급이 갈린다 — 가장 센 것은 ${d.최고좌석} 인데 ${d.낮은것.map(([n, v]) => `${n}번(${v.좌석})`).join(' · ')} 이 그보다 낮다`);
    console.log('   사장님이 「스탠다드로 해도 돼」라 정하셨다(2026-09-17). 흠으로 세지 않는다');
  }
