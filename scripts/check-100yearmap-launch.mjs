...
/* ⚠ ⬜ 로 둔다 — **이 검사가 잰 것이 아니다.** 밖에서 잰 것을 여기 적어 둘 뿐이다.
   ⛔ 남이 잰 것을 내 통과 수에 얹으면, 그 자가 안 돌아도 여기서는 초록으로 보인다 */
못잼(
  '375px 가로 스크롤',
  '⭐ 2026-08-08 16:4x — **자를 만들었다.** `node scripts/check-100y-phone.mjs` 가 ' +
    '진짜 크롬을 375px 로 띄워 파는 3장·무료 2장·학교 1장을 잰다(자가시험 14건). ' +
    '⚠ 이 검사가 직접 안 부르는 까닭은 **크롬과 빌드가 있어야** 돌기 때문이다 — ' +
    '`npm run check:100y:phone` 으로 따로 부른다. ' +
    '🔴 이 자가 오늘 「파는 표에서 진학·그 밖이 화면 밖」을 잡았다. ' +
