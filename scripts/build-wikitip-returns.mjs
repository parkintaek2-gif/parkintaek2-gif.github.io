...
export async function 만들기() {
  ...
  /*
   * 🔴 2026-08-23 — 이 자가 5배를 요구하다 섰다(혼자 306 대 곁에 100 = 3.06배).
   *   자료가 세 주 늘고 작품이 902 → 983편이 되면서 비율이 내려왔다.
   * ⛔ 「걸렸으니 문턱을 낮춘다」로 고치지 않았다. **지면이 실제로 무슨 말을 하는지 다시 읽었다** —
   *   지면은 「in the main, not a sequel」이라고 쓰고 두 몫을 둘 다 찍는다. 5배는 그 문장이
   *   요구하는 것보다 센 조건이었다. 그래서 **문장이 요구하는 것으로 바꾼다** — 과반이다.
   * ⚠ 이것이 0.6 아래로 내려가면 문턱을 또 낮추지 말고 **그 문단을 다시 쓴다.**
   */
  const 혼자몫 = 시즌같음 ? 혼자 / 시즌같음 : 0;
  if (!(혼자몫 > 0.6)) {
    throw new Error(`혼자 ${혼자}/${시즌같음} = ${(100 * 혼자몫).toFixed(1)}% — 「in the main, not a sequel」이 안 선다`);
  }
