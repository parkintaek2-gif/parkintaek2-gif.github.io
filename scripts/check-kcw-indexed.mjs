 ...
 * 🔴 2026-08-23 실측 — **같은 열한 장을 몇 분 사이에 두 번 물었더니 갈림이 달라졌다.**
 *   (들어갔다 6 · 크롤안넣음 2 · 발견만 3) → (들어갔다 6 · 크롤안넣음 2 · 발견만 2 · 구글이모른다 1)
 *   ⛔ 그러니 **한 번 돌린 것을 «그 지면의 상태»로 못박지 않는다.** 그때 그 순간의 답이다.
 *   ⭐ 보고할 때 「몇 시 몇 분에 물었다」를 같이 적고, 갈림이 흔들린다는 것도 적는다.
 *     특히 갓 낸 지면은 구글 쪽에서 아직 자리를 잡는 중이라 더 흔들린다.
 *
 * ── 정직 규칙 ────────────────────────────────────────────────
...
/*
 * 🔴 [2026-08-30] **«구글이모른다» 가 어느 목록에도 안 떴다.**
 *   위(382줄)는 «발견만·크롤했는데안넣음»만 담고, 아래는 «모른다·못물었다»만 담는다.
 *   그런데 실제 꼴 이름은 `구글이모른다` 라 **둘 다에 안 걸린다.** 합계에만 나오고
 *   이름은 한 번도 안 불렸다 — 오늘 작품 지면 마흔 장을 재다 두 장이 그렇게 사라졌다.
 * ⛔ 이 갈래가 «제일 나쁜» 갈래다. 사이트맵에 넣어 줬는데 구글이 그 주소를 아예 모른다는 뜻이다.
 *   발견만(알지만 안 가지러 옴)보다 한 칸 더 앞이다. 그것이 조용한 쪽에 있었다.
 * ⚠ 「못 물었다」와 «갈라» 둔다 — 우리가 못 잰 것과 구글이 모르는 것은 다른 일이다.
 */
const 구글이모르는것 = 결과.filter((x) => x.꼴 === '구글이모른다');
if (구글이모르는것.length) {
  console.log(`
🔴 **사이트맵에 있는데 구글이 «주소 자체를» 모르는 것 ${구글이모르는것.length}장**`);
  console.log('   ⚠ 발견만(알지만 안 옴)보다 앞선 자리다 — 아직 «닿지도» 않았다');
  구글이모르는것.slice(0, 12).forEach((x) => console.log(`     ${x.주소.replace(집, '')}`));
}
