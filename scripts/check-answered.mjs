...
export function 줄읽기(줄) {
  ...
  const [처음시각, 물음, 상태, 히스토리, 어디에] = 칸;
  if (!물음) return null;
  return { 처음시각: 처음시각 || '', 물음, 상태: 상태 || '', 히스토리: 히스토리 || '', 어디에: 어디에 || '' };
}
...
export function 흠찾기(줄들) {
  const 흠 = [];
  for (const r of 줄들) {
    if (!/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$/.test(r.처음시각)) {
      흠.push({ 급함: '🔴', 말: `처음 말씀하신 시각이 없다 — node scripts/find-first.mjs 로 먼저 찾으십시오: 「${r.물음}」` });
    }
    if (!히스토리찾았나(r)) {
...
export function 요약글(줄들) {
  ...
  for (const r of 남은것) {
    const 표 = r.상태 === '막힘' ? '🖐' : '⏳';
    줄.push(`${표} **${r.물음}** — ${r.상태}${r.어디에 ? ` · ${r.어디에}` : ''}  *(${r.처음시각 || '시각 없음'})*`);
  }
  if (흠.length) {
    줄.push('');
...
if (내가실행됐다 && process.argv.includes('--자가시험')) {
  ...
  자가('칸이 모자라면 버린다', 줄읽기('2026-08-09 10:00\t물음') === null);
  const r = 줄읽기('2026-08-09 10:00\t대표 메일 언제\t답함\t✅\t내일 오전');
  자가('물음을 읽는다', r.물음 === '대표 메일 언제');
  자가('상태를 읽는다', r.상태 === '답함');
  자가('처음시각을 읽는다', r.처음시각 === '2026-08-09 10:00');
  자가('히스토리 표시를 읽는다', 히스토리찾았나(r) === true);
  자가('어디에를 읽는다', r.어디에 === '내일 오전');
  자가('빈 물음은 버린다', 줄읽기('2026-08-09 10:00\t\t답함') === null);

  자가('답함은 끝난 것', 끝났나(r) === true);
  자가('진행은 안 끝난 것', 끝났나({ 상태: '진행' }) === false);
  자가('막힘도 안 끝난 것', 끝났나({ 상태: '막힘' }) === false);

  const 안찾음 = 줄읽기('2026-08-09 10:00\t뭐냐\t답함\t⛔\t어딘가');
  자가('🔴 히스토리 안 찾은 것을 잡는다', 흠찾기([안찾음]).some((h) => h.급함 === '🔴'));
  자가('안 찾았다고 말해 준다', 흠찾기([안찾음])[0].말.includes('안 찾고'));
  자가('찾은 것은 안 잡는다', 흠찾기([r]).length === 0);

  // 🔴 처음시각 자물쇠 — 사장님(20:21) 「처음 지시/묻는 시간부터 챙기게」
  const 시각없음 = 줄읽기('\t뭐냐\t답함\t✅\t어딘가');
  자가('🔴 처음시각이 비면 잡는다', 흠찾기([시각없음]).some((h) => h.말.includes('처음 말씀하신 시각')));
  자가('🔴 날짜만 있고 때가 없으면 잡는다',
       흠찾기([줄읽기('2026-08-09\t뭐냐\t답함\t✅\t어딘가')]).some((h) => h.말.includes('처음 말씀하신 시각')));
  자가('find-first 로 찾으라고 알려 준다', 흠찾기([시각없음])[0].말.includes('find-first'));

  const 상태틀림 = 줄읽기('2026-08-09 10:00\t뭐냐\t나중에\t✅\t어딘가');
  자가('상태가 틀리면 잡는다', 흠찾기([상태틀림]).some((h) => h.말.includes('답함·진행·막힘')));

  const 어디에없음 = 줄읽기('2026-08-09 10:00\t뭐냐\t답함\t✅');
  자가('⛔ 답했다는데 어디에 답했는지 없으면 잡는다',
       흠찾기([어디에없음]).some((h) => h.말.includes('어디에 답했는지')));

  const 진행중 = 줄읽기('2026-08-09 11:00\t아직\t진행\t✅\t하는 중');
  const 글 = 요약글([r, 진행중]);
  ...
  자가('막힘은 손 표시로 찍는다', 요약글([줄읽기('2026-08-09 12:00\tx\t막힘\t✅\ty')]).includes('🖐'));
  자가('남은 것 옆에 처음시각을 적는다', 요약글([진행중]).includes('2026-08-09 11:00'));
