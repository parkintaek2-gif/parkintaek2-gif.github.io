...
/**
 * 본문 카드의 글자 크기 — **글이 짧으면 키운다.**
 *
 * 🔴 [2026-09-07] 한 문장짜리 카드가 1080 판 가운데에 작게 떠 있어 텅 비어 보였다.
 *   눈으로 열어 보고 알았다(사장님: 「만들면 렌더해서 눈으로 본다」).
 *
 * ⛔ 무한정 키우지 않는다 — 키운 크기로 «다시 줄을 나눠» 넘치면 한 칸 내린다.
 *   그래서 이 자는 «글자 수»가 아니라 «그 크기로 나눈 줄 수»로 판정한다.
 * ⚠ 표지는 이 자를 안 쓴다. 표지는 큰 수와 제목이 자리를 채운다.
 *
 * @param 글 카드 본문
 * @param 폭 판 너비(1080 등)
 * @param 여백 좌우 여백
 * @param 나누기 (글, 한줄글자수) → 줄 배열. 실제 줄나누기 를 넣어 «같은 자»로 잰다
 */
export function 본문글자크기(글, 폭, 여백, 나누기) {
  const 기본 = Math.round(폭 * 0.044);
  /* 키우는 단계 — 큰 것부터 본다. 맞는 첫 칸을 쓴다 */
  const 칸들 = [
    { 배: 1.85, 줄한도: 2 },   /* 한두 줄짜리 — 가장 크게 */
    { 배: 1.45, 줄한도: 4 },
    { 배: 1.2, 줄한도: 6 },
    { 배: 1, 줄한도: Infinity },
  ];
  for (const 칸 of 칸들) {
    const 크기 = Math.round(기본 * 칸.배);
    const 한줄 = Math.floor((폭 - 여백 * 2) / (크기 * 0.52));
    if (한줄 < 6) continue;                    /* 너무 커서 낱말도 안 들어가면 건너뛴다 */
    const 줄 = 나누기(글, 한줄);
    if (줄.length <= 칸.줄한도) return { 크기, 한줄, 줄 };
  }
  const 한줄 = Math.floor((폭 - 여백 * 2) / (기본 * 0.52));
  return { 크기: 기본, 한줄, 줄: 나누기(글, 한줄) };
}
...
export function 그리기(장, 번호, 총, 규, 주소) {
  ...
  /* 🔴 [2026-09-07] 본문 카드는 «글이 짧으면 글자를 키운다».
     한 문장짜리가 1080 판 가운데에 작게 떠 있어 텅 비어 보였다 — 눈으로 열어 보고 고쳤다.
     ⚠ 표지는 그대로 둔다. 표지는 큰 수와 제목이 이미 자리를 채운다. */
  const 잰것 = 장.꼴 === '표지'
    ? null
    : 본문글자크기(장.글, 폭, 여백, 줄나누기);
  const 글자크기 = 장.꼴 === '표지' ? Math.round(폭 * 0.052) : 잰것.크기;
  const 한줄 = 장.꼴 === '표지'
    ? Math.floor((폭 - 여백 * 2) / (글자크기 * 0.52))
    : 잰것.한줄;
  const 줄 = 장.꼴 === '표지' ? 줄나누기(장.글, 한줄) : 잰것.줄;
...
if (직접불렸나 && process.argv.includes('--selftest')) {
  ...
  /* ── 🔴 [2026-09-07] 짧은 카드가 텅 비어 보이던 것 ── */
  {
    const 나누기 = (글, n) => 줄나누기(글, n);
    const 짧 = 본문글자크기('Nine countries had none.', 1080, 96, 나누기);
    const 김 = 본문글자크기('Wikipedia publishes, for each country, the articles its readers opened most that day, and we took fourteen days of it across twenty countries to ask which of them are Korean.', 1080, 96, 나누기);
    const 기본 = Math.round(1080 * 0.044);
    참('🔴 짧은 글은 글자를 «키운다»', 짧.크기 > 기본);
    참('⛔ 긴 글은 짧은 글보다 «작게» 잡힌다 — 순서가 뒤집히면 안 된다', 김.크기 < 짧.크기);
    참('⛔ 긴 글도 기본보다 크게 잡힐 수 있다 — 판에 들면 그것이 낫다', 김.크기 >= 기본);
    참('키운 크기로 «다시» 줄을 나눈다', 짧.줄.length <= 2);
    참('긴 글은 여러 줄로 남는다', 김.줄.length > 4);
    참('한 줄에 들어가는 글자 수를 함께 낸다', 짧.한줄 > 0 && 김.한줄 > 0);
    참('⛔ 빈 글에도 안 터진다', 본문글자크기('', 1080, 96, 나누기).줄.length >= 0);
    /* ⛔ 키운 크기가 판을 넘지 않는지 — 줄 수 × 줄높이가 판 안에 든다 */
    참('⛔ 키운 뒤에도 판을 넘지 않는다',
      짧.줄.length * Math.round(짧.크기 * 1.38) < 1080 - 96 * 2);
    참('⛔ 긴 글도 판을 넘지 않는다',
      김.줄.length * Math.round(김.크기 * 1.38) < 1080 - 96 * 2);
  }
