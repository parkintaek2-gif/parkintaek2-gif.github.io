...
/**
 * 「키가」·「page가」 — 받침을 보고 조사를 고른다. 한글이 아니면 「가」로 둔다
 * (영문 칸 이름은 소리 나는 대로 읽으면 제각각이라 억지로 맞추지 않는다).
 */
export function 이가(말) {
  const s = String(말 ?? '');
  const 끝 = s.charCodeAt(s.length - 1) - 0xac00;
  if (끝 < 0 || 끝 > 11171) return '가';
  return 끝 % 28 ? '이' : '가';
}

/**
 * 주석을 «빈칸»으로 바꾼다 — 지우지 않고 길이와 줄 수를 그대로 둔다.
 *
 * ── 왜 이것이 필요한가 (2026-09-18 12:4x · 5번) ──────────────
 * 6번이 「HundredAgeWalk 에 필수 칸 «키» 가 안 넘어가는 자리가 있다」를 넘겨 줬다.
 * 실제로 세어 보니 지면 **69장 모두가 `키=` 를 옳게 넘기고 있었고**, 자가 집은
 * 한 자리는 컴포넌트 «자기 파일의 설명 주석» 안에 적힌 글자였다 —
 * ```
 * *  없었다(먼저 만든 세션이 이 파일을 빠뜨렸다) — <HundredAgeWalk> 를 불러도 …
 * ```
 * ⛔ **자가 코드가 아니라 «설명하는 글»을 재고 있었다.** 이런 빨간불은 두 번 해롭다 —
 *   고칠 것이 없는데 사람을 부르고, 그 자리가 늘 빨간 채로 있으면 «진짜» 빨강이 묻힌다.
 * ⚠ 길이를 줄이지 않고 빈칸으로 덮는 까닭은, 이 자의 다른 함수들이 «자리(index)»로
 *   꼬리표를 잘라 오기 때문이다. 길이가 달라지면 그 자리가 어긋난다.
 * ⚠ 한 줄 주석(`//`)은 «줄머리에 있는 것»만 걷는다 — 주소의 `https://` 를 주석으로
 *   잘못 읽으면 그 줄의 진짜 코드까지 통째로 덮어 «못 잡는» 쪽으로 틀린다.
 */
export function 주석지우기(글) {
  const s = String(글 ?? '');
  const 빈칸으로 = (m) => m.replace(/[^\n]/g, ' ');     // 줄바꿈은 남긴다
  let t = s;
  t = t.replace(/<!--[\s\S]*?-->/g, 빈칸으로);           // 지면 주석
  t = t.replace(/\{\s*\/\*[\s\S]*?\*\/\s*\}/g, 빈칸으로); // 템플릿 안의 {/* … */}
  t = t.replace(/\/\*[\s\S]*?\*\//g, 빈칸으로);          // /* … */ · /** … */
...
function 자가시험() {
  ...
  재('🔴 재발 방지 — 여러 줄 설명 주석 안의 꼬리표를 «쓴 자리»로 세지 않는다 (HundredAgeWalk 거짓 빨강)',
    지면재기(주석지우기('/**\n * <Rel> 를 불러도 앞뒤가 안 나온다\n */\n<Rel page="/x" />'), 'Rel', 칸), []);
  재('지면 주석(<!-- -->) 안의 것도 안 센다',
    지면재기(주석지우기('<!-- <Rel slug="x" /> -->\n<Rel page="/x" />'), 'Rel', 칸), []);
  재('템플릿 주석({/* */}) 안의 것도 안 센다',
    지면재기(주석지우기('{/* <Rel slug="x" /> */}\n<Rel page="/x" />'), 'Rel', 칸), []);
  재('줄머리 한 줄 주석도 걷는다',
    지면재기(주석지우기('  // <Rel slug="x" />\n<Rel page="/x" />'), 'Rel', 칸), []);
  재('⭐ 주석을 걷어도 «진짜» 어긋남은 그대로 잡는다',
    지면재기(주석지우기('/** 설명 */\n<Rel slug="x" />'), 'Rel', 칸).map((x) => x.갈래),
    ['없는이름', '필수빠짐']);
  재('주소의 // 를 주석으로 읽지 않는다 — 그 줄의 코드가 살아 있다',
    지면재기(주석지우기('<a href="https://x.kr">a</a> <Rel slug="x" />'), 'Rel', 칸).map((x) => x.갈래),
    ['없는이름', '필수빠짐']);
  재('받침 없는 이름엔 「가」', 이가('키'), '가');
  재('받침 있는 이름엔 「이」', 이가('제목'), '이');
  재('한글이 아니면 「가」', 이가('page'), '가');
  재('길이를 줄이지 않는다 — 자리(index)가 어긋나면 꼬리표를 못 자른다',
    주석지우기('/** 가 */\n<Rel page="/x" />').length,
    '/** 가 */\n<Rel page="/x" />'.length);
...
function 본일() {
  ...
  let 어긋난지면 = 0, 어긋난수 = 0;
  for (const f of 지면들) {
    /* ⛔ 설명 주석 안의 `<Comp>` 를 «쓴 자리»로 세지 않는다 — 위 주석지우기() 머리 참조 */
    const 글 = 주석지우기(fs.readFileSync(f, 'utf8'));
    const 여기 = [];
    ...
    for (const x of 여기) {
      if (x.갈래 === '없는이름') {
        ...
        console.log(`   <${x.컴}>  ← 반드시 넘겨야 할 «${x.이름}»${이가(x.이름)} 빠졌다`);
      } else {
        console.log(`   <${x.컴}>  ← ${x.말}`);
      }
    }
