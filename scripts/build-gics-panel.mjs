#!/usr/bin/env node
/**
 * SeoulMarkets — **GICS 업종 × 우리 고유 노동데이터** 패널.
 *
 *   node scripts/build-gics-panel.mjs                    전 섹터 요약을 낸다
 *   node scripts/build-gics-panel.mjs "Information Technology"   한 섹터 종목 시트 + 본보기 CSV
 *
 * ── 왜 (사장님 지시 2026-08-08) ───────────────────────────────
 * 남이 장악한 곳(GICS 분류 자체)엔 안 들어간다 — KRX·S&P·MSCI·FnGuide 가 이미 판다.
 ...
 *   src/data/ksic-to-gics.json    KSIC division → GICS 11 sector 근사 매핑(우리 것)
 *   archive/raw/stocks/<최신>.ndjson  종목별 시가총액(KRX 종가 기준)
 */
...
const gicsMap = JSON.parse(fs.readFileSync(path.join(ROOT, 'src/data/ksic-to-gics.json'), 'utf8')).map;
...
/* 각 종목에 GICS sector + 시총 붙이기 */
const 종목 = [];
for (const r of rows) {
  const ksic = r[C.industry];
  const g = gicsMap[ksic];
...
if (!고른섹터) {
  console.log(`시총 기준일 ${최신.replace('.ndjson', '')} · 상장사 ${종목.length} · 매핑된 시총 ${종목.filter((t) => t.시총).length}\n`);
  console.log('GICS SECTOR                    종목  시장비중  상위3집중  근속중앙  임금중앙(원)   여성비');
  for (const s of 섹터요약) {
    console.log(
      s.sector.padEnd(28),
  ...
  console.log('\n한 섹터 시트를 내려면:  node scripts/build-gics-panel.mjs "Information Technology"');
  process.exit(0);
}

/* ── 한 섹터: 종목 시트 + 본보기 CSV ─────────────────────────── */
const ts = (섹터별.get(고른섹터) ?? []).slice();
if (!ts.length) { console.log(`⛔ 그런 GICS 섹터가 없다: ${고른섹터}`); process.exit(1); }

const 시총합 = ts.reduce((s, t) => s + (t.시총 || 0), 0);
ts.sort((a, b) => (b.시총 || 0) - (a.시총 || 0));
/* 주도주 = 시총 상위, 누적이 섹터 시총의 70% 에 닿을 때까지. 나머지는 기타. */
let 누적 = 0;
for (const t of ts) { 누적 += t.시총 || 0; t.주도 = 누적 <= 시총합 * 0.70 || 시총합 === 0 ? '주도주' : '기타'; }
const 주도수 = ts.filter((t) => t.주도 === '주도주').length;

const CSV = [
  'gics_sector,ksic_industry,company_en,ticker,lead_or_other,market_cap_krw,employees,avg_annual_pay_krw,avg_tenure_years,female_to_male_pay_pct,female_share_pct,market_cap_per_employee_krw',
  ...ts.map((t) => [
...
const 낼곳 = path.join(ROOT, 'docs/상품안/본보기-gics-패널.csv');
fs.writeFileSync(낼곳, CSV);

console.log(`GICS 섹터: ${고른섹터}  ·  시총 기준일 ${최신.replace('.ndjson', '')}`);
console.log(`종목 ${ts.length}  ·  주도주 ${주도수}(시총 70% 누적)  ·  기타 ${ts.length - 주도수}`);
console.log(`섹터 시총 ${원(시총합)} 원  ·  근속중앙 ${중앙값(ts.map((t) => t.근속))?.toFixed(1)}y  ·  임금중앙 ${원(중앙값(ts.map((t) => t.임금)))}원`);
console.log('\n주도주 (시총 상위):');
