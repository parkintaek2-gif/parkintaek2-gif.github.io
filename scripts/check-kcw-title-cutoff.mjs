...
if (내가실행됐다) {
  ...
  for (const f of 글들) {
    ...
    const 태그에이름 = 사람이름 && (태그 ?? []).some((t) => 사람이름.some((n) => n.toLowerCase() === t.replace(/-/g, ' ').toLowerCase()));
  ...
  if (사람이름 === null) {
    ...
    const 태그빈것 = 이름있는제목.filter((x) => !x.태그에이름);
    console.log(`\n⚠ **제목에 스타 이름이 있는데 «태그»에는 없는 것 ${태그빈것.length}편** (이름 든 제목 ${이름있는제목.length}편 중)`);
    console.log('   사장님: 「인기 검색어는 스타 이름·작품명·노래제목이다」 — 태그가 갈래말뿐이면 그 말과 안 만난다.');
  }
