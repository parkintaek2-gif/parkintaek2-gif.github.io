 ...
 *
 * 🔴🔴 [2026-08-28 · 5번] **위 세 줄은 «물러난» 주장이다 — 이 자를 돌린 사람이 그대로 믿지 않게 적는다.**
 *   ─────────────────────────────────────────────────────────────────────
 *   ① 8/25 에 이 창이 스스로 물렀다 — 「판박이를 **83% → 82%** 로 고쳤는데 색인이 안 바뀌었다.
 *      내 자가 «판박이 문장»이 아니라 «같은 주제라 낱말이 같은 것»을 재고 있었다. 자를 과신했다」
 *   ② 8/28 에 갈래를 더 재서 점이 셋이 되자 관계가 깨졌다 (URL Inspection 표본 20장씩) —
 *        born-on  저만의 글 57% → 색인 **100%**
 *        group             35% → **80%**   ← 가장 판박이인데 title 보다 «높다»
 *        title             54% → **70%**
 *   ⚠ 두 점(title·article)은 언제나 선에 맞는다. 그것이 함정이었다.
 *   ⛔ 그러니 이 자의 «수»는 그대로 쓰되, **「이 수가 색인을 설명한다」로는 쓰지 마라.**
 *      같은 밤에 「인입 링크가 적어서」·「지면이 얇아서」·「자료가 적어서」도 다 반증됐다.
 *      색인을 가르는 것이 무엇인지는 **아직 못 가렸고, 못 가렸다고 적는 것이 우리 강령이다.**
 *
 * ── ⛔ 이 자를 읽는 법 ────────────────────────────────────────
...
/**
 * 🔴 [2026-09-11 06:1x · 5번] **이 자는 «낱말»을 재고 있었다. 「판박이」는 «말투»다.**
 *
 * 오늘 사람 지면 636장에 자료가 고르는 문단을 넣었다 — 쏠림·최고순위·나라 폭·해 폭을
 * 칸으로 나눠 문장 꼴을 111가지로 갈랐다(가장 큰 묶음 26장). 그런데 이 자는
 * **89% → 88%** 라고 했다. 재 보니 까닭이 자에 있었다 —
 *
 * ```
 * 낱말 주머니   같은 갈래 지면은 «쓰는 낱말»이 원래 같다(표 머리·나라 이름·chart·places).
 *              문장을 바꿔도 주머니는 안 바뀐다 ⇒ 이 수는 «어휘»를 재는 수다
 * 이음말(3낱말) 같은 «구절»이 통째로 겹치는지를 잰다 ⇒ 이 수가 «말투»를 재는 수다
 * ```
 *
 * ⭐ 구글의 중복 판정은 뒤쪽(구절)에 가깝다. 그런데 우리는 앞쪽 수를 보고 일을 정해 왔다.
 * ⛔ 그러니 하나만 내지 않는다 — **둘을 나란히 낸다.** 어휘가 높은 것은 갈래 지면의
 *   자연스러운 성질이고, 고칠 수 있는 것은 구절 쪽이다.
 * ⚠ 그리고 판박이선(50·65)은 «어휘» 쪽에서 나온 선이다. 구절 쪽 선은 아직 «못 쟀다» —
 *   재려면 구절 겹침과 색인 여부를 다시 맞춰 봐야 한다. 옛 선을 새 수에 그대로 쓰지 않는다.
 */
export function 이음말(글, n = 3) {
  const 낱말들 = String(글 ?? '').toLowerCase().split(/[^a-z0-9가-힣]+/).filter(Boolean);
  const 것 = new Set();
  for (let i = 0; i + n <= 낱말들.length; i += 1) 것.add(낱말들.slice(i, i + n).join(' '));
  return 것;
}
...
if (내가실행됐다 && process.argv.includes('--자가시험')) {
  ...
  검('이음말: 세 낱말씩 자른다', 이음말('one two three four').size === 2);
  검('⛔ 이음말: 세 낱말이 안 되면 없다', 이음말('one two').size === 0);
  검('🔴 이음말은 «말투»를 잰다 — 같은 낱말을 다른 순서로 쓰면 어휘는 100%, 구절은 낮다', (() => {
    /* ⚠ 낱말() 은 «두 글자 이하»를 버린다 — 시험 낱말을 세 글자 넘게 둔다.
       처음에 두 글자 한글로 적었더니 주머니가 비어 「못 잼」이 나왔다(자가시험이 잡았다) */
    const A = 'alpha bravo charlie delta'; const B = 'delta charlie bravo alpha';
    return Math.round(겹침(낱말(A), 낱말(B))) === 100 && 겹침(이음말(A), 이음말(B)) === 0;
  })());
  검('이음말: 같은 글은 100%', Math.round(겹침(이음말('one two three'), 이음말('one two three'))) === 100);
...
console.log('갈래              지면수   본문글자(중앙)   어휘겹침   구절겹침   판정(어휘)');
const 줄들 = [];
for (const [이름, 파일들] of [...갈래].sort((a, b) => b[1].length - a[1].length)) {
  ...
  const 길이 = []; const 주머니 = []; const 이음주머니 = [];
  for (const f of 뽑기) {
    let 본;
    try { 본 = 본문(readFileSync(f, 'utf8')); } catch { continue; }
    길이.push(본.length); 주머니.push(낱말(본)); 이음주머니.push(이음말(본));
  }
  const 겹 = 서로겹침(주머니);
  /* ⭐ 구절 겹침 — 「판박이」는 말투다. 어휘는 같은 갈래 지면이면 원래 겹친다 */
  const 구절 = 서로겹침(이음주머니);
  const 중 = 중앙값(길이);
  줄들.push({ 이름, n: 파일들.length, 중, 겹, 구절 });
  console.log(`${이름.slice(0, 16).padEnd(17)} ${String(파일들.length).padStart(6)}`
    ...
    + `   ${(구절 === null ? '못 잼' : `${구절.toFixed(0)}%`).padStart(8)}`
    + `   ${판정(겹)}`);
}
...
console.log('\n■ 구절 겹침(3낱말) — 「말투가 같나」. 어휘 쪽과 갈라 읽는다');
for (const x of 줄들.filter((y) => y.구절 !== null).sort((a, b) => b.구절 - a.구절).slice(0, 10)) {
  console.log(`   ${x.이름.slice(0, 16).padEnd(17)} ${String(x.n).padStart(5)}장`
    + `  구절 ${x.구절.toFixed(0)}%  어휘 ${x.겹 === null ? '못 잼' : `${x.겹.toFixed(0)}%`}`);
}
console.log('   ⚠ 이 수의 위험선은 **아직 못 쟀다** — 판박이선(50·65)은 어휘 쪽에서 나온 선이다.');
