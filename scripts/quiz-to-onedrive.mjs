...
export function 글만들기(오늘, 문제들 = 모든문제) {
  const 줄 = [
    ...
    '| 자리 | 문제 수 |',
    '|---|---:|',
    ...Object.keys(자리이름).map((n) => `| ${n}번 ${자리이름[n]} | ${문제뽑기(n, 문제들, 오늘).length}문제 |`),
    '',
    '---',
    '',
  ];
  ...
  for (const n of Object.keys(자리이름)) {
    const 것 = 문제뽑기(n, 문제들, 오늘).filter((q) => !이미.has(q.물음));
...
if (process.argv.includes('--selftest')) {
  ...
  /* ⚠ 보기를 날마다 섞으므로 답 번호를 굳혀 놓고 재면 안 된다.
   *   ⭐ **그날 그 자리가 보는 번호**와 같은지를 잰다 — 그래야 사장님이 보시는 답이 맞는 답이다 */
  const 그날3 = 문제뽑기('3', 본, '2026-08-09').find((q) => q.물음 === 'ㄱ');
  재본다('정답과 까닭이 실린다', 글.includes(`정답 **${그날3.답}** — 까닭ㄱ`), true);
  재본다('실린 답이 그날 보기 차례와 맞는다', 그날3.보기[그날3.답 - 1], 'b');
  ...
  /* 🔴 사장님 — 「그날치것만」. ⛔ 그날 안 낸 문제가 올라가면 안 된다 */
  재본다('그날 안 낸 문제는 안 실린다', (() => {
    const 오늘 = '2026-08-09';
    const 낸것 = new Set(Object.keys(자리이름).flatMap((n) => 문제뽑기(n, 모든문제, 오늘).map((q) => q.물음)));
    const 안낸것 = 모든문제.map((q) => q.물음).filter((물음) => !낸것.has(물음));
    const 글 = 글만들기(오늘);
    return 안낸것.length > 0 && 안낸것.every((물음) => !글.includes(`**${물음}**`));
  })(), true);
  재본다('날이 다르면 실리는 것도 다르다', 글만들기('2026-08-09') !== 글만들기('2026-08-10'), true);
