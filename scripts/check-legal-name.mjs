...
const 예외 = /(아니다|아니라|옛 이름|옛 상호|이전 상호|틀린 표기|틀린 상호|잘못 적|적혀 있었|바꿨다|였다)/;
...
/* 🔴 [2026-09-18 · 5번] **`docs/보관/` 을 통째로 넣었다.**
 *   `세션간-메모\.md` 는 «보관본»(`세션간-메모-보관.md`)에 안 걸린다 — 이름이 다르다.
 *   그래서 보관본 안에 옮겨 적힌 옛 기록 둘이 늘 빨갛게 떠 있었고, 그중 하나는
 *   위 주석이 「검사가 비켜 가야 맞다」고 이미 판정한 **사장님 말씀 원문의 사본**이었다.
 *   ⛔ 보관은 «얼린 기록»이다. 고치면 기록이 거짓이 되고, 안 고치면 검사가 늘 빨갛다.
 *     늘 빨간 검사는 «진짜» 빨강을 옆에서 가린다 — 그것이 이 자를 무디게 만든다.
 *   ⚠ 이름 하나를 예외로 적는 것보다 «자리»(보관 폴더)로 적는 것이 오래 간다 —
 *     다음에 또 다른 파일이 보관으로 들어가도 따라온다. */
const 건너뛸것 =
  /(node_modules|dist|\.git|archive|[\\/]보관[\\/]|세션간-메모\.md|작업일지\.md|사장님-지시-대장\.md|사장님말씀-대장\.md|숙지-사장님지시|check-legal-name\.mjs)/;
