...
export function 칸이같나(칸, 값) {
  ...
  if (c === 값) return true;
  /**
   * 🔴 [2026-09-05 · 5번] **여기를 느슨하게 고쳤다가 되돌렸다. 그 과정을 적어 둔다.**
   *   `/streak-vs-read` 의 칸 「Share of 171 weeks」가 이 규칙에 걸려, 기사
   *   v-led-108… 이 「약속을 안 지켰다」로 잡혔다. 나는 「칸 안에 홑으로 있으면 통과」로
   *   바꿨고, 옛 구멍(「Parasite (2019 film)」의 19)이 안 열리는 것까지 시험으로 확인했다.
   *
   * ⛔ 그런데 **바로 아래 자가시험이 그 반대를 이미 못박고 있었다** —
   *   「칸 안에 «섞여» 있는 것은 안 친다」(`62개국과 25개 도시` 에 `62` 는 거짓).
   *   그 규칙은 2026-09-04 에 92편을 실제로 재고 나서 «일부러» 정한 것이다.
   *   ⇒ 한 사례를 근거로 그 결정을 뒤집는 것은 「이번엔 다르다」다. 되돌렸다.
   *
   * ✅ 옳은 고침은 **자가 아니라 지면**이다 — 그 수를 표의 «값 칸»에 실으면 된다.
   *   자를 글에 맞추지 않고, 글을 자에 맞춘다.
   */
