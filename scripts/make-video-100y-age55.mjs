...
/** 화면에서 수를 도로 캐낸다. ⚠ **주소 안의 숫자는 세지 않는다** */
export function 숫자캐기(글) {
  // ⚠ 지울 것이 셋이다. 하나라도 빼먹으면 「근거에 없는 수」로 헛경보가 난다 —
  //   ① <style> 통째   화면 꾸밈(1080·1920·색값 #12151c)은 손님이 읽는 수가 아니다
  //   ② 남은 태그       style="width:62.00%" 같은 것
  //   ③ 주소            100yearmap.com 의 「100」 (오늘 카드뉴스에서 걸렸다)
  const 알맹이 = String(글)
    .replace(/<style[\s\S]*?<\/style>/gi, ' ')
    .replace(/<[^>]*>/g, ' ')
    .replace(/https?:\/\/\S+/g, ' ')
    .replace(/\b[\w-]+\.(com|kr|net|org)\S*/g, ' ');
...
function 자가시험() {
  ...
  // ⚠ 「등수를 매기지 않습니다」는 **우리 집 문구**다. 그것까지 금지말로 세면 늘 걸린다.
  //   먼저 지우고 나서 본다 — 「안 한다」고 말하는 것과 하는 것은 다르다.
  const 온글 = 칸HTML(12)
    .replace(/<style[\s\S]*?<\/style>/gi, ' ')
    .replace(/<[^>]*>/g, ' ')
  ...
  본다('⑨ 데려올 주소가 있다', 온글.includes('100yearmap.com/age'));
