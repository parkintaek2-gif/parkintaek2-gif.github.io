 ...
 *
 * ⛔⛔⛔ **이 각도는 접었다 (2026-08-15). 자를 고치기 전에는 이 자료로 기사를 내지 않는다.**
 *
 * 세 번 고쳐 자료를 받아 놓고 나서, 실물 한 줄을 재고 알았다 —
 * ```
 *   「오징어 게임」 ko 문서의 첫 판  →  2025-01-02
 *   그런데 그 작품은 2021년 것이다
 * ```
 * ⭐ **첫 판 날짜는 「작품이 적힌 때」가 아니라 「문서가 지금 이 이름으로 존재한 때」다.**
 *   문서를 옮기거나(rename) 합치면 그 앞의 판이 딴 문서로 따라간다. 그러면 「생일」이
 *   실제보다 한참 뒤로 밀린다. Parasite(2019-02)·Kingdom(2018-08)처럼 멀쩡한 것도 있다 —
 *   ⛔ **문제는 어느 것이 오염됐는지 이 자료로는 가릴 수 없다는 것이다.**
 *
 * ⚠ 가리려면 그 언어판의 **월별 조회수**가 있어야 한다. 첫 판보다 앞선 달에 읽혔다면
 *   그건 이동이다(92편의 `창안에서생겼나` 가 그 자다). 지금 우리에겐 ko 판 조회수가 없다.
 *   ⛔ 그리고 네 언어판도 똑같이 오염된다 — 시차 자체를 못 믿는다.
 *
 * ⭐ 자료는 남긴다. 자를 갖추면 다시 온다. **하지만 지금 이것으로는 아무 말도 안 한다.**
 *
 * ── 왜 ─────────────────────────────────────────────────────────
 ...
 *
 * 🔴🔴🔴 **8/15 첫 수집이 헛것이었다 — 한국어판 제목을 안 받았다.**
 *   파도 자료에는 `titles.id/vi/th/ms` 만 있고 **ko 가 없다.** 그래서 영어 제목
 *   `Squid Game` 을 ko 위키에 물었고, 넘김이 있는 몇 편 말고는 다 빈손이었다.
 *   ⭐ 59편 중 **한국어판 생일을 안 것이 1편**이었다. 기준이 없으니 시차도 없다.
 *   ⛔ 다음에 고칠 것 — `langlinks` 로 **ko 제목부터 받는다.** 그 전에는 이 자료로
 *     아무 말도 하지 않는다. 「1편으로 잰 중앙값 −40달」 같은 것을 내면 안 된다.
...
export function 한작품재기(생일들, 기준 = 기준판, 볼것 = 볼판들) {
  ...
  /**
   * 🔴 8/15 — 기준판(ko)을 못 받은 것이 **어디에도 안 적히고** 있었다. 볼 판만 훑었기 때문이다.
   *   그러면 「다 받았다」 판정이 그 구멍을 못 본다. ⛔ 기준판도 같이 센다.
   */
  if (생일들[기준] === 못받음) 못받은판.push(기준);
  for (const p of 볼것) {
...
if (process.argv.includes('--selftest')) {
  ...
  /* 🔴 기준판을 못 받은 것이 어디에도 안 적히고 있었다 — 「다 받았다」가 구멍을 못 본다 */
  참('⛔⛔ 기준판을 못 받은 것도 센다',
    한작품재기({ ko: 못받음, id: '2021-09' }).couldNotFetch.includes('ko'));
  참('⛔ 기준판을 받았으면 안 센다',
    !한작품재기({ ko: '2021-06', id: '2021-09' }).couldNotFetch.includes('ko'));
...
if (내가실행됐다) {
  ...
  const 이전 = fs.existsSync(원본길)
    ? Object.fromEntries((JSON.parse(fs.readFileSync(원본길, 'utf8')).titles ?? [])
      .map((t) => [t.titleEn, {
        births: t.births ?? {}, titles: t.titles ?? {}, koTitleSource: t.koTitleSource ?? null,
      }]))
    : {};
  ...
  for (const a of 파도.articles) {
    ...
    /**
     * 🔴🔴 **어디서 얻은 제목인지 표를 남긴다.**
     *   두 번째 수집도 헛것이었다 — 첫 수집이 `titles.ko` 자리에 **영어 제목**을 넣어
     *   저장했고, 이어받기가 「앞서 받아 둔 ko 제목이 있다」고 믿어 langlinks 를 안 물었다.
     *   ⛔ 이어받기는 **값이 있나**가 아니라 **어떻게 얻었나**를 봐야 한다.
     */
    let ko제목 = 이전[a.titleEn]?.koTitleSource === 'langlinks'
      ? (이전[a.titleEn]?.titles?.[기준판] ?? null) : null;
    let ko출처 = ko제목 ? 'langlinks' : null;
    if (!ko출처) {
      const 답 = await 받기(언어판제목주소(a.titleEn));
      물은수 += 1;
      if (답 === 못받음) { ko제목 = null; ko출처 = '못받음'; } else {
        ko제목 = 언어판제목꺼내기(답);
        ko출처 = ko제목 ? 'langlinks' : '한국어판이 없다';
      }
      await 잠깐(1100);
    }
    ...
    나온것.push({
      titleEn: a.titleEn, titles: 제목들, koTitleSource: ko출처, births: 생일들, ...한작품재기(생일들),
    });
