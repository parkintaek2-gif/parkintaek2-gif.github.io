...
const 볼것 = [
  /**
   * 🔴 **값 지면은 늘 본다** — 표본이 아니다 (2026-08-09 04:5x 신설).
   *
   *   여기가 손님이 「살까」를 정하는 한 장이고, **딱 한 장뿐**이라 표본으로 뽑을 것이 없다.
   *   ⚠ 표가 셋 칸(무엇·몇·값)이라 375px 에서 잘리기 쉬운 자리다 —
   *     파는 지면 표가 실제로 그렇게 잘렸었다(8/8 16:4x, 이 자가 잡았다).
   */
  ...(fs.existsSync(path.join(빌드, 'price.html')) ? ['/price.html'] : []),
