...
for (let i = 1; i <= 몇번; i++) {
  ...
  /* ⚠ **왜 기다리는지를 적는다.** 2026-08-07 21:4x 에 이것이 없어서 곤란했다 —
   *   기록에는 「앞 배포가 아직 돈다」만 세 번 찍혀 있는데, Cloudtype 콘솔은
   *   `Running` 이었다. 도구가 무엇을 보고 기다리는지 알 수 없으니
   *   **기다리는 것이 맞는지 틀린지를 사람이 가릴 수가 없었다.** */
  const 왜 = (글.match(/[^\n]*(이미 Starting|이미 배포 중|배포 중이다|Starting 다)[^\n]*/) || [''])[0].trim();
  console.log(`⏳ ${때()} ${i}/${몇번} — ${쉬는초 / 60}분 뒤 다시 한다`);
  console.log(`   기다리는 까닭: ${왜 || '(못 찾음)'}`);
