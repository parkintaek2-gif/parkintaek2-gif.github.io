#!/usr/bin/env node
/**
 * build-working-hours-annual.mjs — **한국 근로자 1인당 월평균 근로시간, 연도별** (사업체노동력조사)
 *
 * ── 왜 (2026-08-29) ─────────────────────────────────────────────
 ...
 *   13103110311MD_7 = 전체근로시간(상용+임시일용, 근로자 1인당 월평균, 시간)
 ...
 *
 *   node scripts/build-working-hours-annual.mjs        # 받아 저장 + 차트
 *   node scripts/build-working-hours-annual.mjs --dry  # 저장 안 하고 재기만
 *   node scripts/build-working-hours-annual.mjs --자가시험
 */
...
if (process.argv.includes('--자가시험') || process.argv.includes('--selftest')) {
  ...
  console.log(`\n${process.exitCode ? '❌' : '✅'} build-working-hours-annual 자가시험 (${셈})`);
  process.exit();
}
...
const ITM = '13103110311MD_7'; // 전체근로시간(근로자 1인당 월평균, 시간)
...
console.log(`근로시간 — ${Object.keys(월별).sort()[0]}~${최근달} (${Object.keys(월별).length}개월)`);
...
const 출처 = {
  ...
  항목: '전체근로시간 13103110311MD_7 (상용+임시일용, 근로자 1인당 월평균, 시간)',
  모수: '전산업 · 상용근로자 1인 이상 사업체 · 전규모',
  이용허락범위: 'KOSIS 통계정보 활용약관 제8조 — 상업적 활용 가능',
};
fs.writeFileSync(path.join(ROOT, 'src', 'data', 'korea-working-hours.json'),
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" role="img" aria-label="Korea average monthly hours worked per worker, ${첫해}-${끝해}">
<rect width="${W}" height="${H}" fill="#fff"/>
<text x="${pad}" y="26" font-size="14" fill="#111" font-weight="600">Average monthly hours worked per worker — Korea, ${첫해}–${끝해}</text>
...
fs.writeFileSync(path.join(ROOT, 'public', 'charts', 'korea-working-hours.svg'), svg);
console.log('\n저장: src/data/korea-working-hours.json · public/charts/korea-working-hours.svg');
