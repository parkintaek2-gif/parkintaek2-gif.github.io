/**
 * make-cardnews-100y-kids-activity-lifespan.mjs — 카드뉴스 「태권도장 등 아동 체육기관 수명」 5장
 *
 * 소스 지면 — /kids-activity-lifespan (국민연금 가입 사업장 내역, 아동 체육·교육기관)
 * ⛔ 이 카드의 수는 전부 그 지면에 있는 수다. 새로 재지 않는다.
 *
 * 쓰는 법  node scripts/make-cardnews-100y-kids-activity-lifespan.mjs [--selftest]
 */
...
const 자료 = JSON.parse(fs.readFileSync(path.join(ROOT, 'src/data/100yearmap/kids-activity-lifespan.json'), 'utf8'));

export const 갈곳 = '100yearmap.com/kids-activity-lifespan';
export const 바닥 = `${자료.출처.이름} · 아동 체육·교육기관 사업장 수명`;

const 표 = [...자료.자료].sort((a, b) => b.수명중앙값 - a.수명중앙값);
const 가장긺 = 표[0];
const 가장짧음 = 표[표.length - 1];
const 배수 = Math.round((가장긺.수명중앙값 / 가장짧음.수명중앙값) * 10) / 10;
const 태권도 = 표.find((r) => r.업종 === '태권도 및 무술 교육기관');
  ...
  return [
    {
      꼴: '표지',
      큰수: `${태권도.수명중앙값}년`,
      줄들: [`${태권도.업종}`, '사업장 수명 중앙값입니다. (국민연금 자료)'],
    },
    {
      머리: '세 업종, 수명이 다릅니다',
...
if (process.argv.includes('--selftest')) {
  ...
  본다('② 표지에 큰 수가 있다', 장들[0].꼴 === '표지' && 장들[0].큰수.includes(String(태권도.수명중앙값)));
  본다('③ 모든 장에 데려갈 주소가 있다',
...
/* 🔴 여기부터가 «부르면 도는 몸»이다 */
const 내가직접불렸나 = !!process.argv[1] && path.basename(process.argv[1]) === 'make-cardnews-100y-kids-activity-lifespan.mjs';
if (내가직접불렸나) {
  ...
  for (let i = 0; i < 장들.length; i++) {
    ...
    await sharp(Buffer.from(svg)).png().toFile(path.join(낼방, `아동체육기관수명-${i + 1}.png`));
    console.log('✅', `아동체육기관수명-${i + 1}.png`);
  }
