 ...
 *
 *  🔴 2026-08-21 넓혔다 — `자가대조`(표 안에서 합이 맞는지 등 스스로 검산한 것)만 있고
 *     바깥 공표치와 견준 `대조` 칸이 없는 자료 10개를 이 자가 「아무 말 없음」으로 잘못 셌다.
 *     자가대조는 공표 대조보다 약하다(바깥 진위가 아니라 안쪽 앞뒤만 본다) — 그래서
 *     **같다고 하지 않고 「자가대조」라고 이름 붙여 따로 보여준다.** 판정을 흐리지 않는다.
 */
export function 대조요약(자료) {
  const 칸 = Object.keys(자료 ?? {}).filter((k) => /^대조$|공표.*대조|대조.*공표/.test(k));
  if (칸.length) return 칸.map((k) => 첫글월(자료[k])).filter(Boolean).join(' / ').slice(0, 160);
  if (/공표에 같은 칸이 없/.test(JSON.stringify(자료 ?? {}))) return '⛔ 공표에 같은 칸이 없다(그렇게 적어 둠)';
  const 자가칸 = Object.keys(자료 ?? {}).filter((k) => /^자가대조$/.test(k));
  if (자가칸.length) return `⭐ 자가대조(바깥 공표 대조 아님): ${자가칸.map((k) => 첫글월(자료[k])).filter(Boolean).join(' / ')}`.slice(0, 200);
  return '';
}
...
function 자가시험() {
  const 본보기 = [
    ...
    ['⭐ 자가대조만 있어도 빈 글이 아니다', () => 대조요약({ 자가대조: '합 100' }).includes('자가대조')],
    ['⭐ 자가대조는 공표 대조와 안 섞인다', () => 대조요약({ 자가대조: '합 100' }).startsWith('⭐')],
    ['⭐ 대조 칸이 있으면 자가대조보다 앞선다', () => 대조요약({ 대조: '공표와 0.03%', 자가대조: '합 100' }) === '공표와 0.03%'],
...
const 맞춘것 = 대장.filter((r) => r.대조 && !r.대조.startsWith('⛔') && !r.대조.startsWith('⭐ 자가대조')).length;
const 못맞춘것 = 대장.filter((r) => r.대조.startsWith('⛔')).length;
const 자가대조뿐 = 대장.filter((r) => r.대조.startsWith('⭐ 자가대조')).length;
...
console.log(`\n자료 ${대장.length}개 — 공표치와 맞춘 것 ${맞춘것} · 「못 맞췄다」고 적은 것 ${못맞춘것} · 자가대조뿐인 것 ${자가대조뿐}(바깥 공표 대조 아님) · 아무 말 없는 것 ${말없음}`);
