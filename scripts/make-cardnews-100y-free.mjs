...
/* 🔴 2026-08-16 — 여기부터가 «부르면 도는 몸»이다. 근거를 대 보려고 import 했다가
   다른 카드 자들이 곧바로 카드를 다시 그렸다. 이 자는 **200벌 넘게** 그리니 더 위험하다.
   ⇒ 내가 직접 불렸을 때만 돈다. 남이 불러 «짜기»만 얻어 갈 수 있게 한다.
   ⚠ import.meta.url 로 견주면 윈도에서 조용히 안 돈다. 파일 이름으로 견딘다
   ⚠ 개봉 카드 자(voc)는 이미 «내가실행됐다»라는 다른 이름으로 같은 것을 하고 있었다 —
     내 자가 이름만 보고 「안 감쌌다」고 셌다. 낱말이 아니라 **돌려 보고** 확인했다 */
const 내가직접불렸나 = !!process.argv[1] && path.basename(process.argv[1]) === 'make-cardnews-100y-free.mjs';
if (내가직접불렸나) {
