/**
 * collect-100y-cycling.mjs — 나이대별 자전거 참여율(최근 1년간 참여 경험)
 *
 * ── 왜 ─────────────────────────────────────────────────────────
...
const 내가직접불렸나 = !!process.argv[1] && path.basename(process.argv[1]) === 'collect-100y-cycling.mjs';
if (내가직접불렸나) {
  ...
  const 자전거줄 = 날.filter((x) => x.ITM_NM === 종목 && 나이만인가(x.C1));
  const 해들 = [...new Set(자전거줄.map((x) => x.PRD_DE))].sort();
  const 최신 = 해들[해들.length - 1];

  const 해별 = {};
  for (const 해 of 해들) {
    const 이해줄 = 자전거줄.filter((x) => x.PRD_DE === 해);
  ...
  const 낸다 = {
    무엇: '나이대별 자전거 참여율 — 최근 1년간 참여 경험(복수응답)',
    만든날: new Date().toISOString().slice(0, 10),
    출처: { 기관: '문화체육관광부', 표: '최근 1년간 참여 경험이 있는 체육활동(복수응답, 상위 10개 종목)', 창구: '국가데이터처 KOSIS', orgId: ORG, tblId: TBL },
    단위: '%',
    정의: '최근 1년 동안 자전거(사이클·산악자전거 포함)를 타 본 적이 있다고 답한 사람의 비율입니다(복수응답 — 다른 운동도 같이 했을 수 있습니다). 매일 타는 사람만이 아닙니다.',
    '⚠ 이 자료가 못 가르는 것': [
      '표본조사입니다. 나이대별 표본 크기가 달라 작은 칸일수록 오차가 큽니다.',
      '「참여 경험」이라 강도·빈도(한 번 타 본 사람과 매일 타는 사람을 못 가릅니다)를 못 잽니다.',
      '성별×나이를 교차한 값은 이 표에도 있지만(예: 남자 10대만) 이 지면은 나이만 봅니다 — 성별로 가르지 않았습니다.',
    ],
    해들, 최신, 해별,
  };
  const 낼곳 = path.join(뿌리, 'src/data/100yearmap/cycling.json');
