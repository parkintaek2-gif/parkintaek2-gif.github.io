...
if (path.basename(process.argv[1] ?? '') === 'build-100y-qual-duration.mjs') {
  ...
  /* 🔴 2026-08-16 — 「등급을 다 합치면 45.5%」라는 **하나의 수**를 숏영상·카드뉴스·지면이
     각자 계산하고 있었다. 같은 셈이 세 곳에 흩어지면 언젠가 어긋난다.
     ⇒ **자료가 낸다.** 셋은 읽기만 한다.
     ⚠ 등급별 비율을 그냥 평균 내면 안 된다 — 응시한 사람 수가 등급마다 크게 다르다. 가중한다 */
  const 온분모 = 등급.reduce((a, g) => a + g.응시분모, 0);
  const 가중몫 = (칸) => (온분모
    ? Number((등급.reduce((a, g) => a + (g[칸] / 100) * g.응시분모, 0) / 온분모 * 100).toFixed(1))
    : null);
  const 전체 = {
    무엇: '등급을 다 합친 값 — **사람 수로 가중**했다. 등급별 비율의 단순평균이 아니다',
    응시분모: 온분모,
    한번에: 가중몫('한번에'),
    두번: 가중몫('두번'),
    세번넘게: 가중몫('세번넘게'),
    '⛔ 걸린날은 왜 없나': '등급마다 걸린 날의 뜻이 달라 합치지 않는다. 걸린 날은 등급별로만 낸다',
  };
