...
export function 방들() {
  ...
  const 학습 = 읽기('lifelong.json');
  const 노후소득 = 읽기('retire-income.json');
  const 건강수명 = 읽기('healthy-years.json');
  const 문화관람 = 읽기('culture-attendance.json');
  ...
  return [
    ...
    {
      나이: '3~5세', 이름: '우리 동네 유치원을 찾는 사람',
      수: `전국 유치원 ${유치원.전국.곳.toLocaleString()}곳`,
      곁말: `원아 ${유치원.전국.원아.toLocaleString()}명 · 시·군·구 ${유치원.동네수}칸을 전부 실었습니다`,
      지면: '/kindergarten', 지면말: '우리 동네 찾아보기',
    },
    {
    ...
    {
      나이: '0~100세', 이름: '건강수명이 궁금한 사람',
      수: `잣대에 따라 ${건강수명.나이별.find((r) => r.나이 === 0).벌어짐}년이 벌어집니다`,
      곁말: '같은 해 같은 표인데도 두 잣대가 다른 것을 잽니다',
      지면: '/healthy-years', 지면말: '내 나이로 찾기',
    },
    {
    ...
    {
      나이: '성인', 이름: '계속 배우는 사람',
      수: `${학습['⭐ 나이 들수록 계속 낮아지나'].최고칸} 참여율 ${학습['⭐ 나이 들수록 계속 낮아지나'].최고값}%로 가장 높습니다`,
      곁말: `나이 들수록 ${학습['⭐ 나이 들수록 계속 낮아지나'].최저칸} ${학습['⭐ 나이 들수록 계속 낮아지나'].최저값}%까지 한 번도 안 꺾이고 내려갑니다`,
      지면: '/lifelong', 지면말: '나이대별로 보기',
    },
    {
      나이: '노후', 이름: '은퇴 앞둔·은퇴한 사람',
      수: `소득 ${노후소득['⭐ 소득이 가장 크게 떨어지는 구간'].변화율}% 급락하는 구간이 있습니다`,
      곁말: `같은 구간 자산은 ${노후소득['⭐ 자산이 떨어지는 구간(비교용)'].변화율}%만 줄어듭니다`,
      지면: '/retire-income', 지면말: '나이대별로 보기',
    },
    {
      나이: '전 연령', 이름: '문화예술을 즐기는 사람',
      수: `20대 영화 관람률 ${(문화관람.최신연령별.find((r) => r.연령 === '20대')).영화}%로 가장 높습니다`,
      곁말: `전통예술 공연만은 거꾸로 60·70대가 더 봅니다(${(문화관람.최신연령별.find((r) => r.연령 === '70세이상'))['전통예술 공연']}%)`,
      지면: '/culture-attendance', 지면말: '나이대별로 보기',
    },
  ];
}
