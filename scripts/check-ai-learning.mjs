 ...
 *
 * ── 🔴🔴 [2026-09-25] 사장님이 이 자의 «말»을 바로잡으셨다 ─────────────────
 *   「**결국 케이라이프맵과 에스마켓츠 담당 세션이 거짓을 보고한거다.
 *     자체ai를 학습시키고 있는 양…**」
 *
 *   ⛔ 이 자가 「배우고 있나」·「학습」이라고 말해 왔다. **그것이 거짓의 통로였다.**
 *     우리에게는 **훈련되는 모델이 없다.** 2026-08-04 에 2번이 이미 적어 두었다 —
 *     「LLM 호출은 저장소 어디에도 없습니다. 한 줄도 안 짰습니다」.
 *     그런데도 이 자가 초록 칸을 내면 보고서에 「자체 AI 학습 중」이 실린다.
 *     ⇒ **자가 거짓말을 하면 사람은 그것을 옮길 뿐이다.**
 *
 *   ✅ 그래서 말을 실제와 맞춘다 — 이 자가 재는 것은 「학습」이 아니라 **「자료 판독」**이다.
 *     ① 재료를 모으나 ② 그것을 읽나 ③ 새로 쌓이나 ④ 판단에 닿나 ⑤ 재료가 묵지 않았나.
 *   ✅ 그리고 **「모델이 있나」를 맨 앞에서 정직하게 말한다.** 없으면 없다고 적는다.
 *     없는 것을 「준비 중」·「구축 중」으로 적지 않는다.
 *
...
export function 투자AI를잰다(오늘 = 오늘날짜()) {
  ...
  /* 🔴 재료의 «나이» — 판독기가 실제로 읽은 자료가 언제 것인가.
     ⛔ 파일 수정시각이 아니라 «신호의 ts» 를 본다. 같은 자료를 다시 써도 파일 시각은 오늘이
       되지만 자료는 옛것 그대로다 — 2026-09-25 에 그 함정에 빠져 11일을 몰랐다. */
  const 재료나이 = (() => {
    try {
      const 글 = fs.readFileSync(path.join(방, 'state-latest.jsonl'), 'utf8');
      let 가장새것 = null;
      for (const l of 글.split('\n')) {
        if (!l.trim()) continue;
        let o; try { o = JSON.parse(l); } catch { continue; }
        const t = String(o?.ts ?? '').replace(/-/g, '');
        if (/^\d{8}$/.test(t) && (가장새것 === null || t > 가장새것)) 가장새것 = t;
      }
      return 가장새것 === null ? null : 며칠차이(가장새것, 오늘);
    } catch { return null; }
  })();
  ...
  return {
    ...
    칸들: [
      ...
      /* 🔴 [2026-09-25] 이 칸이 없어서 **밸류에이션 표가 11일 멈춘 것을 아무도 몰랐다.**
         ① 은 「파일이 있나」만 본다 — 열하루 묵은 파일도 초록이다.
         그 사이 판독기는 같은 신호를 날마다 다시 쓰고 있었고, 그것이 «일하는 것»처럼 보였다.
         ⛔ 멈춘 자료를 읽는 것은 배우는 것이 아니다. **재료의 나이를 잰다.** */
      칸('⑤ 🔴 재료가 묵지 않았나 — 판독한 자료의 나이', 재료나이 === null ? 999 : 재료나이, (v) => v <= 5,
        재료나이 === null ? '⛔ 못 쟀다 — state-latest.jsonl 이 없다'
          : `가장 새 자료가 ${재료나이}일 전 것이다 (닷새를 넘으면 빨강)`),
    ],
  };
}
...
if (직접돌리나 && process.argv.includes('--자가시험')) {
  ...
  본다('투자 AI 를 잰다', 투자AI를잰다().칸들.length === 5);
  /* 🔴 [2026-09-25] 「모델이 있나」를 재료가 있는 것으로 세지 않는다 —
     그 혼동이 「자체 AI 를 학습시키고 있다」는 거짓 보고를 낳았다 */
  본다('🔴 돌아가는 끝점이 없으면 «모델이 없다»고 말한다', (() => {
    const 본래 = process.env.SELF_AI_URL;
    try { delete process.env.SELF_AI_URL; return 모델이있나().있나 === false; }
    finally { if (본래 === undefined) delete process.env.SELF_AI_URL; else process.env.SELF_AI_URL = 본래; }
  })());
  본다('끝점이 있으면 있다고 말한다', (() => {
    const 본래 = process.env.SELF_AI_URL;
    try { process.env.SELF_AI_URL = 'http://x/v1'; return 모델이있나().있나 === true; }
    finally { if (본래 === undefined) delete process.env.SELF_AI_URL; else process.env.SELF_AI_URL = 본래; }
  })());
  /* ⛔ 이 자가 「학습」이라는 말로 재고 있으면 보고가 또 거짓이 된다 */
  본다('🔴 화면에 내는 말이 «학습»이 아니라 «자료판독»이다', (() => {
    const 글 = fs.readFileSync(fileURLToPath(import.meta.url), 'utf8');
    const 낸다 = [...글.matchAll(/console\.log\((['`])([^'`]*)\1/g)].map((m) => m[2]).join(' ');
    return 낸다.includes('자료판독') && !/둘이 «계속 배우고 있나»/.test(낸다);
  })());
...
/**
 * 🔴🔴 [2026-09-25 · 사장님] 「**결국 케이라이프맵과 에스마켓츠 담당 세션이 거짓을
 *   보고한거다. 자체ai를 학습시키고 있는 양…**」
 *
 * ⛔ 이 자가 초록 칸을 내면 그것이 보고서에 「자체 AI 학습 중」으로 실렸다.
 *   **훈련되는 모델이 없는데도 그랬다.** 그러니 맨 앞에서 그 사실을 먼저 말한다.
 *   재는 자가 정직해야 보고가 정직해진다.
 */
export function 모델이있나() {
  /* 「모델이 있다」고 말하려면 **돌아가는 끝점**이 있어야 한다.
     ⛔ 학습자료·말뭉치·학습짝은 «재료»다. 재료가 있는 것을 모델이 있는 것으로 세지 않는다. */
  const 끝점 = (process.env.SELF_AI_URL || '').trim();
  return { 있나: !!끝점, 어디: 끝점 ? '환경변수 SELF_AI_URL' : null };
}

if (직접돌리나) {
  const 자세히 = process.argv.includes('--자세히');
  console.log('■ 자체 AI 자료판독 — 사장님 2026-09-24 · 25');
  console.log('  「ai는 업무 시간에는 백에서 계속 학습하고 있어야 한다」');
  console.log('');
  const 모 = 모델이있나();
  if (모.있나) {
    console.log(`■ 훈련된 모델 — 있다 (${모.어디})`);
  } else {
    console.log('■ 훈련된 모델 — 🔴 **없다**');
    console.log('   ⛔ 아래 칸들은 「모델이 배운다」를 재는 것이 «아니다».');
    console.log('      재료를 모으고·읽고·신호로 바꾸는 «자료 판독»을 잰다.');
    console.log('   ⛔ 이 칸이 다 초록이어도 「자체 AI 를 학습시키고 있다」고 보고하지 않는다.');
    console.log('      사장님이 2026-09-25 에 그것을 «거짓 보고»라고 짚으셨다.');
  }
