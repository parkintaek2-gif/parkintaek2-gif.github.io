 ...
 */
/* 🔴 [2026-09-03 · 5번] 여기 한 줄뿐이던 「dist/wikitip/esports.html」을 뺐다. 까닭 둘이다.
 *   ① **지키던 파일이 없다.** 지면 이름이 바뀌어 지금은 esports-games.html · esports-nations.html 이다.
 *      없는 파일을 지키라고 두면 이 자는 늘 「못 쟀다」를 내고, 그러면 아무도 안 본다.
 *      ⛔ 잴 수 없는 검사는 없는 검사보다 나쁘다 — 초록도 빨강도 아닌 것이 쌓이면 자 전체가 꺼진다.
 *   ② **금지의 까닭이 끝났다.** 사장님이 Riot 을 걷어내라 하셔서 archive/raw/riot-ladder 자체가 없다.
 *      지킬 Riot API 가 없으니 「승인 전에 광고가 뜨면 취소된다」는 까닭이 성립하지 않는다.
 *      ⚠ 사장님이 함께 바로잡아 주신 것 — 「내가 riot을 제거하라고 했지, e스포츠를 제거하라고는
 *        하지 않았잖아」. e스포츠 지면은 살아 있다. 없어진 것은 «금지의 근거»뿐이다.
 *   ⚠ 지금 네 사이트의 광고 자리(ins.adsbygoogle)는 실측 0개다. 그래서 이 변경으로 오늘 나가는
 *     광고는 없다. 광고를 실제로 붙일 때 지켜야 할 지면이 생기면 그때 «까닭과 함께» 여기에 적는다.
 */
export const 광고금지 = [];
...
if (직접불렸나 && !process.argv.includes('--selftest')) {
  ...
  const 결과 = [];
  /* ⚠ 「지킬 지면이 없다」와 「지켜야 하는데 못 쟀다」는 다른 말이다.
     목록이 비어 있으면 못 잰 것이 아니라 지킬 것이 없는 것이다 — 그렇다고 적고 통과한다.
     ⛔ 둘을 같은 빨강으로 내면 다음 사람이 목록을 되살릴지 자를 끌지 알 수 없다. */
  if (!광고금지.length) {
    console.log(String.fromCharCode(10) + "⬜ 지킬 지면이 없다 — 광고금지 목록이 비어 있다(까닭은 이 파일 맨 위).");
    console.log("   광고를 붙일 때 지켜야 할 지면이 생기면 «까닭과 함께» 목록에 적는다.");
    process.exit(0);
  }
  for (const x of 광고금지) {
