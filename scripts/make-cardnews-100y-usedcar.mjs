/**
 * make-cardnews-100y-usedcar.mjs — 카드뉴스 「중고차 브랜드평판 1위, 중고차 판매업 수명은 3.54년」 5장
 *
 * 🔴 사장님 상시 지시 — 매일 카드·카드뉴스·숏영상도 낸다. 외부유입용이다.
 * ⛔ 주소 없는 카드는 안 만든다 — 다섯 장 전부에 100yearmap.com/ranking-usedcar-brand 를 박는다.
 * ⛔ 순위·등수 표현을 안 쓴다(그리기 자가 이미 지킨다). 이 카드는 «브랜드평판»과
 *   «업종 생존»이 다른 자료라는 것과, 이 시리즈에서 중고차 판매업이 어디쯯 자리하는지 대비를 낸다.
 *
 * ⛔ 화면의 수는 전부 src/data/100yearmap/ranking-*-brand-2026.json 에서 온다. 손으로 안 박는다.
 *
 * 쓰는 법  node scripts/make-cardnews-100y-usedcar.mjs [--selftest]
 */
  ...
  return [
    {
      꼴: '표지',
      큰수: `${실측.수명중앙값_년}년`,
      줄들: ['중고 자동차 판매업', '사업장 수명 중앙값입니다.'],
    },
    {
      머리: '브랜드평판 1위는 따로 있습니다',
      줄들: [
        `${중고차.순위출처.이름.replace('한국기업평판연구소 ', '')}`,
        `1위 — ${일위.브랜드}`,
        '',
        '이 순위는 화제성 지수입니다.',
        '가게 생존 기간을 재지 않습니다.',
    ...
    },
    {
      ...
      줄들: [
        `화장품전문점   ${화장품.실측대조.수명중앙값_년}년`,
        `중고차 판매업 ${실측.수명중앙값_년}년`,
        `안경원         ${안경.실측대조.수명중앙값_년}년`,
        `독립보험대리점 ${보험대리점.실측대조.수명중앙값_년}년`,
      ],
    },
    {
      꼴: '마무리',
...
if (process.argv.includes('--selftest')) {
  ...
  본다('⑤ 이 시리즈 다른 세 업종과 비교한 장이 있다',
    [안경, 화장품, 보험대리점].every((d) => 민글.includes(String(d.실측대조.수명중앙값_년) + '년')));
  본다('⑥ 🔴 모든 장에 데려갈 주소가 있다',
  ...
  const 댈수 = new Set([
    ...
    안경.실측대조.수명중앙값_년, 화장품.실측대조.수명중앙값_년, 보험대리점.실측대조.수명중앙값_년,
    2026, 9, 1, 2, 3, 4, 5,
  ...
  console.log(`\n중고차 판매업 수명중앙값 ${실측.수명중앙값_년}년 · 1위 ${일위.브랜드}`);
  process.exit();
}

/* 🔴 여기부터가 «부르면 도는 몸»이다 */
const 내가직접불렸나 = !!process.argv[1] && path.basename(process.argv[1]) === 'make-cardnews-100y-usedcar.mjs';
if (내가직접불렸나) {
  ...
  for (let i = 0; i < 장들.length; i++) {
    ...
    await sharp(Buffer.from(svg)).png().toFile(path.join(낼방, `중고차판매업수명-${i + 1}.png`));
    console.log('✅', `중고차판매업수명-${i + 1}.png`);
  }
