 ...
 */
export function 팔리나({ 설정, 정본, 주문, 단추 } = {}) {
  const 판 = 돈받는판인가(설정);
  const 값 = 값이맞나(설정, 정본);
  const 주 = 주문됐나(주문);
  const 막힌것 = [...판.막힌것, ...값.어긋난것.map((x) => '값이 어긋난다 — ' + x)];
...
async function 잰다() {
  ...
  const [로그인자리, 단추] = await Promise.all([로그인있나(), 단추그려지나()]);

  const 답 = 팔리나({ 설정, 정본, 주문, 단추 });
  const 때 = new Date();
  ...
  if (답.판정 === '팔린다') {
    console.log('   ✅ 팔린다 — 로그인 없이 바로 살 수 있다');
  } else {
...
export function 자가시험() {
  ...
  재다('오늘 실제 답은 «팔린다»', 팔리나({ 설정: 살아있는설정, 정본, 주문, 단추 }).판정 === '팔린다');
  ...
  재다('⬜ 못 쟀으면 «막혔다»로 몰지 않는다 — 못 쟀다고 따로 말한다', (() => {
    const r = 팔리나({ 설정: 살아있는설정, 정본, 주문, 단추: null });
