...
async function 잰다() {
  ...
  if (!process.argv.includes('--안적는다')) {
     ...
     *   22:26 에 이 자를 돌려 「팔린다」를 받고 마커까지 남겼는데, 손님길 자물쇠는
     *   여전히 「21:30 칸 — 아직 안 쟀다」였다. 두 곳이 서로 다른 것을 보고 있었다 —
     *     이 자  → docs/고정업무-마커/…txt   (마커)
     *     자물쇠 → docs/손님길점검.tsv        (대장)
     *   ⛔ 재도 안 풀리는 자물쇠는 자물쇠가 아니라 장식이다.
     *   ✅ 잰 그 자리에서 대장에도 적는다. */
    try {
      const { 적는다 } = await import('./손님길-자물쇠.mjs');
      적는다({ 사이트: 'seoulmarkets', 누구, 판정: 답.판정,
        말: '잰 다리: 들어오기·돈 내기·다시 보기' });
    } catch (e) {
      console.log('   🔴 대장에 못 적었다 — ' + String(e?.message ?? e).slice(0, 80));
    }

    /* 🔴🔴 [2026-09-19] **매시 소통을 세 시간 연달아 놓쳤다. 세 번 다 이 자를 돌리던 중이었다.**
       사장님: 「소통 똑바로 해라」 · 「너도 소통에 신경써」
       18:26 메모에 「결제 점검을 돌릴 때 소통 줄도 함께 남기겠습니다」라고 적어 놓고
       19시·20시에 또 안 남겼다. ⛔ **다짐은 자물쇠가 아니다.**
       ⇒ 잰 그 자리에서 매시 소통 대장에도 적는다. 손님길 대장에 적는 것과 같은 까닭이다.
       ⚠ 지어내지 않는다 — 방금 «실제로 잰 결과»를 그대로 옮긴다. 안 쟀으면 안 적힌다.
       ⚠ 이미 이 시간 칸에 적었으면 덧적지 않는다(같은 시간에 두 줄이 쌓이지 않게). */
    try {
      const 소통 = await import('./매시소통-자물쇠.mjs');
      const 이제 = new Date();
      if (소통.걸리는자리.includes(누구) && !소통.이번시간에적었나(소통.모두읽기(), 누구, 이제)) {
        소통.적는다({ 누구, 이제,
          말: `서울마켓츠 결제 점검 — ${답.판정}. `
            + `주문 ${답.주문번호 || '만들어 봄'} · 승인(capture)은 안 불렀다. `
            + `다음: 이어서 맡은 일.` });
        console.log('   ✅ 매시 소통 대장에도 적었다 — 재면서 소통을 빠뜨리지 않게');
      }
    } catch (e) {
      console.log('   ⬜ 소통 대장에 못 적었다 — ' + String(e?.message ?? e).slice(0, 80));
    }
  }
