#!/usr/bin/env node
/**
 * measure-100y-ctr-gap.mjs — **1페이지에 있으면서 클릭이 0인 지면**을 뽑는다.
 *
 * ── 왜 (2026-08-25) ────────────────────────────────────────────
 * 오늘 하루 이 일을 $CLAUDE_JOB_DIR/tmp 에 임시 스크립트로 네 번 다시 짰다.
 * 5번이 「자 668개, 서로 같은 것을 다시 만들고 있다」고 지적한 뒤 자기 자
 * (measure-kcw-ctr-gap.mjs)의 뼈대를 그대로 옮겨 100yearmap용으로 굳힌다.
 * ⛔ 5번 파일을 고치지 않는다 — 사이트만 다른 같은 결의 자다.
 *
 ...
 *
 * 쓰는 법  node scripts/measure-100y-ctr-gap.mjs --자가시험
...
const 낼길 = path.join(뿌리, 'src/data/100yearmap-ctr-gap.json');

/** Search Console 보고 한 줄을 뜯는다 — search-console-report.mjs --축=page 출력 꼴 */
export function 줄뜯기(줄) {
  ...
  return {
    ...
    클릭: Number(m[2]),
    순위: Number(m[3]),
    주소: m[4].replace('https://100yearmap.com', '') || '/',
  };
}

/** 지면 갈래 — 어느 틀이 아픈지 보려면 갈래가 필요하다 */
export function 갈래(주소) {
  ...
  if (p.startsWith('/before') || p.startsWith('/after')) return 'life-hub';
  return 'topic';
}
...
if (process.argv.includes('--자가시험')) {
  ...
  const 한줄 = 줄뜯기('   노출    175 · 클릭    0 · 순위 7.9   https://100yearmap.com/report/area/서울특별시-마포구');
  검('한 줄을 뜯는다', 한줄 && 한줄.노출 === 175 && 한줄.클릭 === 0 && 한줄.순위 === 7.9);
  검('집 주소를 뗀다', 한줄.주소 === '/report/area/서울특별시-마포구');
  검('첫 화면은 «/»', 줄뜯기('노출 21 · 클릭 2 · 순위 8.9 https://100yearmap.com/').주소 === '/');
  검('⛔ 아닌 줄은 null', 줄뜯기('아무 글') === null);

  검('갈래를 안다', 갈래('/school/1') === 'school' && 갈래('/university/1') === 'university');
  검('report-area 갈래', 갈래('/report/area/x') === 'report-area');
  검('college-major 갈래', 갈래('/college-major') === 'college-major' && 갈래('/college-major/건축학과') === 'college-major');
  검('첫 화면 갈래', 갈래('/') === 'home');
  검('생애단계 지면은 topic', 갈래('/hiking') === 'topic' && 갈래('/parental-leave') === 'topic');

  const 줄들 = [
    { 노출: 175, 클릭: 0, 순위: 7.9, 주소: '/report/area/x' },
    { 노출: 67, 클릭: 0, 순위: 7.9, 주소: '/college-major' },
  ...
  검('노출 큰 것이 먼저', 아픈[0].주소 === '/report/area/x');
...
const 글 = execFileSync('node', [path.join(뿌리, 'scripts/search-console-report.mjs'),
  'sc-domain:100yearmap.com', '--축=page', '--행수=1000'], { encoding: 'utf8', cwd: 뿌리, maxBuffer: 1e8 });
...
console.log('\n## 갈래별로 어디가 아픈가');
for (const [k, v] of 갈래별(아픈)) console.log(`  ${k.padEnd(14)} ${String(v.장).padStart(3)}장 · 노출 ${v.노출}`);
