...
// ③ 문 갈래 수 — 🔴 [2026-09-09 · 3번] 옛 정규식이 배열 안 주석의 「[2026-09-05 · 5번]」
//    같은 날짜꼬리표 대괄호에서 멈춰 106개를 0으로 잘못 셌다. `] as const` 까지 가게 고쳤다.
//    ⚠ 지면이 늘 때마다 갈래도 자연히 는다 — 특정 수와 정확히 맞는지가 아니라
//      «0이 아닌지·역사상 가장 적었던 22 밑으로 줄지 않았는지»만 본다(문서에도 그렇게 적음).
{
  const 갈래수 = (문코드.match(/붙일수있는갈래[\s\S]*?=\s*\[([\s\S]*?)\]\s*as\s*const/)?.[1].match(/'/g)?.length ?? 0) / 2;
  잰것.push('③ 문 갈래 — 코드 ' + 갈래수 + ' (0이 아니고 22 이상이어야 한다)' +
    (갈래수 >= 22 ? '  ✅' : '  🔴'));
  if (갈래수 < 22) 흠.push('문 갈래가 ' + 갈래수 + ' 다 — 22 밑이면 갈래를 못 읽었거나 실제로 줄어든 것이다');
}

// ④ 지금 상태 — 🔴 [2026-09-09 · 3번] PG는 2026-08-31에 정식으로 켜졌다(사장님 지시·2번 확인,
//    커밋 c25d6d89c). 「반드시 꺼져 있어야 한다」는 8/15 전용 가정이라 지금은 안 맞는다.
//    차례표의 「지금상태: PG붙었나 = …」 줄을 읽어 코드와 «맞는지»만 본다 — 방향을 안 정한다.
{
  const 값 = 내보낸값(값코드, 'PG붙었나');
  const 문서상태 = /지금상태:\s*PG붙었나\s*=\s*(true|false)/.exec(차례표)?.[1] ?? null;
  잰것.push('④ 지금 상태 — 코드 ' + 값 + ' · 차례표 「지금상태」 ' + (문서상태 ?? '(없음)') +
    (문서상태 === null ? '  🔴' : 문서상태 === 값 ? '  ✅' : '  🔴'));
  if (문서상태 === null) 흠.push('차례표에 「지금상태: PG붙었나 = …」 줄이 없다 — 지금 상태를 못 견준다');
  else if (문서상태 !== 값) 흠.push('차례표는 지금 PG붙었나 = ' + 문서상태 + ' 라는데 코드는 ' + 값 + ' 다');
}
