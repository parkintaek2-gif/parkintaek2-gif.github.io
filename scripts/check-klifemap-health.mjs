...
/**
 * 🔴 [2026-09-16] **한 번 다시 뜨는 것과 «되풀이해» 다시 뜨는 것은 다른 사고다.**
 *
 * 사장님: 「케이라이프맵 다운 빨리 조치해」 — 가 보니 13:18 과 13:22, 4분 사이에 두 번
 * 다시 떠 있었다. 까닭은 0.5GB 칸에서 PDF 용 크롬을 띄운 것이었다(메모리를 넘겨 죽었다).
 *
 * ⛔ 그때 이 자는 「⚠ 방금 다시 떴다」 한 줄만 냈다. 그 줄은 «배포 직후»에도 똑같이 뜨므로
 *   아무도 사고로 읽지 않는다. **되풀이를 못 세면 고리를 못 본다.**
 * ⇒ 잰 것을 남겨 두고, 창 안에 «서로 다른 부팅»이 몇 번인지 센다.
 *
 * @param 자취들 [{ 잰때(ms), uptimeSec }] — 오래된 것부터
 */
export function 뜬때(잰때, uptimeSec) {
  if (!Number.isFinite(잰때) || !Number.isFinite(uptimeSec) || uptimeSec < 0) return null;
  return 잰때 - uptimeSec * 1000;
}

export function 되풀이해떴나(자취들, { 창 = 3600_000, 문턱 = 3, 같은것으로볼차 = 60_000, 지금 = Date.now() } = {}) {
  const 때들 = (자취들 ?? [])
    .map((x) => 뜬때(x?.잰때, x?.uptimeSec))
    .filter((t) => Number.isFinite(t) && 지금 - t <= 창)
    .sort((a, b) => a - b);
  /* ⚠ 같은 부팅을 여러 번 잰 것은 «한 번»으로 센다 — 안 그러면 점검 주기가 곧 사고 수가 된다 */
  const 다른부팅 = [];
  for (const t of 때들) {
    if (!다른부팅.length || t - 다른부팅[다른부팅.length - 1] > 같은것으로볼차) 다른부팅.push(t);
  }
  return { 횟수: 다른부팅.length, 고리다: 다른부팅.length >= 문턱, 뜬때들: 다른부팅 };
}
...
function 자가시험() {
  ...
  /* ── 🔴 [2026-09-16] 되풀이해 다시 뜨는 것 — 실제로 난 사고를 시험으로 굳힌다 ── */
  const 기준 = Date.parse('2026-09-16T13:30:00+09:00');
  검('못 잰 것은 null 이다', 뜬때(NaN, 10), null);
  검('뜬 때 = 잰 때 − 떠 있은 시간', 뜬때(기준, 60), 기준 - 60_000);

  /* 그날 실제 모양 — 13:18·13:22 두 번 다시 떴고, 그 사이에도 우리가 여러 번 쟀다 */
  const 그날자취 = [
    { 잰때: Date.parse('2026-09-16T13:19:40+09:00'), uptimeSec: 67 },   /* 13:18:33 부팅 */
    { 잰때: Date.parse('2026-09-16T13:20:40+09:00'), uptimeSec: 127 },  /* 같은 부팅 — 한 번으로 센다 */
    { 잰때: Date.parse('2026-09-16T13:28:51+09:00'), uptimeSec: 378 },  /* 13:22:33 부팅 */
  ];
  검('같은 부팅을 두 번 세지 않는다', 되풀이해떴나(그날자취, { 지금: 기준 + 600_000 }).횟수, 2);
  검('두 번이면 아직 고리라 하지 않는다(문턱 3)', 되풀이해떴나(그날자취, { 지금: 기준 + 600_000 }).고리다, false);
  검('세 번째가 붙으면 고리다',
    되풀이해떴나([...그날자취, { 잰때: Date.parse('2026-09-16T13:35:00+09:00'), uptimeSec: 30 }],
      { 지금: 기준 + 600_000 }).고리다, true);
  검('창 밖의 옛것은 안 센다',
    되풀이해떴나([{ 잰때: 기준 - 7200_000, uptimeSec: 10 }], { 지금: 기준 }).횟수, 0);
  검('자취가 없으면 0 이다', 되풀이해떴나([]).횟수, 0);
  검('찌그러진 자취는 버린다', 되풀이해떴나([{ 잰때: 'x', uptimeSec: 5 }, null], { 지금: 기준 }).횟수, 0);
...
/* 🔴 잰 것을 «남긴다». 남기지 않으면 되풀이를 못 세고, 되풀이를 못 세면 고리를 못 본다 */
const 자취칸 = new URL('../archive/klifemap-uptime.jsonl', import.meta.url);
let 고리 = { 횟수: 0, 고리다: false };
try {
  const { appendFileSync, readFileSync, existsSync, mkdirSync } = await import('node:fs');
  mkdirSync(new URL('../archive/', import.meta.url), { recursive: true });
  const 옛것 = existsSync(자취칸)
    ? readFileSync(자취칸, 'utf8').trim().split('\n').slice(-500).map((s) => { try { return JSON.parse(s); } catch { return null; } })
    : [];
  const 이번 = { 잰때: Date.now(), uptimeSec: 몸통.uptimeSec };
  appendFileSync(자취칸, JSON.stringify(이번) + '\n', 'utf8');
  고리 = 되풀이해떴나([...옛것, 이번]);
} catch { /* 자취를 못 남겨도 점검 자체는 계속한다 */ }

if (고리.고리다) {
  console.log(`   🔴🔴 «되풀이해» 다시 뜨고 있다 — 한 시간에 ${고리.횟수}번`);
  console.log('      뜬 때 — ' + 고리.뜬때들.map((t) => new Date(t).toLocaleTimeString('ko-KR')).join(' · '));
  console.log('      ⚠ 첫 의심은 «메모리»다. 이 앱은 PDF 를 만들 때 크롬을 띄운다 —');
  console.log('        2026-09-16 에 0.5GB 칸에서 크롬을 띄우다 죽었다. 콘솔에서 1GB 로 올려 멈췄다.');
  console.log('      ⛔ 고칠 때 ctype apply 를 쓰지 않는다 — 환경변수가 지워진다. 대시보드로 한다.');
} else if (고리.횟수 >= 2) {
  console.log(`   ⚠ 한 시간에 ${고리.횟수}번 다시 떴다 — 한 번 더면 고리로 본다`);
}
