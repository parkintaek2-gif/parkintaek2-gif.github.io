...
if (내가직접불렸나) {
  ...
  for (const r of 소아) {
    ...
    else if (w.갈래 === '시군구') 동네.push({
      /* 🔴 코드를 남긴다 — 다른 표와 이을 때 **이름으로 맞추면 중구가 여섯이라 틀린다** */
      코드: String(r.C1).split('.').pop(),
      시도: 시도이름.get(w.시도코드) ?? '모름', 이름: r.C1_NM, 곳,
    });
    else if (w.갈래 === '모름') 모름++;
  }

  /**
   * 🔴 2번 지시(8/21 05:2x) — 「의원만」을 닫아라.
   *   ⛔ 그런데 **시군구 × 종별 × 과목의 «기관 수» 표가 KOSIS 에 없다.** 찾아본 것 —
   * ```
   *   DT_117049_A015  시군구 × 유형 × 과목 — 있는데 **전문의 «사람» 수**이고 **2015년 한 해**뿐
   *   DT_HIRA4K       종별 × 과목 전문의 — **시·군·구가 없다**
   * ```
   *   ⇒ 「병원 안에 소아청소년과가 있나」는 못 센다. 사람 수와 기관 수를 섞으면 그건 지어내는 것이다.
   *
   * ⭐ 대신 **같은 단위로 확실히 셀 수 있는 것**을 센다 — 그 동네에 병원급이 아예 있나.
   *   같은 표 계열(HIRA4B) · 같은 분기 · 같은 «기관 수» · 같은 코드다.
   *   · 병원급이 한 곳도 없으면 → 병원 안 소아과도 **있을 수 없다**(확실하다)
   *   · 병원급이 있으면 → 그 안에 소아과가 있는지는 **모른다**. 「없다」고 쓰지 않는다
   */
  const 종별날 = await (await fetch(
    `https://kosis.kr/openapi/Param/statisticsParameterData.do?method=getList&apiKey=${KEY}`
    + `&itmId=ALL&objL1=ALL&objL2=ALL&format=json&jsonVD=Y`
    + `&orgId=${ORG}&tblId=DT_HIRA4B&prdSe=${기간꼴}&newEstPrdCnt=1`)).json();
  const 종별때 = Array.isArray(종별날) ? 종별날[0].PRD_DE : null;
  const 병원급 = ['상급종합병원', '종합병원', '병원'];
  const 병원수 = new Map();
  if (Array.isArray(종별날)) {
    for (const r of 종별날) {
      if (무엇인가(r.C1).갈래 !== '시군구') continue;
      if (!병원급.includes(r.C2_NM)) continue;
      const k = String(r.C1).split('.').pop();
      병원수.set(k, (병원수.get(k) ?? 0) + (수로(r.DT) ?? 0));
    }
  }
  ...
  const 없는곳 = 동네.filter((r) => r.곳 === 0).map((r) => ({ ...r, 병원급: 병원수.has(r.코드) ? 병원수.get(r.코드) : null })).sort((a, b) => a.시도.localeCompare(b.시도) || a.이름.localeCompare(b.이름));
  const 없는시도 = {};
  ...
  const 낸다 = {
    ...
    병원급: {
      때: 종별때, 표: 'DT_HIRA4B', 센것: 병원급.join(' · '),
      맞춘법: '이름이 아니라 시·군·구 코드로 이었습니다 — 중구가 여섯이라 이름으로는 못 맞춥니다.',
      '⛔ 못 쟀다': '병원 «안에» 소아청소년과가 있는지는 못 쟀습니다. 시군구 × 종별 × 과목의 기관 수 표가 KOSIS 에 없습니다(DT_117049_A015 는 2015년 전문의 사람 수, DT_HIRA4K 는 시·군·구가 없습니다). 사람 수와 기관 수를 섞으면 지어내는 것이 됩니다.',
      아예없는곳: 없는곳.filter((r) => r.병원급 === 0).length,
      못맞춘곳: 없는곳.filter((r) => r.병원급 === null).length,
    },
    없는곳, 없는시도, 시도별,
