...
export const 유형 = [
  ...
  /*
   * 🔴 [2026-09-14 지침] ceo-change·control-change — CLAUDE.md 「주력과 서비스」:
   * 「회사 주인이 바뀌거나 CEO/회장이 바뀌는 경우에나 사람이 중요한 거 아닌가」.
   * 실측(2026-09-14, 실제 CNINFO 제목 표본 검색)으로 넣는다 — 한국(collect-dart-breaking.mjs)·
   * ADX·DFM 에는 이미 있고 상하이만 빠져 있었다. 새 나라를 열 때 이 두 태그를 반드시 넣는다.
   * ⚠ 副总经理(부사장급) 변경은 총경리 변경이 아니다 — 부(副) 앞글자가 붙으면 제외한다
   *   (실측: "副总经理辞职"류가 "总经理"류보다 훨씬 흔하다 — 안 가르면 잡음이 대부분이 된다).
   */
  { re: /(?!.*变更名称)(?:实际控制人.*(变更|拟变更)|控制权.*(变更|拟变更|发生变更)|控股股东.*(变更|发生变更))/, 무게: 8, 태그: 'control-change' },
  { re: /(?<!副)总经理.*?(变更|变动|离任|辞职|聘任|任命)|(变更|变动|离任|辞职|聘任|任命|选举|换届|退休).*?(?<!副)总经理|董事长.*(辞职|离任|变更|选举|任命|换届|退休)|(辞职|离任|变更|选举|任命|换届|退休).*董事长/, 무게: 8, 태그: 'ceo-change' },
...
function 자가시험() {
  ...
  /* 🔴 [2026-09-14 · 5번 지침] ceo-change·control-change — 실제 CNINFO 제목 표본으로 검증 */
  재다('실측: 董事长辞职 → ceo-change 무게8', 유형찾기('鲁银投资关于董事长辞职的公告')?.태그 === 'ceo-change');
  재다('실측: 董事长变更 → ceo-change', 유형찾기('关于董事长变更及调整部分董事会专门委员会委员的公告')?.태그 === 'ceo-change');
  재다('실측: 选举董事长 → ceo-change', 유형찾기('关于补选非独立董事、选举董事长并调整董事会战略委员会委员的公告')?.태그 === 'ceo-change');
  재다('실측: 董事长退休离任暨选举董事长 → ceo-change', 유형찾기('关于公司董事长退休离任暨选举董事长的公告')?.태그 === 'ceo-change');
  재다('실측: 董事长任命公告 → ceo-change', 유형찾기('董事长任命公告')?.태그 === 'ceo-change');
  재다('실측: 总经理（CEO）离任暨聘任总经理（CEO） → ceo-change', 유형찾기('安迪苏关于公司总经理（CEO）离任暨聘任总经理（CEO）的公告')?.태그 === 'ceo-change');
  재다('실측: 总经理变动 → ceo-change', 유형찾기('海钦股份关于公司总经理变动的公告')?.태그 === 'ceo-change');
  재다('실측: 变更总经理 → ceo-change', 유형찾기('关于变更总经理的公告')?.태그 === 'ceo-change');
  재다('⛔ 오탐 방지: 董事长专题会议事规则(절차규정)은 ceo-change 가 아니다', 유형찾기('董事长专题会议事规则') === null || 유형찾기('董事长专题会议事规则')?.태그 !== 'ceo-change');
  재다('⛔ 오탐 방지: 副总经理 선임(부사장급)은 ceo-change 가 아니다', 유형찾기('关于聘任公司副总经理的公告')?.태그 !== 'ceo-change');
  재다('⛔ 오탐 방지: 副总经理 사직도 ceo-change 가 아니다', 유형찾기('关于公司副总经理辞职的公告')?.태그 !== 'ceo-change');
  재다('⛔ 오탐 방지: 董事·副总经理 동시 변경도 ceo-change 가 아니다(부사장급 위주)', 유형찾기('鲁银投资关于董事、副总经理辞职暨补选董事、聘任副总经理的公告')?.태그 !== 'ceo-change');

  재다('실측: 控股股东、实际控制人拟发生变更 → control-change 무게8', 유형찾기('关于股东签署《股份转让协议》及公司签署《附条件生效的股份认购协议》暨公司控股股东、实际控制人拟发生变更的提示性公告')?.태그 === 'control-change');
  재다('실측: 实际控制人发生变更 → control-change', 유형찾기('关于实际控制人发生变更暨股东权益变动的进展公告')?.태그 === 'control-change');
  재다('실측: 控制权拟变更事项终止 → control-change', 유형찾기('关于控股股东、实际控制人解除《股份转让协议》《表决权放弃协议》暨控制权拟变更事项终止的公告')?.태그 === 'control-change');
  재다('실측: 控股股东…实际控制人发生变更(과户등기) → control-change', 유형찾기('关于控股股东协议转让股份完成过户登记暨控股股东、实际控制人发生变更的公告')?.태그 === 'control-change');
  재다('실측: 实际控制人工商变更 → control-change', 유형찾기('南京证券股份有限公司关于实际控制人工商变更及其权益变动进展的公告')?.태그 === 'control-change');
  재다('⛔ 오탐 방지: 지배주주 «이름만» 바뀜(变更名称)은 control-change 가 아니다', 유형찾기('天马科技关于控股股东、实际控制人之一致行动人、持股5%以上股东变更名称及其他工商信息的公告')?.태그 !== 'control-change');
  재다('⛔ 오탐 방지: 일반 대주주 지분변동(5% 이상)은 control-change 가 아니라 ownership-change', 유형찾기('关于第一大股东权益变动的提示性公告')?.태그 === 'ownership-change');
  ...
  /* 🔴 [2026-09-14 정정] 总经理(대표) 사직은 이제 officer-change 가 아니라 ceo-change 다 —
   * 위 「实측: 总经理変动 → ceo-change」류와 같은 이유(사람 축 중 유일하게 값나가는 경우). */
  재다('总经理辞职 → ceo-change(정정)', 유형찾기('关于总经理辞职的公告')?.태그 === 'ceo-change');
  재다('董事(일반 이사) 사직은 여전히 officer-change', 유형찾기('关于董事辞职的公告')?.태그 === 'officer-change');
