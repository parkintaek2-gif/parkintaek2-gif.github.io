/**
 * collect-100y-hiking.mjs — 나이대별 등산 참여율(최근 1년간 참여 경험)
 *
 * ── 왜 ─────────────────────────────────────────────────────────
 * 자동완성 실측(2026-08-25) — 「등산 인구」 4줄(등산 인구·등산 인구 통계·등산 인구
 * 증가·한국 등산 인구). 신호가 확인된 뒤에야 표를 찾았다(짐작으로 먼저 안 만든다).
 *
 * ── 🔴 이 표를 처음 열었을 때 실수할 뻔한 자리 ──────────────────
 * `objL1=ALL`로 통째로 받으면 «성별×나이 교차»(예: 005001004 = 남자+40대) 줄과
 * «나이만»(004004 = 40대) 줄이 **같은 이름("40대")으로 섞여 나온다.** C1 코드를
 * 열어 보지 않고 이름만 보고 골랐으면 두 값이 뒤섞여 틀린 수를 냈을 것이다.
 * ```
 * 131021315501.001            전체
 * 131021315501.004NNN         나이만(원하는 것)         ← 6자리
 * 131021315501.005001NNN      남자 × 나이(교차)         ← 9자리, 뺀다
 * 131021315501.005002NNN      여자 × 나이(교차)         ← 9자리, 뺀다
 * ```
 * ⇒ 정규식으로 **나이만 있는 6자리 코드**(`004\d{3}$`, 접두사 앞에 다른 숫자 없이)만 쓴다.
 *
...
const 내가직접불렸나 = !!process.argv[1] && path.basename(process.argv[1]) === 'collect-100y-hiking.mjs';
if (내가직접불렸나) {
  ...
  const 등산줄 = 날.filter((x) => x.ITM_NM === 종목 && 나이만인가(x.C1));
  const 해들 = [...new Set(등산줄.map((x) => x.PRD_DE))].sort();
  const 최신 = 해들[해들.length - 1];

  const 해별 = {};
  for (const 해 of 해들) {
    const 이해줄 = 등산줄.filter((x) => x.PRD_DE === 해);
  ...
  const 낸다 = {
    ...
    '⚠ 이 자료가 못 가르는 것': [
      ...
      '「참여 경험」이라 강도·빈도(한 번 갔다 온 사람과 매주 가는 사람을 못 가릅니다)를 못 잽니다.',
      '성별×나이를 교차한 값은 이 표에도 있지만(예: 남자 40대만) 이 지면은 나이만 봅니다 — 성별로 가르지 않았습니다.',
    ],
    해들, 최신, 해별,
  };
  const 낼곳 = path.join(뿌리, 'src/data/100yearmap/hiking.json');
