...
/** 사장님이 정하신 이슈 반응 창. 2026-09-05 에 12시간에서 줄었다 */
export const 이슈창시간 = 6;

/**
 * 씨앗이 «몇 시간 전» 것인가 — **「언제」로 잰다.**
 *
 * 🔴 [2026-09-07] 「처음본때」로 재면 안 된다. 그건 «우리가 처음 본 때»이고,
 *   47일 전 기사를 오늘 처음 봤으면 「오늘」로 보인다. 내가 그렇게 읽고
 *   1,131시간 지난 것을 「정면으로 붙는 이슈」라고 메모에 올렸다.
 * ⛔ 「언제」가 없으면 `null` 이다 — 0 도, 오늘도 아니다.
 */
export function 몇시간전(항목, 이제 = Date.now()) {
  const 언제 = 항목?.언제;
  if (!언제) return null;
  const t = new Date(언제).getTime();
  if (!Number.isFinite(t)) return null;
  return Math.round((이제 - t) / 3600e3 * 10) / 10;
}

/** 씨앗 줄에 붙일 나이 딱지. ⏰ 는 «이슈 창에 든 것»만 */
export function 나이딱지(항목, 이제 = Date.now(), 창 = 이슈창시간) {
  const h = 몇시간전(항목, 이제);
  if (h === null) return ' · 나이 모름';
  if (h < 0) return ` · ${Math.abs(h)}시간 뒤(앞날 표기)`;
  if (h <= 창) return ` · ⏰ ${h}시간 전 — 이슈 창(${창}시간) 안`;
...
export const 우물들 = [
  ...
  /* 🔴 [2026-09-07] 3번 몫(교육·진로)이 하루 «4건»뿐이었고 그중 3건이 같은 글이었다.
     3번이 안 한 것이 아니라 «우물이 3번 쪽을 안 긷고 있었다».
     커뮤니티 셋은 게임·유머 방이고, 영문 뉴스 셋은 K팝·드라마·경제다 — 교육이 들어올 문이 없었다.
     ⚠ 막힌 에펨코리아(430)가 생활·교육 글이 가장 많던 곳이라 더 비었다.
     ⇒ 한국어 구글뉴스 둘을 «따로» 낸다. 3번 지면(백년지도)이 0~100세를 다루므로
        입시 한 축, 취업·진로 한 축으로 가른다.
     ⭐ 실측(2026-09-07): 둘 다 200 · 100건씩 온다.
     ⛔ 여기서 온 글도 «유닛가르기»가 낱말로 다시 가른다 — 우물이 3번 것이라고 3번에 몰아주지 않는다
        (「수능만점」이 제목에 든 증시 기사가 실제로 섞여 왔다). */
  { 곳: '구글뉴스 입시·대학(한국어)', 갈래: '뉴스', 꼴: 'gnews',
    주소: 'https://news.google.com/rss/search?q=%EC%88%98%EB%8A%A5+OR+%EC%9E%85%EC%8B%9C+OR+%EB%8C%80%ED%95%99%EB%93%B1%EB%A1%9D%EA%B8%88+OR+%ED%95%99%EA%B3%BC&hl=ko&gl=KR&ceid=KR:ko', 바탕: '' },
  { 곳: '구글뉴스 취업·진로(한국어)', 갈래: '뉴스', 꼴: 'gnews',
    주소: 'https://news.google.com/rss/search?q=%EC%B7%A8%EC%97%85%EB%A5%A0+OR+%EC%B1%84%EC%9A%A9+OR+%EC%B2%AD%EB%85%84%EC%8B%A4%EC%97%85+OR+%EC%9D%B4%EC%A7%81&hl=ko&gl=KR&ceid=KR:ko', 바탕: '' },
];
...
export const 유닛낱말 = {
  '5번': ['케이팝', '아이돌', '걸그룹', '보이그룹', '드라마', '배우', '가수', '영화', '예능',
    ...
    /* 🔴 [2026-09-08 · 5번] 트렌드에서 「아이유」(2,000+)·「박지윤」(1,000+)이 갈래없음으로 샜다.
       까닭 칸은 제대로 왔는데(「이종석과 결별…단발 비주얼」·「8kg 감량…화보」) 목록에 그 낱말이 없었다.
       ⭐ 오늘 자료 757건에 대 보고 «잡는 것이 있고 남의 것을 안 빼앗는 것»만 넣었다.
       ⛔ 「스타」는 거부했다 — 「인스«타»」·「바리«스타»」에 박혀 다섯을 잘못 잡고 3번 것을 하나 빼앗았다.
         이 파일이 이미 경고한 「공모 → 항공모함」과 같은 일이다. 짧은 낱말은 글자 안에 박힌다.
       ⛔ 「열애·방송·티저·팬미팅·연기·캐스팅·뮤직비디오」는 오늘 «한 건도 안 걸려» 재지 못했다.
         그래서 안 넣었다 — 못 잰 낱말을 목록에 넣는 것은 짐작이다. */
    '화보', '결별', '비주얼'],
  '6번': ['주식', '증시', '코스피', '코스닥', '환율', '금리', '부동산', '집값', '연봉', '물가',
    ...
    /* 🔴 [2026-09-08 · 5번] 「수출」이 영어(exports)로만 있어 «한국어» 기사가 샜다 —
       트렌드 「화장품」(1,000+)의 까닭이 「'수출 2위' K뷰티…」였는데 안 걸렸다.
       「적금」(2,000+)도 목록에 없었다. 둘 다 오늘 자료에서 빼앗음 0 을 확인하고 넣었다. */
    '수출', '적금'],
  '3번': ['대학', '수능', '입시', '취업', '학교', '학원', '전공', '졸업', '연금', '출산',
    ...
    /* 🔴 [2026-09-08 · 5번] 오늘 자료에 대 보고 «둘만» 넣었다.
       ⛔ 거부한 것 셋 — 남의 것을 빼앗는다. 교육 낱말처럼 보이지만 금융 기사에 더 많이 쓰인다:
         「채용」   6번 것 32건을 빼앗는다 (은행·삼성 채용 기사)
         「정규직」 6번 것 15건을 빼앗는다 (기업은행 수시채용)
         「학과」   6번 것 7건을 빼앗는다 (반도체 계약학과 합격선 — 산업 기사다)
       ⭐ 이것이 이 목록의 어려움이다. 낱말이 «어느 갈래의 말인가»가 아니라
         «어느 갈래 기사에 더 많이 나오나»로 정해야 한다. 그래서 오늘 자료에 대 본다. */
    '모평', '월급'],
};
...
function 자가시험() {
  ...
  본다('우물이 열세 곳이다', 우물들.length === 13);
  본다('🔴 3번 몫(교육·진로)을 긷는 우물이 있다 — 없으면 3번 칸이 늘 빈다',
    우물들.filter((u) => /입시|취업/.test(u.곳)).length === 2);
  본다('그 둘은 한국어판을 부른다', 우물들.filter((u) => /입시|취업/.test(u.곳))
    .every((u) => /hl=ko/.test(u.주소) && /ceid=KR:ko/.test(u.주소)));
  본다('⛔ 우물이 3번 것이라고 3번에 몰아주지 않는다 — 낱말로 다시 가른다',
    유닛가르기('사상 첫 수능만점 일 냈다…코스피 6,900 회복').유닛 === '6번');
  본다('교육 낱말이 들면 3번으로 간다',
    유닛가르기('80% 선발 수시 원서접수 시작…막차 입시 변수').유닛 === '3번');
  ...
  /**
   * 🔴 [2026-09-07] **내가 47일 전 것을 「오늘 난 이슈」로 읽고 메모에 올렸다.**
   *   「언제」는 2026-07-22 인데 「처음본때」가 오늘 08:00 이었고, 나는 뒤엣것을 봤다.
   *   ⇒ 사장님이 정하신 「이슈에 반응 6시간」 창을 1,131시간 지난 것에 대고 잰 것이다.
   * ⛔ 그래서 자가 나이를 «찍는다». 아래 시험이 그것을 지킨다.
   */
  본다('나이를 「언제」로 잰다', 몇시간전({ 언제: 'Wed, 22 Jul 2026 07:00:00 GMT' }, Date.parse('2026-09-07T19:00:00+09:00')) > 1000);
  본다('⛔ 「처음본때」를 나이로 쓰지 않는다 — 그 칸만 있으면 «모름»이다',
    몇시간전({ 처음본때: '2026. 9. 7. 오전 8:00:01' }, Date.parse('2026-09-07T19:00:00+09:00')) === null);
  본다('⛔ 「언제」가 없으면 0 이 아니라 null 이다',
    몇시간전({}, Date.parse('2026-09-07T19:00:00+09:00')) === null && 몇시간전(null, Date.parse('2026-09-07T19:00:00+09:00')) === null);
  본다('⛔ 읽을 수 없는 날짜도 null 이다', 몇시간전({ 언제: '아무말' }, Date.parse('2026-09-07T19:00:00+09:00')) === null);
  본다('여섯 시간 안이면 ⏰ 를 붙인다', 나이딱지({ 언제: '2026-09-07T15:00:00+09:00' }, Date.parse('2026-09-07T19:00:00+09:00')).includes('⏰'));
  본다('⛔ 여섯 시간을 넘으면 ⏰ 를 안 붙인다', !나이딱지({ 언제: '2026-09-07T05:00:00+09:00' }, Date.parse('2026-09-07T19:00:00+09:00')).includes('⏰'));
  본다('경계(정확히 6시간)는 창 «안»이다', 나이딱지({ 언제: '2026-09-07T13:00:00+09:00' }, Date.parse('2026-09-07T19:00:00+09:00')).includes('⏰'));
  본다('이틀을 넘으면 날로 적는다', /47일 전/.test(나이딱지({ 언제: 'Wed, 22 Jul 2026 07:00:00 GMT' }, Date.parse('2026-09-07T19:00:00+09:00'))));
  본다('나이를 모르면 «모름»이라 적는다 — 오늘로 적지 않는다',
    /모름/.test(나이딱지({}, Date.parse('2026-09-07T19:00:00+09:00'))));
  본다('창 시간은 사장님이 정하신 6시간이다', 이슈창시간 === 6);
...
async function main() {
  ...
  for (const 유닛 of 순서) {
    ...
    for (const d of 앞선) {
      ...
      /* 🔴 [2026-09-07] 나이를 «찍어» 준다. 안 찍으면 세션이 「처음본때」를 나이로 읽는다 */
      console.log(`  · [${d.곳}] ${d.제목}${수표}  (댓글 ${d.댓글}${나이딱지(d)})`);
    }
    console.log('');
  }
