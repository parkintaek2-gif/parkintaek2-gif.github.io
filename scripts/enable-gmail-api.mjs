...
if (내가실행됐다) {
  ...
  if (본것.ok) {
    ...
    /* 🔴 [2026-08-30] 여기서 «조용히 넘어갔다.» 읽기가 막혔는데 아무 말도 안 하고
       「보기만 했다」로 끝나서, 켜졌는지 안 켜졌는지 모르는 채로 지나갔다.
       ⛔ 못 잰 것은 **못 쟀다고 적는다.** 침묵은 「괜찮다」로 읽힌다. */
    const 진단 = 무엇이막혔나(본글);
    console.log(`  지금 상태     : ⚠ **못 쟀다** (HTTP ${본것.status} · ${진단.무엇})`);
    if (진단.할것) console.log(`                  ${진단.할것}`);
    console.log('  ⚠ 읽는 것조차 막혔으니 «켜졌는지 아닌지»를 여기서는 모른다.');
    console.log('  ⭐ 켜졌는지 확실히 보는 길 — node scripts/send-mail.mjs --시험 뒤 실제로 한 통 보내 본다.');
  }
