 ...
 *
 * ── 🔴🔴 [2026-09-09 04:4x · 5번] **두 축의 합계가 다르다. 섞어 쓰면 수가 틀린다** ──
 *
 * 같은 사이트를 두 축으로 받아 합계를 대 봤다 (kculturewire · 28일).
 * ```
 *   --축=page   (지면축)        행 890 · 노출 5,124 · 클릭 34 · 0.66%
 *   기본        (검색어×지면축)  행 488 · 노출 1,130 · 클릭  3 · 0.27%
 * ```
 * ⇒ **검색어축이 노출의 78%, 클릭의 91% 를 안 보여 준다.**
 *   구글이 «드문 검색어»를 개인정보로 감추기 때문이다(공개된 동작이다).
 *
 * ⛔ 그래서 이렇게 하면 틀린다 —
 *   · 검색어축으로 클릭률을 재는 것 → 실제보다 «훨씬 낮게» 나온다
 *   · 검색어축의 몫(%)을 「우리 노출의 N%」로 옮겨 적는 것
 *     ⇒ 그것은 «보이는 22% 안에서의» 몫이다. 오늘 내가 실제로 이렇게 잘못 적었다 —
 *       「붙여넣은 주소가 우리 노출의 33%」 → 바른 말은 「검색어가 보이는 1,130노출의 33%」
 *
 * ✅ 그러니 이렇게 쓴다 —
