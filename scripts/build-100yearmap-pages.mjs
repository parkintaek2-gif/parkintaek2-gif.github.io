...
async function main() {
   ...
   */
  /* ⚠ 학교 한 줄마다 기준시각을 넣어야 해서 여기서 먼저 읽는다(전에는 저장 직전에 읽었다) */
  ...
  const schoolPages = 낼학교들.map((s) => {
    ...
    return {
      ...
      출처: 'NEIS 교육정보 개방 포털',
      /**
       * 🔴 2026-08-14 — 여기가 **2,525곳 전부 null** 이었다. 8번이 무오류 확인에서 잡았다.
       *
       *   전에는 「수집 시각은 summary 에 둔다」로 비워 뒀다. 지면에는 날이 찍히니
       *   손님에게 보이는 흠은 아니었다. 그런데 **이 파일을 파는 날** 다시 물릴 자리다 —
       *   자료만 떼어 주면 「언제 받은 것인지」를 그 파일이 스스로 말하지 못한다.
       *
       * ⛔ 손으로 안 박는다. summary 와 **같은 값**을 쓴다 — 두 곳에 두면 갈린다.
       */
      기준시각: 수집시각,
    };
  ...
  const summary = {
    ...
    원천수집시각: 수집시각,
    /**
     * 🔴 사장님(2026-08-15) — 「판단 장치는 <데이터, 검증된 과학기술, 학술적 근거>」
     * ⭐ 5번(8/15) — 「③은 «그 방법의 알려진 한계가 적혀 있나»로 잰다」
     *   ⇒ «어떻게 셌나»와 «무엇을 못 보여 주나»를 자료 안에 같이 낸다
     */
    셈과한계: {
      '② 어떻게 셌나': {
        전국개설교수: '그 학과 이름을 가진 **학교를 센 것**이다(학과 수가 아니다)',
        같은학과판정: '**학과 이름이 같으면 같은 학과**로 본다',
        학과페이지기준: `학교 ${MIN_SCHOOLS}곳 이상인 학과만 독립 지면을 만든다`,
      },
      '③ 무엇을 못 보여 주나': [
        '한 학교에 같은 학과가 둘이어도 **하나로 센다**',
        '이름이 다르면 다른 학과가 된다 — 「정보처리과」와 「정보처리계열」은 갈린다',
        '🔴 `전국순위` 칸이 자료에 있으나 **지면에 「몇 위」를 쓰지 않는다**(우리 규칙). 자료에 있다고 화면에 내보내지 않는다',
        `학교 ${MIN_SCHOOLS}곳 미만인 학과는 독립 지면이 없다 — 「없는 학과」가 아니라 「지면이 없는 학과」다`,
      ],
    },
