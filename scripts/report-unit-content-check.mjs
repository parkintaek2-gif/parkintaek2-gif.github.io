 ...
 * 새  만듦   ① **제목이 관심을 확 끄는가**(판단) **그리고**
 *            ② **그 지면의 방문이 우리 평균보다 많은가**(잰 수 — GA4 지면열림)
 * ```
 * ⛔ ①은 «판단»이고 ②는 «잰 수»다. 섞어서 「측정했다」고 말하지 않는다.
 *   ②를 못 잰 지면(GA4 에 줄이 없는 것)은 **0 이 아니라 「못 쟀다」**로 둔다.
...
export function 공격형갈래(글, 배포시각 = null, 열림 = null, 평균열림 = null) {
  ...
  const 제목 = 앞말(글, 'title') ?? '';
  /* ② 방문이 평균보다 많은가 — 못 재면 null 이다(0 이 아니다) */
...
if (내가) {
  ...
  쓴다('| 유닛 | 제목 | 무엇을 말한 글인가 | 반응 | 만듦 |');
    ...
    쓴다('이슈 메이킹   ① 제목이 관심을 확 끄나(**판단**) ② 그래서 방문이 **평균보다 많나**(**잰 수**)');
    쓴다(`             평균은 우리 기사 지면 ${기사지면.length}개의 지면열림 평균 = ${평균열림 ?? '⬜ 못 쟀다'}`);
    쓴다('```');
    쓴다('⛔ ②를 못 잰 지면은 **0 이 아니라 ⬜(못 쟀다)** 입니다 — GA4 에 줄이 없으면 수를 지어내지 않습니다.');
    쓴다('⛔ ①은 제가 «읽고 매긴 판단»입니다. 제목을 위 표에 그대로 실어 두었으니 뒤집으실 수 있습니다.', '');
  }
  ...
  쓴다('## Ⅳ. 앞서 「못 쟀다」던 셋 — **해결했습니다** (사장님 2026-09-03 「해결해」)', '');
