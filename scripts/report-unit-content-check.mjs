...
/**
 * 제목에 든 수를 그대로 뽑는다. 「이슈를 만듦」의 «근거»를 표에 보이기 위한 것이다 —
 * ⛔ 「만듦 ✅」만 적으면 사장님이 왜 그렇게 갈렸는지 되짚을 수가 없다.
 */
export function 제목의수(제목) {
  return (String(제목 ?? '').match(/[0-9]+(?:[.,][0-9]+)*%?/g) ?? []);
}
 ...
 *   「Securitie」로 나갔다. 정규식을 heredoc 으로 넣다가 역슬래시가 먹혀 «공백»을 뜻하던 것이
 *   «글자 s»가 됐다. 사장님께 나갈 표에서 남의 회사 이름이 틀리는 자리다.
...
if (내가 && process.argv.includes('--자가시험')) {
  ...
  검('제목에서 수를 뽑는다', JSON.stringify(제목의수('421 titles moved 3.5%')) === JSON.stringify(['421', '3.5%']));
  검('수가 없으면 빈 배열', 제목의수('Korea does not chart it first').length === 0);
  검('⛔ 빈 것도 안 터진다', 제목의수(undefined).length === 0);
  검('세로막대를 갈아 표를 안 깨뜨린다', !칸글('가|나').includes('|'));
  검('긴 글은 자르고 표시한다', 칸글('가'.repeat(200)).endsWith('…'));
  검('짧은 글은 그대로', 칸글('짧다') === '짧다');
  ...
  for (const 방 of 유닛.콘텐트) {
    ...
    for (const f of fs.readdirSync(d).filter((x) => x.endsWith('.md'))) {
      ...
      결과.편들.push({
        ...
        갈래: 앞말(글, 'category'),
        반응: g.반응, 만듦: g.만듦, 늦음: g.늦음,
        기준일: (앞말(글, 'dataAsOf') ?? '').slice(0, 10) || null,
        수: 제목의수(제목),
      });
    }
  }
...
if (내가) {
   ...
   */
  쓴다('### 1-4. 어제 나간 글 한 편씩 — 무엇이고, 어느 갈래인가', '');
  ...
  if (!편전부.length) {
    ...
    쓴다('| 유닛 | 제목 | 자료 기준일 | 발행일 | 늦음 | 제목에 든 수 |');
    쓴다('| --- | --- | --- | --- | ---: | --- |');
    for (const x of 편전부) {
      쓴다(`| ${x.유닛} | ${칸글(x.제목, 46)} | ${x.기준일 ?? '⬜ 없다'} | ${날}`
        + ` | ${x.늦음 === null ? '⬜ 못 쟀다' : `${x.늦음}일`}`
        + ` | ${x.수.length ? x.수.join(' · ') : '없다'} |`);
    }
    쓴다('');
    쓴다('⚠ 「늦음」이 7일 안이면 **이슈에 반응**, 제목에 수가 있으면 **이슈를 만듦**으로 셌습니다.');
    쓴다('그 두 칸이 위 표의 ✅ 와 하나하나 맞물립니다 — 되짚어 보실 수 있습니다.', '');
  }
