...
if (내가) {
  ...
  쓴다(`⚠ 「해당 없음」은 0 이 아닙니다 — 이슈 시계가 «없는» 지면입니다. 3번 자료는 국가통계`, '');
  쓴다('  (연간·격년 조사)라 KCW 처럼 「바깥 콘텐트 발행 시각」이 아예 없습니다. 없는 시계에', '');
  쓴다(`  ${반응한도}시간 창을 대면 영영 0 이고, 그 0 이 「안 했다」로 읽힙니다. 그래서 창을 바꾸지 않고`, '');
  쓴다('  **분모에서 뺍니다.** ⭐ 3번이 지면에 `issueAt` 한 줄을 적으면 그 장은 KCW 와 같은 자로 셉니다.', '');
  쓴다('  「이슈생성」(방문이 평균보다 많은가)은 시계가 없어도 GA4 지면열림으로 잽니다.', '');
