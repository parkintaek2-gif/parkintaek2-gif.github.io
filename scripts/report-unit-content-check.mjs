...
function 발행센다(날, 유닛, 지면표 = null, 평균열림 = null) {
   ...
   * index.astro 를 새 지면으로 센다.
   * ⛔⛔ [2026-09-04 00:0x · 5번 제안에 답함] 「만듦」(2번 조건 — 방문이 평균보다 많은가)은
   *   dataAsOf 없이 GA4 지면열림만으로 잴 수 있다 — 아래에서 잰다.
   * ⛔ 「반응」(바깥 콘텐트 발행 시각에서 12시간 안)은 여기서 여전히 못 잰다. 3번 자료는
   *   국가통계(월간·연간·2년 주기 조사 등)라 KCW 같은 «바깥 발행 시각»이 촘촘히 없다 —
   *   12시간 창을 그대로 대면 항상 반응 아님만 나와 헛도는 자가 될 위험이 있다(6번의 옛
   *   pages: 앞말 함정과 같은 결). 이 잣대를 3번에 어떻게 바꿔 낼지는 사람 판단(5번·
   *   사장님)을 먼저 여쭙는다 — 그때까지 반응은 ⬜못쟀다로 정직하게 둔다. */
  if (유닛.아스트로지면) {
...
if (내가) {
  ...
  for (const u of 유닛들) {
    ...
    const 반응칸 = !텍스트있나 ? '—' : p.반응못잼 ? '⬜ 못 쟀다' : p.반응;
    const 만듦칸 = !텍스트있나 ? '—' : p.만듦;
    쓴다(`| ${u.번호} | ${u.이름} | **${몫}** | ${반응칸} | ${만듦칸} | ${커밋[u.번호] ?? 0} |`);
  }
  쓴다('', `합계 — 발행 **${총편수}편** · 이슈 반응 ${총반응} · 이슈 생성 ${총만듦}`, '');
  쓴다('⚠ 3번은 «이슈화»(12시간 창)만 못 잰다(위 표에 ⬜로 뜬다) — 국가통계 자료라 KCW처럼', '');
  쓴다('  촘촘한 「바깥 발행 시각」이 없다. 「이슈생성」(방문이 평균보다 많은가)은 GA4로 잰다.', '');
  쓴다('  총반응 합계에는 3번분이 0으로 들어가 있다. 실제보다 적게 보일 수 있다.', '');
  ...
  쓴다('| 유닛 | 제목 | 무엇을 말한 글인가 | 반응 | 만듦 |');
    ...
    쓴다('```');
    쓴다('이슈화   바깥 콘텐트 발행 시각에서 **12시간 안**에 우리가 냈나.');
