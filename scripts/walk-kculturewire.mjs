...
/**
 * 본문 글자 수 — 태그·스크립트·**양식**을 걷어낸 뒤 센다.
 * ⛔ `<style>` 을 안 걷으면 우리 CSS 가 본문으로 세어진다. 한 바퀴째에 그래서
 *    어느 지면이든 3,500자가 넘어 보였고 「빈 지면 0건」이 거짓일 뻔했다.
 */
const 글자수 = (h) => h
...
/**
 * **지면이 아닌 것**. 지면 잣대를 대면 안 된다.
 * ⚠ 두 바퀴째에 `/rss.xml` 이 「막다른 곳(나가는 링크 0개)」으로 걸렸다.
 *   피드는 원래 HTML 링크가 없다. **자가 틀린 것이지 사이트가 틀린 것이 아니다.**
 *   이런 것을 걸린 곳으로 세면 진짜 걸린 곳이 묻힌다.
 */
const 지면아님 = (u) => /\.(xml|json|txt)$/i.test(u);
...
for (const [경로, v] of 줄) {
  /* 안 열리는 것은 지면이든 아니든 걸린 것이다. */
  if (v.code !== 200) { 적기('① 안 열린다', 경로, `${v.code || '못 붙음'}${v.err ? ` (${v.err})` : ''}`); continue; }
  /* 그 아래 잣대는 **지면에만** 댄다. 피드·사이트맵에 대면 헛것이 나온다. */
  if (지면아님(경로)) continue;
  if (v.글자 < 900) 적기('② 빈 채로 뜬다', 경로, `본문 ${v.글자}자`);
  if (v.표 && v.표줄 <= 1) 적기('② 빈 채로 뜬다', 경로, '표가 있다는데 줄이 없다');
  if (v.나가는링크 <= 3) 적기('③ 막다른 곳', 경로, `나가는 링크 ${v.나가는링크}개`);
