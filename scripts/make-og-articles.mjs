 ...
 */
/**
 * Georgia 옛꼴 숫자가 기준선 «아래»로 내려가는 깊이 (글자크기에 대한 비율).
 *
 * 🔴 3·4·5·7·9 가 내려간다. `make-kcw-cardnews.mjs` 가 2026-08 에 같은 것으로
 *   겹침을 냈고 그 파일에 값이 적혀 있다. **이 자에는 안 옮겨져 있었다.**
 */
export const 옛꼴숫자내림 = 0.22;

/** 딱지 글자의 윗머리 높이 (글자크기 대비 어림) */
export const 딱지윗머리 = 0.78;

/** 딱지 글자 크기 — 카드SVG 와 «같은 값»을 써야 한다 */
export const 딱지크기 = 28;

/**
 * 큰 수 기준선에서 딱지 기준선까지 몇 px 인가.
 *
 * ⛔ 고정 px 로 두지 않는다 — 수 크기가 바뀌면 또 겹친다.
 *   숫자 내림(0.22em) + 딱지 윗머리(0.78 × 28px) + 여유 6px 로 «잰다».
 * ⚠ 「774」처럼 내려가는 숫자만 든 수가 가장 위험하다. 그 최악을 기준으로 둔다 —
 *   숫자마다 재서 좁히면 다음 사람이 그 미묘함을 모르고 깬다.
 */
export function 딱지간격(수크기값, 딱지글자 = 딱지크기, 여유 = 6) {
  const 내림 = 수크기값 * 옛꼴숫자내림;
  return Math.ceil(내림 + 딱지글자 * 딱지윗머리 + 여유);
}

export function 자리(figure, 줄수) {
  const 크기 = 수크기(figure);
  const 수 = 130 + 크기;          // 수의 밑줄
  /* 🔴 [2026-09-07] 전에는 +42 고정이었다. 「774」에서 4px 겹쳤다 — 눈으로 보고 고쳤다 */
  const 딱지 = 수 + 딱지간격(크기);
  const 가로줄 = 딱지 + 36;
...
if (process.argv[1] && process.argv[1].endsWith('make-og-articles.mjs')) {
  ...
  /**
   * 🔴 [2026-09-07] 「774」의 딱지가 숫자와 4px 겹쳐 있었다. 눈으로 열어 보고 알았다.
   *   Georgia 옛꼴 숫자(3·4·5·7·9)가 기준선 아래로 0.22em 내려가는데,
   *   딱지를 +42px 고정으로 뒀다. 아래 시험이 그 간격을 지킨다.
   */
  본다('🔴 딱지가 옛꼴 숫자 내림을 피한다 (120px 수)',
    딱지간격(120) >= 120 * 옛꼴숫자내림 + 딱지크기 * 딱지윗머리);
  본다('🔴 전에 쓰던 42px 고정으로는 모자랐다', 딱지간격(120) > 42);
  본다('수가 작아지면 간격도 준다', 딱지간격(68) < 딱지간격(120));
  본다('⛔ 수가 작아져도 내림보다는 넓다',
    딱지간격(68) >= 68 * 옛꼴숫자내림 + 딱지크기 * 딱지윗머리);
  본다('자리() 가 그 간격을 쓴다',
    자리('774', 2).딱지 - 자리('774', 2).수 === 딱지간격(수크기('774')));
  본다('⛔ 가로줄이 딱지 아래에 있다', 자리('774', 2).가로줄 > 자리('774', 2).딱지);
  본다('⛔ 제목이 가로줄 아래에 있다', 자리('774', 2).제목 > 자리('774', 2).가로줄);
  본다('⛔ 마지막 줄이 630 안에 든다', 자리('774', 3).마지막 < 630);
  본다('⛔ 가장 긴 수로도 630 안에 든다', 자리('1,234,567,890', 3).마지막 < 630);
