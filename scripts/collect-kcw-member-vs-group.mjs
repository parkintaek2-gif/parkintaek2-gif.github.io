...
async function 주된일() {
  ...
  const 나갈것 = {
    /* 🔴 [2026-09-08] 여기에 `toLocaleString('ko-KR')` 을 썼더니 「오후」가 자료에 박혔고,
       ...
       *
       * ⭐⭐ [같은 날 오후에 알았다] **고치는 자가 이미 있었다.**
       *   `scripts/lib/kcw-date.mjs` 의 `지은날()` 이 「2026. 9. 7. 오후 11:58:54」를
       *   「2026-09-07」로 바꿔 준다. 8/09 에 같은 사고(「Updated … 오전 8:40:57 KST」)를
       *   겪고 만들어 둔 자다. 검사도 이미 있었다 — `check-kcw-korean-leak` 이 오늘 그것을 잡았다.
       *   ⛔ 나는 그것을 «안 찾고» 자료의 시각을 직접 갈아 끼웠다. 결과는 같지만 길이 두 갈래가 됐다.
       *   ✅ 다음에는 이 순서로 한다 — ① 지면에서 `지은날()` 을 씌운다
       *     ② 그것으로 안 되면 그때 자료를 손댄다.
       *   ⚠ 같은 꼴이 src/data 의 자료 43개에 들어 있다(2026-09-08 실측). 그것들은 «지면이
       *     안 찍는 한» 새지 않는다. 그러니 43개를 쓸어 고치는 것이 할 일이 아니고,
       *     **새 지면에서 시각을 찍을 때 `지은날()` 을 씌우는 것**이 할 일이다.
