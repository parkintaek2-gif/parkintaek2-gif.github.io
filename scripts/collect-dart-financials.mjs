...
export function 계정정규화(원문) {
  ...
  if (!s) return '';
  /* 🔴 [2026-09-11 실측] 맨 앞 차례표 — 「XI. 당기순이익」(고려아연) · 「Ⅵ. 당기순이익」(가온전선).
     K-GAAP 계정명은 늘 한글이다 — 맨 앞이 로마자·숫자+마침표로 시작하는 진짜 계정은 없다. */
  s = s.replace(/^(?:[ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫ]+|[IVXLCDM]+|\d+)\.\s*/, '');
  s = s.replace(/\s+/g, '');
...
function 자가시험() {
  ...
  재다('🔴 계정정규화: 유니코드 로마숫자 차례표 — 「Ⅵ. 당기순이익」(가온전선 실측) → 「당기순이익」',
    계정정규화('Ⅵ. 당기순이익') === '당기순이익');
  재다('🔴 계정정규화: ASCII 로마자 차례표 — 「XI. 당기순이익」(고려아연 실측) → 「당기순이익」',
    계정정규화('XI. 당기순이익') === '당기순이익');
  재다('계정정규화: 아라비아 숫자 차례표도 뗀다 — 「1. 매출액」 → 「매출액」',
    계정정규화('1. 매출액') === '매출액');
  재다('⛔ 계정정규화: 차례표를 떼도 끝 꼬리는 그대로 마저 뗀다 — 「Ⅵ. 영업이익(손실)」 → 「영업이익」',
    계정정규화('Ⅵ. 영업이익(손실)') === '영업이익');
  ...
  재다('🔴 계정찾기: 「당기순손실」(SK이노베이션 실측, 적자라 이름이 다르다) 도 당기순이익으로 잡는다', (() => {
    const 것 = [{ sj_nm: '포괄손익계산서', account_nm: '당기순손실', thstrm_amount: '-5436441431000' }];
    return 금액(계정찾기(것, '당기순이익')?.thstrm_amount) === -5436441431000;
  })());
  재다('계정찾기: 「당기순이익」이 있으면 「당기순손실」보다 먼저 잡는다 — 동의어 순서가 먼저다', (() => {
    const 것 = [
      { sj_nm: '포괄손익계산서', account_nm: '당기순손실', thstrm_amount: '-1' },
      { sj_nm: '포괄손익계산서', account_nm: '당기순이익', thstrm_amount: '999' },
    ];
    return 금액(계정찾기(것, '당기순이익')?.thstrm_amount) === 999;
  })());
