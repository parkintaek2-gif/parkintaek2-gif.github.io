...
/*
 * 🔴 [2026-09-16 · 6번 · 5번 지시, 9/17 21:00] 신용등급(SMarkets Grade)의 «유일한 병목».
 *   /data/company-credit 를 냈는데 여섯 축 가운데 넷을 못 재 등급 나오는 회사가 0곳이었다.
 *   이자보상배율(금융비용)·차입금의존도(단기·장기차입금·사채)·유동비율(유동자산·유동부채)·
 *   알트만 Z(이익잉여금)가 이 일곱 계정에 걸려 있다. DART 응답에서 실제로 확인한 꼴만
 *   넣는다(2026-09-16, 삼성전자 00126380·2025·CFS 로 한 번 불러 실측) — 짐작 금지.
 */
export const 핵심계정 = ['자산총계', '자본총계', '당기순이익', '매출액', '영업이익',
  '유동자산', '유동부채', '이익잉여금', '금융비용', '단기차입금', '장기차입금', '사채'];
...
export function 계정정규화(원문) {
  ...
  for (let i = 0; i < 3; i += 1) {
    const 앞 = s;
    s = s.replace(/[,·][^,·()]*$/, '');          /* 「,총액」 */
    s = s.replace(/\((손실|이익|손익|결손금|B|A|주|단위:[^)]*)\)$/, ''); /* 「(손실)」·「(B)」·「(결손금)」 */
    if (s === 앞) break;
  }
  return s;
}
...
export const 동의어 = {
  ...
  영업이익: ['영업이익', '영업손익'],
  /* 🔴 [2026-09-16] 신용등급 일곱 계정 — 삼성전자 실측(재무상태표·손익계산서)에서 그대로 가져왔다.
     현금흐름표에는 「단기차입금의 순증가(감소)」·「사채 및 장기차입금의 상환」처럼 이름이 다르게
     붙어 있어 헷갈리지 않는다 — 계정찾기() 는 정확히 같은 이름만 잡는다(부분일치 아님). */
  유동자산: ['유동자산'],
  유동부채: ['유동부채'],
  이익잉여금: ['이익잉여금'],
  금융비용: ['금융비용'],
...
function 자가시험() {
  ...
  재다('핵심계정이 열둘이다(2026-09-16, 신용등급 일곱 계정 추가)',
    핵심계정.length === 12 && 핵심계정.includes('자본총계') && 핵심계정.includes('사채'));
  ...
  const 줄들 = [
    ...
    /* 🔴 [2026-09-16] 삼성전자 실측(00126380·2025·CFS)에서 그대로 가져온 일곱 줄 +
       현금흐름표의 «가짜 후보»(계정찾기가 헷갈리면 안 되는 줄들) */
    { sj_nm: '재무상태표', account_nm: '유동자산', thstrm_amount: '247684612000000' },
    { sj_nm: '재무상태표', account_nm: '유동부채', thstrm_amount: '106411348000000' },
    { sj_nm: '재무상태표', account_nm: '이익잉여금', thstrm_amount: '402135600000000' },
    { sj_nm: '재무상태표', account_nm: '단기차입금', thstrm_amount: '17574980000000' },
    { sj_nm: '재무상태표', account_nm: '장기차입금', thstrm_amount: '6479517000000' },
    { sj_nm: '재무상태표', account_nm: '사채', thstrm_amount: '7134000000' },
    { sj_nm: '손익계산서', account_nm: '금융비용', thstrm_amount: '11733764000000' },
    { sj_nm: '현금흐름표', account_nm: '단기차입금의 순증가(감소)', thstrm_amount: '4655349000000' },
    { sj_nm: '현금흐름표', account_nm: '사채 및 장기차입금의 상환', thstrm_amount: '2852971000000' },
  ];

  재다('🔴 계정찾기: 「단기차입금」은 재무상태표 것을 잡는다 — 현금흐름표의 「…순증가(감소)」와 안 섞인다',
    금액(계정찾기(줄들, '단기차입금')?.thstrm_amount) === 17574980000000);
  재다('🔴 계정찾기: 「사채」는 재무상태표 것을 잡는다 — 현금흐름표의 「사채 및 장기차입금의 상환」과 안 섞인다',
    금액(계정찾기(줄들, '사채')?.thstrm_amount) === 7134000000);
  재다('계정찾기: 유동자산·유동부채·이익잉여금·금융비용·장기차입금도 잡는다', (() => (
    금액(계정찾기(줄들, '유동자산')?.thstrm_amount) === 247684612000000
    && 금액(계정찾기(줄들, '유동부채')?.thstrm_amount) === 106411348000000
    && 금액(계정찾기(줄들, '이익잉여금')?.thstrm_amount) === 402135600000000
    && 금액(계정찾기(줄들, '금융비용')?.thstrm_amount) === 11733764000000
    && 금액(계정찾기(줄들, '장기차입금')?.thstrm_amount) === 6479517000000
  ))());
  재다('🔴 계정정규화: 「이익잉여금(결손금)」 → 「이익잉여금」', 계정정규화('이익잉여금(결손금)') === '이익잉여금');
  ...
  재다('🔴 한줄만들기: 없는 칸은 null 로 «남는다» — 0 으로 채우지 않는다', (() => {
    const r = 한줄만들기([]);
    return 핵심계정.every((k) => r.값[k] === null) && r.없는계정.length === 핵심계정.length;
  })());
  ...
  if (재본다) {
    ...
    for (const c of 상장.slice(0, 10)) {
      ...
      if (판정.정상) {
        ...
        console.log(`  ✅ ${c.이름.padEnd(14)} ${j.list.length}행 · 핵심 ${핵심계정.length - r.없는계정.length}/${핵심계정.length}`
          + ` · 영문 ${en.붙은}/${en.전체}`);
