...
if (내가실행됐다) {
  ...
  if (빠진것.length) {
    ...
    /**
     * 🔴 [2026-09-08] **반쯤 지어진 dist 를 알아본다.**
     *   옆 유닛이 빌드를 돌려 dist 를 지우고 다시 채우는 «그 순간»에 이 자가 읽으면
     *   멀쩡한 지면 수십 장이 「없다」로 잡힌다. 오늘 실제로 37줄이 빠졌고
     *   그중에 홈(/)·/titles·/watched 가 있었다. 잠시 뒤엔 다 있었다.
     * ⛔ 그대로 쓰면 llms.txt 가 짧아진 채로 나간다 — 사장님이 못박은 GEO 에 걸린다.
     * ⭐ 지면 하나가 정말 없어진 경우는 «한두 줄»이 빠진다. 37줄은 그것이 아니다.
     */
    const 반쯤지어짐문턱 = 0.10;
    /* ⚠ 칸 이름은 `줄들` 이다 — 자료를 열어 확인했다. 처음에 `것들` 로 짐작해 쓰는 바람에
       분모가 0 이 되어 빠진몫이 100% 로 나왔다(멀쩡한 판을 막았다). */
    const 전체줄수 = 빠진것.length + 갈래들.reduce((a, g) => a + (g.줄들?.length ?? 0), 0);
    const 빠진몫 = 전체줄수 > 0 ? 빠진것.length / 전체줄수 : 0;
    if (빠진몫 > 반쯤지어짐문턱) {
      console.log(`\n🔴 **안 쓴다.** 뺀 줄이 ${(빠진몫 * 100).toFixed(0)}% 다 — 문턱 ${반쯤지어짐문턱 * 100}% 를 넘었다.`);
      console.log('   ⭐ dist 가 «반쯤 지어진» 상태로 보인다. 옆 자리가 빌드를 돌리는 중일 수 있다.');
      console.log('   ✅ `npm run build` 를 «끝까지» 돌린 뒤 이 자를 다시 돌리십시오.');
      console.log('   ⛔ 이 상태로 쓰면 안내문이 짧아진 채로 나갑니다 — 그것이 더 나쁩니다.');
      process.exit(1);
    }
