...
export function 글에서뽑는다(글) {
  ...
  const 자본 = 뽑(자본재);
  const 대차합계 = 뽑(대차합계재);
  /* 🔴🔴 [2026-09-17 04:3x] **OCR 깨짐을 «먼저» 걸러 냈다가 멀쩡한 것을 버렸다.**
   * ─────────────────────────────────────────────────────────────────────────
   * UNIONCOOP 재무상태표를 OCR 하니 이렇게 나왔다 —
   * ```
   *   TOTAL ASSETS      3,823,987,230   3,637,558.646     ← 전기 열만 깨졌다
   *   Total equity      2,722,100,923   _—_-2.619.756,881
   *   Total liabilities 1,101,886,307   1,017,801,765
   * ```
   *   **당기 열은 멀쩡하고 검산도 딱 맞는다**(1,101,886,307 + 2,722,100,923 = 3,823,987,230).
   *   그런데 「점 천단위가 세 번 넘게 보인다」는 이유로 통째로 버렸다.
   * ⭐ 깨진 수는 «검산»이 어차피 걸러 낸다. 미리 버릴 까닭이 없다.
   * ✅ 그래서 순서를 뒤집는다 — **먼저 뽑아 보고, 실패했을 때만** 「OCR 이 깨졌다」고 적는다.
   *   ⛔ 판정을 앞에 두면 「못 쟀다」가 아니라 «재 보지도 않은 것»이 된다. */
  if (!자산.length && !부채.length && !자본.length && !대차합계.length) {
    return { 됐나: false, 왜: OCR깨졌나(글) ? 'ocr-garbled-numbers' : 'no-balance-sheet-in-document' };
  }
  /* ① 셋이 다 있으면 곧바로 검산한다 */
  ...
  const 끝 = 검산해서고른다(자산, 부채, 자본);
  /* 라벨은 찾았는데 검산이 안 맞았다 — 그 수가 OCR 로 깨진 것이면 그렇게 적는다.
     ⚠ 「검산이 안 맞았다」와 「글자를 잘못 읽었다」는 다음 사람이 손댈 곳이 다르다. */
  if (!끝.됐나 && OCR깨졌나(글)) return { 됐나: false, 왜: 'ocr-garbled-numbers' };
  return 끝;
}
...
function 자가시험() {
  ...
  /* 🔴🔴 UNIONCOOP 실측 — **전기 열만 깨지고 당기 열은 멀쩡한 판**이 흔하다.
     깨졌다고 먼저 버리면 멀쩡한 것까지 잃는다. 검산이 어차피 걸러 준다. */
  const 반만깨짐 = [
    'TOTAL ASSETS      3,823,987,230   3,637,558.646',
    'Total equity      2,722,100,923   2.619.756,881',
    'Total liabilities 1,101,886,307   1,017,801,765',
  ].join('\n') + 'x'.repeat(2100);
  const r5 = 글에서뽑는다(반만깨짐);
  다('🔴 깨진 열이 섞여 있어도 «검산이 맞는 짝»이 있으면 받는다',
    r5.됐나 && r5.자산 === 3823987230 && r5.부채 === 1101886307 && r5.자본 === 2722100923);
  /* ⚠ 위 보기는 «점 천단위»가 한 번뿐이라 OCR깨짐 판정에 안 걸린다(시험이 잡아 줬다).
     그래서 확실히 깨진 글을 따로 만들어, 그래도 «검산이 맞으면 받는지»를 본다. */
  const 많이깨짐 = 반만깨짐.replace('x'.repeat(2100), '1.017.801.765 2.619.756.881 3.637.558.646' + 'x'.repeat(2100));
  다('🔴 글이 OCR 깨짐으로 판정되어도, 검산이 맞는 짝이 있으면 받는다',
    OCR깨졌나(많이깨짐) === true && 글에서뽑는다(많이깨짐).됐나 === true);
