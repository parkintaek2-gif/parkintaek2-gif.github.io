...
적기('광고수입', '🔴 구글에 **실제로 잡혔나**', '못 쟀다', '콘솔을 아직 안 열었다');
적기('광고수입', '🔴 네이버에 잡혔나', '구멍', '등록 전 · 크롤러 5일 0건');
...
적기('외부유입', '🔴 스레드·X·인스타에 **나갔나**', '구멍', '0장 (계정은 사장님 손 · 자산은 준비됨)');
적기('외부유입', '🔴 유튜브에 나갔나 — **우리는 문이 있다**', '구멍', '오늘 0편 (총 1편)');
적기('외부유입', '카드뉴스가 있나', 카드뉴스생성기 ? '통과' : '구멍', 카드뉴스생성기 ? '있다' : '생성기 없음 · 0장');
...
const 셈 = { 통과: 0, 구멍: 0, '못 쟀다': 0 };
for (const r of 줄) 셈[r.꼴] += 1;

console.log('');
console.log(`■ 백년지도 배포 퀴즈 — ${오늘}`);
console.log('');
for (const 목적 of ['광고수입', '외부유입', '둘 다', '🔴 나이']) {
  const ㄱ = 줄.filter((r) => r.목적 === 목적);
  if (!ㄱ.length) continue;
  console.log(`  [${목적}]`);
  for (const r of ㄱ) {
    const 표 = r.꼴 === '통과' ? '✅' : r.꼴 === '구멍' ? '⛔' : '⬜';
    console.log(`   ${표} ${r.물음.padEnd(34)} ${r.값}`);
  }
}
console.log('');
console.log(`통과 ${셈.통과} · ⛔ 구멍 ${셈.구멍} · ⬜ 못 쟀다 ${셈['못 쟀다']}`);
console.log('');
if (셈.구멍 === 0 && 셈['못 쟀다'] === 0) {
