...
export const 면제 = [
  ...
  /**
   * 🔴 [2026-09-03] 소리 검토 지면. **손님 지면이 아니다** —
   *   `<meta name="robots" content="noindex,nofollow">` 이고, 영상에 입힌 소리를
   *   우리끼리 들어 보는 자다. 사장님 지시 원문을 그 화면에 그대로 실어야 해서 한국어다.
   * ⛔ 「내부용이니까」로 지면을 면제하는 것이 아니다 — noindex 라서 검색·손님이 오지 않는다.
   *   noindex 를 떼는 날 이 면제는 **틀린 면제**가 된다. 그때 이 줄을 지운다.
   */
  { 지면: 'video/review/', 글월: null, 잰다: 'noindex 인 내부 검토 자다. 사장님 지시 원문을 그대로 실어야 하므로 한국어다. noindex 를 떼면 이 면제를 지운다' },
  /**
   * 🔴 [2026-09-07] 공공데이터 자료의 «제 이름»과 «제 딱지». 우리가 지은 말이 아니다.
   *   「지역별 청소년활동 관심지도」는 그 자료의 이름이고, 「갱신불가」는 포털이 그 자료에
   *   붙여 놓은 딱지다. 우리가 「a map of youth-activity interest by region」으로만 적으면
   *   독자가 포털에서 **그 자료를 찾을 수 없다.** 그래서 원어를 함께 둔다.
   * ⭐ 지면이 그 자리에서 뜻을 영어로 풀어 준다 —
   *   「carry the label 갱신불가, meaning they cannot be updated」.
   *   뜻을 안 풀어 주면 이 면제는 **틀린 면제**다.
   * ⛔ 지면을 통째로 면제하지 않는다 — 이 «낱말들»만 비운다.
   */
  { 지면: 'korea-culture-data-market.html', 글월: '갱신불가', 잰다: '공공데이터포털이 그 자료에 붙인 딱지다. 지면이 바로 뒤에서 뜻을 영어로 풀어 준다' },
  { 지면: 'korea-culture-data-market.html', 글월: '지역별 청소년활동 관심지도', 잰다: '자료의 제 이름이다. 영어로만 적으면 독자가 포털에서 그 자료를 못 찾는다' },
  { 지면: 'korea-culture-data-free-but-not-maintained.html', 글월: '지역별 청소년활동 관심지도', 잰다: '같은 자료의 제 이름이다. 기사와 지면이 같은 자료를 가리킨다' },
  /**
   * 🔴 [2026-09-07] `/tour-city-languages` 는 **같은 이름이 언어마다 어떻게 적히는지**를
   *   보여 주는 지면이다. 한국어 칸에 「유노윤호」가 없으면 그 표가 무엇을 재는지 사라진다.
   *   같은 줄에 타이 글자도 함께 있다 — 한국어만 특별히 두는 것이 아니다.
   * ⛔ 그러니 이것은 «새는 한국어»가 아니라 «자료 그 자체»다.
   */
  { 지면: 'tour-city-languages.html', 글월: null, 잰다: '이름이 언어마다 어떻게 적히는지 보여 주는 지면이다. 한국어 표기가 자료의 한 칸이다' },
];
 ...
 * **법으로 정해진 등록번호는 비운다.**
 *
 * 🔴 [2026-09-03] 이 자가 지면 2,218장을 «전부» 빨강으로 냈다. 걸린 것은 하나뿐이었다 —
 *   꼬리에 있는 통신판매업 신고번호 `2026-세종-0591`. 그 번호의 «가운데 토막이 관청 이름»이라
 *   한글이고, 괄호 밖이라 규칙을 그대로 통과했다.
 * ⛔ 2,218장이 다 빨강이면 그 안에 있는 «진짜» 한국어 누출을 아무도 못 본다.
 *   맞는 상태를 흠으로 부르는 검사는 사람이 끄고, 끄면 그날부터 아무것도 안 지킨다.
 * ⭐ 우리가 지어낸 말이 아니라 **정부가 발급한 번호**다. 전자상거래법이 이 번호를 화면에
 *   내라고 정하고 있고, 우리가 영어로 바꿔 적으면 그건 조회가 안 되는 다른 번호가 된다.
 * ⛔ 지면을 통째로 면제하지 않는다 — 이 **한 가지 꼴**만 비운다.
 *   `20dd-<한글>-<숫자>` 밖의 한국어는 그대로 잡힌다. 번호 옆에 메모를 흘리면 여전히 운다.
 */
export function 등록번호비우기(글) {
  return String(글).replace(/\b20\d{2}-[가-힣]{2,6}-\d{3,6}\b/g, ' ');
}
...
if (내가실행됐다 && process.argv.includes('--selftest')) {
  ...
  /* 🔴 법으로 정해진 등록번호 — 이 넷이 이 규칙의 «좁음»을 지킨다 */
  재본다('신고번호는 안 잡는다', 한글조각('Mail-order licence 2026-세종-0591 Unit 101'), []);
  재본다('번호 밖의 한국어는 잡는다',
    한글조각('Mail-order licence 2026-세종-0591 우리 회사').map((x) => x.낱말), ['우리', '회사']);
  재본다('해가 아닌 것은 안 비운다', 한글조각('abc-세종-0591').map((x) => x.낱말), ['세종']);
  재본다('번호 없이 관청 이름만 있으면 잡는다', 한글조각('licence 세종 0591').map((x) => x.낱말), ['세종']);
