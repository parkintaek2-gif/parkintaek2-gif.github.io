 ...
 * 그래서 검색어를 보고 셋으로 가른다.
 */
/** 이만큼도 검색어가 안 잡히면 «본 것»이 «전체」를 대표하지 못한다고 본다 */
export const 최소커버리지 = 0.3;

export function 왜안눌리나(검색어들, 자료있나 = null, 페이지노출 = null) {
  const 합 = 검색어들.reduce((a, b) => a + (b.노출 || 0), 0);
  if (!검색어들.length || !합) {
    /* 🔴 [2026-09-04] 「자료를 안 받았다」와 「구글이 안 알려 준다」는 **다른 말**이다.
       SeoulMarkets 은 qp 를 받았는데도 노출 651 중 102(16%)만 검색어가 붙어 있었다.
       구글은 적게 나온 검색어를 «감춘다». 그 지면은 받아도 영영 못 잰다. */
    return {
      판정: '못 쟀다',
      까닭: 자료있나 === false
        ? '검색어(qp) 자료를 아직 안 받았다 — node scripts/fetch-gsc.mjs 로 받는다'
        : '자료는 받았으나 «구글이 이 지면 몫 검색어를 안 알려 준다» (적게 나온 것은 감춘다)',
      받아서될까: 자료있나 === false,
    };
  }
  /* 🔴 [2026-09-04, 3번] 검색어는 «있지만» 페이지 노출의 일부만 잡을 때가 있다.
     백년지도 `/school/7531159`(93노출)는 검색어 합이 16(17%)뿐이었다 — 나머지 83%는
     구글이 감춘 다른 검색어들이다. 그 16을 «전체 그림»으로 읽고 제목을 고치면,
     본 적도 없는 나머지 83%를 향해 헛짚는 것이다. ⛔ 합이 0보다 크다고 충분한 게 아니다. */
  if (페이지노출 != null && 페이지노출 > 0 && 합 / 페이지노출 < 최소커버리지) {
    return {
      판정: '못 쟀다',
      까닭: `검색어가 페이지 노출의 ${(합 / 페이지노출 * 100).toFixed(0)}%만 잡는다 — 나머지는 구글이 감췄다, 전체를 대표 못 한다`,
      받아서될까: false,
    };
  }
...
export function 재기() {
  ...
  /* 🔴 「안 받았다」와 「구글이 안 알려 준다」는 다른 말이다 — 처방이 갈린다 */
  봄('⬜ 자료를 «안 받은» 것은 받으면 된다고 말한다', 왜안눌리나([], false).받아서될까 === true);
  봄('⬜ 받았는데 없는 것은 «받아도 소용없다»고 말한다', 왜안눌리나([], true).받아서될까 === false);
  봄('받았는데 없는 것의 까닭에 «구글이 감춘다»가 적힌다', /감춘다/.test(왜안눌리나([], true).까닭));
  /* 🔴 [2026-09-04, 3번] 검색어 합이 페이지 노출의 일부만 잡을 때 — 「제목 문제」로 단정하지 않는다 */
  봄('⬜ 검색어가 페이지 노출의 30% 미만만 잡으면 «못 쟀다»로 가른다',
    왜안눌리나([{ 검색어: 'korean drama netflix', 노출: 16 }], true, 93).판정 === '못 쟀다');
  봄('⬜ 그 까닭에 「구글이 감췄다」가 적힌다',
    /감췄다/.test(왜안눌리나([{ 검색어: 'x', 노출: 16 }], true, 93).까닭));
  봄('✅ 30% 넘게 잡으면 그대로 「제목 문제」로 판정한다',
    왜안눌리나([{ 검색어: 'korean drama netflix', 노출: 40 }], true, 100).판정 === '제목 문제');
  봄('페이지노출을 안 주면(기존 호출) 커버리지 검사를 안 한다',
    왜안눌리나([{ 검색어: 'korean drama netflix', 노출: 1 }], true).판정 === '제목 문제');
  const 통 = 검색어모으기({ rows: [
...
if (나인가) {
  ...
  for (const s of 사이트들) {
    ...
    const 갈래셈 = {};
    for (const x of 샘) {
      const 판 = 왜안눌리나(검색어통.get(x.주소) || [], !!qp길, x.노출);
      x._판정 = 판;
