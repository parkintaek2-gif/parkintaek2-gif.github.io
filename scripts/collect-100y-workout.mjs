/**
 * collect-100y-workout.mjs — 나이대별 헬스(보디빌딩) 참여율(최근 1년간 참여 경험)
 *
 * ── 왜 ─────────────────────────────────────────────────────────
 * 자동완성 실측(2026-08-25) — 「헬스 인구」류 신호 확인 후 등산·골프와 같은 표
 * (문화체육관광부 국민생활체육조사, org 113 · DT_113_STBL_1029668)에서 찾았다.
 *
 * ── 항목명 확인(golf 함정 재확인) ───────────────────────────────
 * 5개 연도(2021~2025) 모두 원자료 항목명이 「보디빌딩(헬스)」로 **동일**하다 —
 * golf처럼 연도마다 이름이 바뀐 사례가 아니다. 5년을 그대로 잇는다.
 *
 * ── 표 함정 — hiking·golf와 같다 ─────────────────────────────
 * `objL1=ALL`로 통째로 받으면 «성별×나이 교차»(005001NNN/005002NNN)와 «나이만»(004NNN)
 * 이 같은 이름으로 섞여 나온다. 나이만 있는 6자리 코드만 쓴다.
 *
 * 쓰는 법  node scripts/collect-100y-workout.mjs [--selftest]
 */
...
if (process.argv.includes('--selftest')) {
  ...
  본다('② 나이만(6자리) 코드를 남긴다', 나이만인가('131021315501.004003'));
  본다('③ 성별×나이 교차(9자리, 남자)는 뺀다', !나이만인가('131021315501.005001003'));
  본다('④ 성별×나이 교차(9자리, 여자)는 뺀다', !나이만인가('131021315501.005002003'));
...
const 내가직접불렸나 = !!process.argv[1] && path.basename(process.argv[1]) === 'collect-100y-workout.mjs';
if (내가직접불렸나) {
  ...
  const 헬스줄 = 날.filter((x) => x.ITM_NM === 종목 && 나이만인가(x.C1));
  const 해들 = [...new Set(헬스줄.map((x) => x.PRD_DE))].sort();
  const 최신 = 해들[해들.length - 1];

  const 해별 = {};
  for (const 해 of 해들) {
    const 이해줄 = 헬스줄.filter((x) => x.PRD_DE === 해);
  ...
  const 낸다 = {
    무엇: '나이대별 헬스(보디빌딩) 참여율 — 최근 1년간 참여 경험(복수응답)',
    만든날: new Date().toISOString().slice(0, 10),
    출처: { 기관: '문화체육관광부', 표: '최근 1년간 참여 경험이 있는 체육활동(복수응답, 상위 10개 종목)', 창구: '국가데이터처 KOSIS', orgId: ORG, tblId: TBL },
    단위: '%',
    정의: '최근 1년 동안 헬스(보디빌딩)를 해 본 적이 있다고 답한 사람의 비율입니다(복수응답 — 다른 운동도 같이 했을 수 있습니다). 매일 가는 사람만이 아닙니다.',
    '⚠ 이 자료가 못 가르는 것': [
      '표본조사입니다. 나이대별 표본 크기가 달라 작은 칸일수록 오차가 큽니다.',
      '「참여 경험」이라 강도·빈도(한 번 가 본 사람과 매주 가는 사람을 못 가릅니다)를 못 잽니다.',
      '성별×나이를 교차한 값은 이 표에도 있지만(예: 남자 30대만) 이 지면은 나이만 봅니다 — 성별로 가르지 않았습니다.',
    ],
    해들, 최신, 해별,
  };
  const 낼곳 = path.join(뿌리, 'src/data/100yearmap/workout.json');
