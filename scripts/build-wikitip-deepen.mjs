...
if (내가실행됐다) {
  ...
  /*
   * 🔴🔴 [2026-08-29] **제목 → 어느 차트가 이 작품의 것인가.**
   *   넷플릭스는 영화 차트와 시리즈 차트를 따로 낸다. 이름이 같은 «다른 작품»이 양쪽에 앉으면
   *   제목만으로 줄을 모으는 순간 남의 작품 줄이 섞인다.
   *   ⚠ 이 흠은 여기서 «스스로 잡혔다» — 아래 검산이 「snowpiercer: 자리 합 60 이 원래 2 와
   *     다르다」라고 멈춰 세웠다. 그 검산이 없었으면 60 이 조용히 나갔을 것이다.
   *   ⭐ 어느 쪽이 우리 작품인지는 build-wikitip-title-pages 가 이미 증거로 정해 뒀다(type 칸).
  ...
  for (const l of fs.readFileSync(나라파일, 'utf8').split('\n')) {
    ...
    if (!s) continue;
    /* ⛔ 같은 이름이라도 «다른 차트»의 줄은 다른 작품의 것이다. 안 담는다 */
    if (제목차트.get(r.제목) && r.구분 !== 제목차트.get(r.제목)) continue;
