...
/**
 * 🔴🔴 [2026-09-06] **이 자가 «남의 자가시험»을 가로채고 있었다.**
 *
 * 이 파일 머리글은 이미 이렇게 경고해 두었다 —
 *   「`--selftest` 를 argv 로만 보면 남의 시험을 가로챈다. 직접 실행됐을 때만 돈다」
 * 그런데 **막아 둔 것이 `--selftest` 뿐이고, 정작 이 자가 쓰는 깃발은 `--자가시험`** 이었다.
 * ⇒ 다른 자가 여기서 별자리찾기() 를 import 하고 `--자가시험` 으로 돌면, 이 블록이 먼저
 *   돌면서 `process.exit(0)` 해 버린다. **부르는 쪽 시험이 한 줄도 안 돌고 「통과」로 보인다.**
 *
 * 오늘 실제로 그랬다 — build-kcw-people.mjs 에 별자리 시험 10개를 붙였는데
 * 화면에는 이 파일 시험 21개만 뜨고 내 것은 «하나도 안 돌았다». 초록불이라 넘어갈 뻔했다.
 *
 * ⛔ 「조용히 성공한 척하는 것이 제일 나쁘다」 — 이 저장소의 오래된 규칙 그대로다.
 * ✅ 그래서 **직접 실행됐을 때만** 돌게 막는다. 경고를 적어 둔 것과 막아 둔 것이 달랐다.
 */
