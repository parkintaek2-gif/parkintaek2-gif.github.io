...
/* 🔴🔴 [2026-09-18 · 5번] **주석이 아닌 자리로 샜다.**
   ───────────────────────────────────────────────────────
   `https://100yearmap.com/age` 본문에 이것이 그대로 찍혀 있었다 —
     「… 이 표의 C1=75 줄이다 — **사장님이 인용하신 기사**(2026-09-16 …)와 실측이 일치한다」
   `src/data/100yearmap/age-axis.json` 의 「비고」 칸인데, 지면이 그 칸을 **본문에 그대로 그린다.**
   ⛔ 이 자는 「주석만 본다」고 못박아 두었고 **그 판단 자체는 옳다** — 낱말로 찾으면
     /how-long 의 「사장님이 혼자 장사하고 계실 수도」(손님에게 하는 말)에 울기 때문이다.
   ⭐ 그래서 낱말이 아니라 **우리 내부 «화법»**만 좁게 잡는 갈래를 하나 더 둔다.
     「사장님이 인용하신」·「사장님 지시」는 우리끼리 하는 말이고,
     「사장님이 혼자 장사하고 계실 수도」는 손님에게 하는 말이다 — 뒤는 안 걸린다. */
const 본문내부화법 = [
  ['사장님 화법', /사장님(이|께서)?\s*(지시|말씀|인용|감수|짚어|바로잡|정하신|주신)/],
  ['자리 화법', /[1-9]번(이|가)\s*(감수|지시|말|적|올렸|짚)/],
  ['우리 문서', /세션간-메모|보스 도와줘요|인수인계\.md/],
];

/** 손님이 «읽는 본문»에 우리끼리 하는 말이 있나. ⚠ 주석은 위 자가 따로 본다 */
export function 본문에서새는말찾기(글) {
  return 본문내부화법.filter(([, 무늬]) => 무늬.test(글)).map(([이름]) => 이름);
}
...
function 자가시험() {
  const 것들 = [
    ...
    /* 🔴 [2026-09-18] 본문으로 샌 것을 잡는 갈래 — 실제로 100yearmap.com/age 에 나갔던 글이다 */
    ['본문의 「사장님이 인용하신」을 잡는다', () =>
      본문에서새는말찾기('… C1=75 줄이다 — 사장님이 인용하신 기사(2026-09-16)와 실측이 일치한다').length > 0],
    ['본문의 「사장님 지시」를 잡는다', () => 본문에서새는말찾기('사장님 지시로 축을 늘렸다').length > 0],
    ['⛔ 손님에게 하는 말에는 안 운다', () =>
      본문에서새는말찾기('사장님이 혼자 장사하고 계실 수도 있습니다').length === 0],
    ['⛔ 「811번가길」 같은 주소에도 안 운다', () => 본문에서새는말찾기('공항로811번가길 12').length === 0],
    ['우리 문서 이름을 잡는다', () => 본문에서새는말찾기('세션간-메모 를 보라').length > 0],
...
for (const 길 of 볼것) {
  ...
  /* 🔴 [2026-09-18 · 5번] 주석 «밖»으로 샌 것도 본다 — /age 가 여기로 샜다.
     자료 파일의 「비고」 칸이 지면 본문에 그대로 그려졌다. 주석만 보는 자는 그것을 못 본다. */
  const 본문난것 = 본문에서새는말찾기(글);
  if (본문난것.length) {
    const m = 글.match(/[^<>]{0,40}(사장님|세션간-메모|보스 도와줘요)[^<>]{0,60}/);
    걸린것.push({
      길,
      난것: 본문난것.map((x) => x + '(본문)'),
      보기: (m ? m[0] : '').replace(/\s+/g, ' ').trim().slice(0, 70),
    });
  }
}
