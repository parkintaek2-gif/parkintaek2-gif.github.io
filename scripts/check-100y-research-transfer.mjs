...
/**
 * 🔴🔴 [2026-09-09 05:0x · 5번] **여기서 exit 1 을 하면 뒤의 검사들이 «한 개도» 안 돈다.**
 *
 * `npm test` 는 검사들을 `&&` 로 이어 붙인다. 그래서 앞에서 하나가 1 을 내면 그 뒤는
 * 통째로 안 돈다. 오늘 그것 때문에 실제로 이런 일이 났다 —
 * ```
 *   check-100y-kosis-transfer 가 「원본이 없다」로 exit 1  ← 실은 «파일 이름을 잘못 짐작»한 것
 *   ⇒ 그 뒤 검사 백여 개가 며칠째 한 번도 안 돌고 있었다
 *   ⇒ 그 안에 「기사와 자료가 어긋난다」 64칸이 숨어 있었다. 아무도 못 봤다
 * ```
 * ⛔ 「못 쟀다」로 게이트를 세우면, 못 잰 하나가 «잴 수 있는 백 개»를 같이 가린다.
 *
 * ⭐ 이 저장소가 이미 그 답을 정해 뒀다 — `check-kpop-spike-article.mjs` 머리글에 이렇게 적혀 있다.
 *   「곳간(archive/)은 git 에 없다 … 그때 던지면 뒤의 검사 백여 개가 한 개도 안 돈다.
 *    ⛔ 「못 쟀다」와 「깨졌다」는 다른 말이다 … 「통과」로 읽히면 안 되므로 경고 표를 붙여 찍고 나간다」
 *   그 꼴을 여기에도 쓴다. 규칙을 무르게 하는 것이 아니라 «이미 정해 둔 규칙»을 따르는 것이다.
 *
 * ⚠ 이 갈래(`raw/kdi`)는 원드라이브 백업에서 «일부러» 뺐다 — R2 에 있어서다
 *   (`backup-archive-onedrive.ps1` 의 뺄갈래). 그래서 백업으로는 되살릴 수 없다.
 *   되살리려면 R2 에서 받아야 한다. 그것은 6번 갈래다.
 */
if (!fs.existsSync(원본방)) {
  console.log(`⬜ 못 쟀다 — ${path.relative(뿌리, 원본방)} 이 없다.`)
  console.log('   ⛔ 이것은 「통과」가 아니다. 재 보지 못했다는 뜻이다.')
  console.log('   ⭐ 이 갈래는 R2 에 있고 원드라이브 백업에서는 일부러 뺐다. 받으려면 R2 에서 받는다.')
  console.log('   ⛔ 그래도 여기서 1 을 내지 않는다 — 뒤의 검사 백여 개를 같이 막기 때문이다.')
  process.exit(0)
}
