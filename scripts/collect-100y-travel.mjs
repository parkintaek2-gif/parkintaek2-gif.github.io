...
/** ⭐ 「일수는 안 늘었는데 경험률(간 사람 자체의 몫)은 어떻게 됐나」 — 코로나 전 대비 최근 경험률 */
export function 경험률비교(경험률흐름) {
  const 코로나전 = 경험률흐름.filter((r) => Number(r.해) < 2020);
  const 최근 = 경험률흐름[경험률흐름.length - 1];
  if (!코로나전.length || !최근) return null;
  const 코로나전평균 = 한자리(코로나전.reduce((s, r) => s + r.경험률, 0) / 코로나전.length);
  return { 코로나전평균, 최근: 최근.경험률, 올랐나: 최근.경험률 >= 코로나전평균 };
}
...
if (process.argv.includes('--selftest')) {
  ...
  const 경험표본 = [
    { 해: '2018', 경험률: 89.2 }, { 해: '2019', 경험률: 92.4 },
    { 해: '2020', 경험률: 86.4 }, { 해: '2025', 경험률: 97 },
  ];
  const 경험비교 = 경험률비교(경험표본);
  본다('⑥ 경험률비교 — 코로나전 평균을 낸다(89.2+92.4)/2=90.8', 경험비교.코로나전평균 === 90.8);
  본다('⑦ 최근이 코로나전보다 높으면 올랐다', 경험비교.올랐나 === true);
  본다('⑧ 코로나 전 해가 없으면 못 잰다', 경험률비교([{ 해: '2020', 경험률: 86 }]) === null);
  process.exit();
}
...
if (내가직접불렸나) {
  ...
  /* ⭐ 「일수는 코로나 전보다 적은데, 경험률(간 사람의 몫)도 그런가」 — 다른 표(1029196)로 짝을 맞춘다 */
  const ORG2 = '113'; const TBL2 = 'DT_113_STBL_1029196';
  const j2 = await (await fetch(
    `https://kosis.kr/openapi/Param/statisticsParameterData.do?method=getList&apiKey=${KEY}`
    + `&format=json&jsonVD=Y&orgId=${ORG2}&tblId=${TBL2}&itmId=ALL&objL1=ALL&prdSe=Y&startPrdDe=2018&endPrdDe=${최신}`)).json();
  if (!Array.isArray(j2)) throw new Error(`${TBL2}: ${JSON.stringify(j2).slice(0, 150)}`);
  const 값2 = (해, 칸) => 수로(j2.find((x) => x.PRD_DE === 해 && x.C1_NM === 칸 && x.ITM_NM === '국내전체')?.DT);
  const 경험률흐름 = 해들.map((해) => ({ 해, 경험률: 값2(해, '전체') })).filter((r) => r.경험률 != null);
  const 경험률연령별 = 연령칸.map((칸) => ({ 칸, 경험률: 값2(최신, 칸) }));
  const 경험비교 = 경험률비교(경험률흐름);
  ...
  const 낸다 = {
    ...
    출처: {
      기관: '문화체육관광부',
      표: '국민여행조사 「1인 평균 국내여행 일수(만 15세 이상 전 국민)」 · 「국내여행 경험률」',
      창구: 'KOSIS', orgId: ORG, tblId: `${TBL} · ${TBL2}`,
    },
    '⚠ 이 자료가 못 가르는 것': [
      ...
      '「경험률」은 1회라도 다녀왔는지만 봅니다 — 몇 번, 며칠 갔는지는 「일수」 표를 봐야 합니다.',
    ],
  ...
  console.log(`   경험률: 코로나 전 평균 ${경험비교?.코로나전평균}% → ${최신}년 ${경험비교?.최근}% — ${경험비교?.올랐나 ? '오름' : '못 미침'}`);
