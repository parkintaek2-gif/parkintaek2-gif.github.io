#!/usr/bin/env node
/**
 * SeoulMarkets — **기사마다 다른 공유 카드**(1200×630 PNG).
 *
 *   node scripts/make-og-seoulmarkets.mjs          카드를 만든다
 *   node scripts/make-og-seoulmarkets.mjs --list   무엇을 고를지만 보여준다 (안 만든다)
 *
 * ── 왜 ─────────────────────────────────────────────────────────
 * 2026-08-08. 2번 지시(37231). 5번(K Culture Wire)이 89302c6 으로 먼저 했다.
 * 서울마켓 기사 34편의 og:image 가 **전부 /og-default.png 한 장**이었다.
 *   ① 34편이 같은 그림이면 공유돼도 다 똑같다. 공유되는 순간이 유입의 첫 칸인데 비어 있다
 *   ② 카드가 PNG 여야 카카오톡·X·페이스북이 그린다 (SVG 는 안 그린다)
 *
 * ── ⛔ 자동으로 고르지 않는다 ─────────────────────────────────
 * 「글머리(dek)의 첫 수를 뽑는다」로 짜면 대개 맞지만 **몇 편이 조용히 틀린다.**
 *   · 연도(2026)·「Top 10」의 10·정정으로 버린 값을 집을 수 있다.
 * 그래서 자동은 **제안**만 하고, 고친 값은 `src/data/seoulmarkets-og-cards.json` 에 적는다.
 * 그리고 **고른 수가 그 기사 본문에 실제로 있는지 확인**하고 없으면 선다 —
 * 없는 수를 카드에 박으면 공유된 그림이 기사와 다른 말을 한다. 그게 제일 나쁘다.
 *
 * ── ⚠ 어디에 두나 ─────────────────────────────────────────────
 * Astro 는 `public/*` 를 `dist/*` 로 복사한다. 서울마켓은 뿌리에서 낸다 —
 *   `public/og/<slug>.png` → `https://seoulmarkets.com/og/<slug>.png` 다.
 * ⛔ `public/wikitip/og/` 에 두면 **K Culture Wire 자리**로 가서 여기서는 404 다.
 *
...
const 기사방 = path.join(ROOT, 'content', 'articles');
const 고른것 = path.join(ROOT, 'src', 'data', 'seoulmarkets-og-cards.json');
const 낼방 = path.join(ROOT, 'public', 'og');

/** 지면과 같은 값이다. `global.css` 의 어두운 쪽 토큰을 옮겨 적었다 */
...
export function 제안(dek) {
  ...
  const 수들 = [...걸러진.matchAll(/[$₩]?\d[\d,]*(?:\.\d+)?\s*(?:%|bn|m\b|billion|million|trillion|tn|×|x)?/g)];
  for (const m of 수들) {
    const 값 = m[0].trim();
    /* 맨 수 네 자리가 1900~2099 면 연도로 본다. 「2,623」 처럼 쉼표가 있으면 연도가 아니다 */
    ...
    const 뒤 = 걸러진.slice(m.index + m[0].length).trim().replace(/(\d),(\d)/g, '$1\x00$2');
    const 토막 = 뒤.split(/[.,;:—–]|\sand\s|\sbut\s|\sagainst\s/)[0].replace(/\x00/g, ',').trim();
...
/**
 * 세로 자리를 **하나씩 쌓아서** 잡는다.
 * ⛔ 자리를 따로따로 적으면 수 크기가 달라질 때 딱지가 가로줄과 제목을 뚫는다 —
 *    그래서 앞의 것 위에 더한다. (5번이 첫 장을 열어 보고 잡은 것.)
 */
...
export function 카드SVG({ figure, label, title }) {
  ...
  <rect x="0" y="0" width="1200" height="8" fill="${색.파랑}"/>

  <text x="80" y="96" font-family="${고딕}" font-size="26" font-weight="bold"
        fill="${색.파랑}" letter-spacing="6">SEOUL MARKETS</text>
  ...
  <text x="1120" y="576" text-anchor="end" font-family="${고딕}" font-size="24"
        fill="${색.파랑}" letter-spacing="1">seoulmarkets.com</text>
</svg>`;
}

/* ── 여기부터 실행 ────────────────────────────────────────────── */
if (process.argv[1] && process.argv[1].endsWith('make-og-seoulmarkets.mjs')) {
  /* 자가시험 — 자를 먼저 잰다 */
  let 시험 = 0; let 통과 = 0;
  const 본다 = (이름, 참) => { 시험++; if (참) 통과++; else console.log(`  ⛔ 자가시험 실패 — ${이름}`); };
  본다('연도를 안 집는다', 제안('In 2026 the two were the same size, and banks took 54.1%.').figure === '54.1%');
  본다('Top 10 의 10 을 안 집는다', 제안('Five years of Top 10 hold 23.7 billion won.').figure === '23.7 billion');
  본다('쉼표 있는 네 자리는 연도가 아니다', 제안('The bill tops out at 2,623 won a head.').figure === '2,623');
  본다('원화를 붙여 집는다', 제안('Pay ran from ₩22m in 2005 to ₩1.80bn.').figure === '₩22m');
  본다('수가 없으면 못 고른다', 제안('No figures live in this sentence at all.') === null);
  본다('수 속의 쉼표로 자르지 않는다', 제안('154 of the 1,545 workplaces also report.').label === 'of the 1,545 workplaces also report');
  본다('끊긴 꼬리를 뗀다', 제안('660 Korean firms have some kind of a recorded board.').label === 'Korean firms have some kind');
  ...
  for (const f of 파일들) {
    ...
    const { 머리, 몸 } = 가른다(원문);
    /* draft: true 는 지면에 안 나가니 카드도 안 만든다 */
    if (/^draft:\s*true\s*$/m.test(머리)) continue;
    ...
    if (!쓸것) { 막힌것.push(`${slug} — 글머리에 수가 없다. seoulmarkets-og-cards.json 에 손으로 적어라`); continue; }
  ...
  if (보기만) {
    console.log(`\n제안만 봤다 — 기사 ${결과.length + 막힌것.length}편 중 ${결과.length}편 고름 (손 ${결과.filter((r) => r.출처 === '손').length}편)`);
    process.exit(막힌것.length ? 1 : 0);
  }

  if (막힌것.length) {
    console.log(`\n⚠ ${막힌것.length}편은 수가 요점이 아니라 개별 카드를 안 만든다 — 기본 카드로 나간다(정직). 억지 수를 박지 않는다.`);
  }
  ...
  const 기본 = 카드SVG({
    ...
    title: 'Korean equities, FX, rates and the workforce behind them',
  });
  fs.writeFileSync(path.join(ROOT, 'public', 'og-default.png'), await sharp(Buffer.from(기본)).png().toBuffer());

  console.log(`\n카드 ${결과.length}장 + 기본 1장(og-default.png) — 평균 ${(잰것 / 결과.length / 1024).toFixed(0)}KB`);
  console.log(`→ public/og/<slug>.png · https://seoulmarkets.com/og/<slug>.png`);
