#!/usr/bin/env node
/**
 * build-seoulmarkets-markdown.mjs — SeoulMarkets 기사마다 «AI가 읽기 쉬운 마크다운 판»을 낸다.
 *
 ...
 *   node scripts/build-seoulmarkets-markdown.mjs        (build-once.mjs 뒤에 실행 — dist/article 필요)
 */
...
/** dist HTML의 `<section class="notes">...</section>`을 마크다운으로 바꾼다. 없으면 null */
export function 노트변환(articleHtml) {
  const secM = articleHtml.match(/<section class="notes"[^>]*>([\s\S]*?)<\/section>/i);
  if (!secM) return null;
  const inner = secM[1];
  const 항목들 = [...inner.matchAll(/<dt[^>]*>(.*?)<\/dt>\s*<dd[^>]*>([\s\S]*?)<\/dd>/gi)];
  if (!항목들.length) return null;
  const 토막 = ['## Data & Verification Notes'];
  for (const [, dt, dd] of 항목들) {
    const 이름 = 노트인라인(dt);
    const 목록 = [...dd.matchAll(/<li[^>]*>([\s\S]*?)<\/li>/gi)].map((m) => `- ${노트인라인(m[1])}`);
    토막.push(`**${이름}**`);
    토막.push(목록.length ? 목록.join('\n') : 노트인라인(dd));
  }
  return 토막.join('\n\n');
}
...
export function 기사변환(원문, articleHtml) {
  ...
  const 꼬리 = ['---', '', 'Not investment advice. SeoulMarkets publishes data journalism for general information only.'];
...
if (process.argv.includes('--selftest')) {
  ...
  console.log('✅ build-seoulmarkets-markdown 자가시험 통과 (13)');
  process.exit(0);
}
...
for (const 파일 of 파일들) {
  ...
  if (!md) { 건너뜀++; continue; }
  if (!md.includes('Verification Notes')) 노트없음++;
  fs.writeFileSync(path.join(distDir, `${slug}.md`), md);
  냄++;
}
console.log(`✅ 마크다운 판 ${냄}개 냄 · 못 낸(draft·빌드없음) ${건너뜀}개 · notes 없이 낸 것 ${노트없음}개 — dist/article/*.md`);
