 ...
 *
 * 🔴 [2026-08-28 · 5번] **바로 윗줄은 «물러난» 주장이다.** 8/25 에 이 창이 스스로 물렀고
 *   (83% → 82% 로 고쳐도 색인이 안 바뀌었고, 자가 「판박이 문장」이 아니라 「같은 주제라
 *   낱말이 같은 것」을 재고 있었다), 8/28 에 갈래를 더 재니 관계가 깨졌다 —
 *   born-on 저만의 글 57% → 색인 100% · **group 35% → 80%** · title 54% → 70%.
 *   판박이가 가장 심한 group 이 title 보다 «높다». ⚠ 두 점으로 그은 선이었다.
 *   ⛔ 아래 「고유한 것을 늘린다」는 여전히 할 만한 일이지만 **「이러면 색인된다」의 근거로
 *     쓰지 마라.** 까닭은 `src/pages/wikitip/title/[slug].astro` 에 길게 적어 뒀다.
 * ```
