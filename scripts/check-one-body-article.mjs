...
if (틀림) {
  ...
  const 기사글들 = ["content/kculturewire/korean-netflix-titles-one-body.md"]
  ...
  const 자료때들 = ["archive/raw/netflix-top10/korean-titles-keyed.json","archive/raw/netflix-top10/korean-cast-joined.json"].map(자료의때).filter((d) => d instanceof Date);
