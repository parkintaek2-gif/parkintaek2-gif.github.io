 ...
 */
/**
 * 🔴 [2026-09-01] **이 표는 지금 비어 있다.** 숨기지 않고 적어 둔다.
 *
 * 여기 있던 셋(`riot-ladder` · `wikitip-ladder-churn` · `wikitip-ladder-gap`)은
 * 사장님 지시로 Riot·e스포츠를 통째로 걷어내면서 같이 사라졌다.
 * ⇒ **「몇 시간마다 움직이는 수」를 쓰는 지면이 지금 K Culture Wire 에 하나도 없다.**
 *
 * ⛔ 자는 지우지 않는다 — 아래 「싱싱한 척하는 주장」 검사는 그대로 돌고, 날마다
 *   움직이는 자료가 다시 생기면 여기 한 줄 적는 것으로 끝난다.
 * ⚠ 표가 비었다고 「낡은 것이 없다」로 읽으면 안 된다. **잴 대상이 없는 것**이다.
 *   못 잰 것과 없는 것은 다른 말이다.
 */
export const 볼것 = [
];
...
/**
 * 🔴 [2026-09-01] `toISOString()` 은 **UTC** 다. 한국 새벽 0~9시에 «어제»를 내놓는다.
 *   오늘 08:10 에 돌렸더니 「오늘 2026-08-31」이라고 적었다 — 하루가 통째로 어긋난다.
 *   ⚠ 낡음을 «날 수»로 재는 자라 이 어긋남이 그대로 판정을 바꾼다.
 *   ⭐ 같은 잘못을 8/30 에 `send-mail.mjs` 에서도 했다. 시각은 서울 시계로 적는다.
 */
...
/**
 * 🔴 [2026-09-01] **0장을 재고 ✅ 를 내지 않는다.**
 *   `볼것` 이 비면 이 자는 아무 지면도 안 열고 「통과」라고 말했다. 그건 «잰 것»이 아니다.
 *   ⚠ 나는 이 자리에서 여러 번 속았다 — 0 이나 100% 가 나오면 **자를 먼저 의심한다.**
 *   ⛔ 「못 쟀다」와 「깨끗하다」는 다른 말이다. 갈라서 적는다.
 */
if (본지면 === 0) {
  console.log('⚠ **아무것도 못 쟀다** — 볼 목록이 비어 있다. 「깨끗하다」가 아니라 「잴 것이 없다」다.');
  console.log('   날마다 움직이는 자료를 다시 쓰게 되면 `볼것` 에 한 줄 적는다.');
} else {
