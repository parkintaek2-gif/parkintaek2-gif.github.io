...
const 검사 = [
  ...
  ['고친 내역 표의 전·후가 참말인가', 'check-correction-tables.mjs'],
  ['제목에 든 수가 본문에 있나', 'check-kcw-title-numbers.mjs'],
  ...
  /* 🔴 2026-08-22 21:0x — 4번이 찾아 전 유닛에 돌렸다: 「구글이 사이트맵을 3주째 안 읽고 있었다.
     파일은 늘 최신이었지만 구글이 다시 와서 보지 않으면 새 글은 영영 안 뜬다」.
     그 방식으로 우리를 재니 **www 쪽이 828장에서 멈춰** 있었다(우리 canonical 이 www 다).
     재제출하니 그 자리에서 1,223장으로 갱신됐다. ⭐ 다시 낡는 것을 이 자가 잡는다 —
     사람이 기억해서 누르는 구조를 남기지 않는다. API 를 한 번만 부르므로 상시로 물려도 된다 */
  ['구글이 사이트맵을 최근에 읽었나', 'check-kcw-sitemap-read.mjs'],
  /* 🔴 2026-08-22 — 같은 병을 하루에 **네 번** 만났다: 제목이 그 지면이 주는 것과 어긋난 것.
     /market/* 아흔세 장이 똑같은 꼴 · /about 이 「About & methodology」 · /titles 가
     93개국 528장으로 링크하면서 「Southeast Asia」라고 말함 · /contact 가 「Contact」뿐.
