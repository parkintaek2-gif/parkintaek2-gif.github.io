...
/**
 * 무엇으로 그리나 — 🔴 **16시 보고는 «PPT 슬라이드 양식»이다. 그것이 기본이다.**
 *
 * 사장님 고정지시 원문 — 「업무보고 오후 4시에 정확히 내가 **ppt슬라이드의 정해진 양식**을
 *   보고 받아야 한다」
 *
 * ```
 * 2026-09-16   양식을 안 쓰고 그냥 글 메일로 보냈다가 사장님께 지적받았다
 * 2026-09-18   또 흐르는 글 PDF 가 갔다. 까닭은 게으름이 아니라 «이 자의 기본값»이었다 —
 *              이 자가 md-to-pdf 를 부르고 있었고, 부르는 사람은 그것이 슬라이드인 줄 알았다
 * ```
 * ⭐ 그래서 기본값을 바꿨다. 사람이 꼬리표를 안 붙여도 슬라이드가 나간다.
 *   흐르는 글로 내야 할 일이 생기면 `--흐름글` 을 «일부러» 붙인다.
 *
 * ⚠ 슬라이드는 `## ` 하나가 한 장이다(297×167mm, 16:9). 넘치면 자르지 않고 글자를 줄인다.
 */
export function 그릴자(인자들 = []) {
  return 인자들.includes('--흐름글')
    ? { 자: 'md-to-pdf.mjs', 꼴: '흐르는 글' }
    : { 자: 'build-daily-report.mjs', 꼴: 'PPT 슬라이드' };
}
 ...
 */
/** 「통합」 보고에 여섯 자리가 다 들어왔나를 «본문에서» 센다.
 *
 * 🔴 [2026-09-09 · 5번] 사장님: 「pdf는 표지만 있고...자꾸 동일한 실수할래?」
 *   09-05 · 09-06 · 09-09 — **사흘째 같은 사고**다. 09-09 16:40 에 나간
 *   `업무보고_통합_20260909_1640.pdf` 는 1,434바이트 md 를 그린 것이었고 «1번 칸만» 있었다.
 *   ⛔ 까닭은 모으는 자가 고장난 것이 «아니었다» — 그 자를 안 쓰고, 1번 md 하나를
 *     「통합」이라 이름만 붙여 그린 것이다(collect-1600-report 는 6/6·8,686바이트로 잘 돈다).
 *   ⇒ 쪽수만 재면 이걸 못 잡는다. **이름이 「통합」이면 여섯 칸을 센다.**
 */
export function 유닛칸수(본문) {
  const 봤다 = new Set();
  for (const 줄 of String(본문 ?? '').split(/\r?\n/)) {
    // 모으는 자가 내는 꼴: "## 1번 KLifeMap" · "## 6번 SeoulMarkets"
    const m = 줄.match(/^##\s+(\d)번(?:\s|$)/);
    if (m) 봤다.add(Number(m[1]));
  }
  return 봤다.size;
}

export function 보낼만한가({ 쪽수, 글자수, 첨부있나, 이름 = '', 본문 = '' }) {
  const 흠 = [];
  ...
  // 이름에 「통합」이 붙었으면 여섯 자리가 다 있어야 통합이다. 하나만 있으면 그건 그 유닛 보고다.
  if (/통합/.test(String(이름))) {
    const 칸 = 유닛칸수(본문);
    if (칸 < 6) 흠.push(`「통합」인데 유닛 칸이 ${칸}개다 — 여섯이어야 한다. node scripts/collect-1600-report.mjs 로 모으십시오`);
  }
...
export function 자가시험() {
  ...
  /* 🔴 사흘째 같은 사고 — 「통합」인데 한 자리만 들어간 것을 막는다 (2026-09-09) */
  const 여섯칸 = [1, 2, 3, 4, 5, 6].map((n) => `## ${n}번 무엇\n\n내용\n`).join('\n');
  검('🔴 「통합」인데 1번 칸만 있으면 막는다 (09-05·06·09 사흘째 사고)',
    보낼만한가({ 쪽수: 4, 글자수: 5000, 첨부있나: true, 이름: '업무보고_통합_20260909.md', 본문: '## 1번 KLifeMap\n\n내용\n' }).된다 === false);
  검('「통합」에 여섯 칸이 다 있으면 보낸다',
    보낼만한가({ 쪽수: 4, 글자수: 5000, 첨부있나: true, 이름: '업무보고_통합_20260909.md', 본문: 여섯칸 }).된다 === true);
  검('「통합」이 아닌 이름(제 유닛 보고)에는 여섯 칸을 안 따진다',
    보낼만한가({ 쪽수: 4, 글자수: 5000, 첨부있나: true, 이름: '5번_업무보고_20260909.md', 본문: '## 5번 KCW\n' }).된다 === true);
  검('유닛칸수 — 같은 유닛이 두 번 나와도 한 칸으로 센다',
    유닛칸수('## 1번 가\n## 1번 나\n## 2번 다\n') === 2);
  검('유닛칸수 — h3(###)는 유닛 칸이 아니다', 유닛칸수('### 3번 안쪽소제목\n') === 0);

  /* 🔴 사장님 고정지시 — 16시 보고의 «기본»은 PPT 슬라이드 양식이다 (09-16 · 09-18 두 번 어겼다) */
  검('🔴 그릴자 — 꼬리표가 없으면 «슬라이드»로 그린다',
    그릴자([]).자 === 'build-daily-report.mjs');
  검('그릴자 — --보낸다 만 붙어도 여전히 슬라이드다',
    그릴자(['node', '자.mjs', '보고.md', '--보낸다']).자 === 'build-daily-report.mjs');
  검('그릴자 — --흐름글 을 «일부러» 붙였을 때만 흐르는 글이다',
    그릴자(['--흐름글']).자 === 'md-to-pdf.mjs');
  검('그릴자 — 꼴 이름을 화면에 낼 수 있다', 그릴자([]).꼴 === 'PPT 슬라이드');
  검('⛔ 그릴자가 부르는 자가 실제로 있다',
    fs.existsSync(path.join(뿌리, 'scripts', 그릴자([]).자))
...
if (나) {
  ...
  const 그리기 = 그릴자(process.argv);
  console.log(`1) PDF 를 만든다 — ${그리기.꼴} (${그리기.자})`);
  돌린다(그리기.자, 그리기.자 === 'build-daily-report.mjs'
    ? [md, '--out', pdf]
    : [md, '--out', pdf, '--제목', `16시 업무보고 · ${날}`]);
  ...
  // ⚠ 이름·본문을 «둘 다» 넘긴다 — 「통합」 이름표에 유닛 칸이 모자라면 여기서 막힌다.
  //   PDF 에서 뽑은 글이 아니라 «원본 md» 를 센다(그리는 과정에서 헤더 꼴이 바뀔 수 있다).
  const 판정 = 보낼만한가({
    쪽수, 글자수, 첨부있나: fs.existsSync(pdf),
    이름: path.basename(md), 본문: fs.readFileSync(md, 'utf8'),
  });
  if (!판정.된다) {
    console.error(`\n⛔ 안 보낸다 — ${판정.흠.join(' · ')}`);
