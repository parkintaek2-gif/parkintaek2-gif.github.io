...
/**
 * 🔴 [2026-09-06 · 5번] **같은 인자를 두 번 쓰면 조용히 첫 것만 갔다.**
 *   나는 `--첨부=A.pdf --첨부=B.pdf` 로 PDF 둘을 붙였는데 미리보기에 «하나»만 떴다.
 *   `인자()` 가 `find` 라 첫 것을 집고 나머지를 버리기 때문이다. 미리보기를 안 봤으면
 *   「둘 다 보냈다」고 믿었을 것이다 — 2026-09-03 의 「pdf첨부 빠짐」과 똑같은 꼴이다.
 *
 *   ⭐ 여럿을 붙이는 길은 «쉼표»다 — `--첨부=A.pdf,B.pdf`. 아래가 그것을 알려 준다.
 *   ⛔ 조용히 하나만 보내지 않는다. 서고 말한다.
 */
export function 겹친인자찾기(argv) {
  const 셈 = new Map();
  for (const a of argv) {
    const m = String(a).match(/^--([^=]+)=/);
    if (m) 셈.set(m[1], (셈.get(m[1]) ?? 0) + 1);
  }
  return [...셈].filter(([, n]) => n > 1).map(([이름]) => 이름);
}
 ...
 */
/**
 * 🔴 [2026-09-16 · 5번] 정해진 보고인데 자물쇠가 보는 표(「16시 업무보고」)가 제목에 없나.
 *
 * check-2h 의 ③-2 는 `docs/보낸메일.tsv` 에서 그 글자를 찾아 「보고를 냈나」를 판정한다.
 * 오늘 나는 「[5번] 업무보고 2026-09-16 — …」로 보냈고, 보고는 사장님께 갔는데
 * 자물쇠는 저녁 내내 「안 냈다」로 빨간불을 켰다.
 *
 * ⛔ 이 함수는 «막지» 않는다 — 보내는 일은 맞는 일이다. 이름만 바로잡게 한다.
 * ⛔ 자물쇠 쪽을 느슨하게 고쳐 통과시키지 않는다. 그건 뒷문이지 고침이 아니다.
 */
export function 제목보고표빠졌나(제목) {
  const s = String(제목 || '');
  if (!/업무보고|방문자수·체류시간/.test(s)) return false;   /* 정해진 보고가 아니다 */
  if (/16시 업무보고/.test(s)) return false;                 /* 표가 있다 */
  if (/방문자수·체류시간/.test(s)) return false;              /* 22시 방송은 표가 다르다 */
  return true;
}
...
if (내가실행됐다 && process.argv.includes('--selftest')) {
     ...
     그런데 나는 --첨부 를 붙여 보내고 「첨부했다」고 믿었다. 아래 시험들이 그것을 막는다. */
  {
    ...
    /* 🔴 [2026-09-06 · 5번] 같은 인자를 두 번 쓰면 첫 것만 갔다 — 그것을 잡는 시험 */
    참('같은 인자 두 번을 잡는다', 겹친인자찾기(['--첨부=a.pdf', '--첨부=b.pdf']).length === 1);
    참('겹친 이름을 그대로 알려 준다', 겹친인자찾기(['--첨부=a', '--첨부=b'])[0] === '첨부');
    참('한 번씩만 쓰면 안 잡는다', 겹친인자찾기(['--받는곳=a', '--제목=b', '--첨부=c']).length === 0);
    참('깃발은 세지 않는다(= 이 없다)', 겹친인자찾기(['--보낸다', '--보낸다']).length === 0);
    참('둘 이상 겹쳐도 다 찾는다', 겹친인자찾기(['--첨부=a', '--첨부=b', '--제목=c', '--제목=d']).length === 2);
    참('쉼표 한 줄은 겹침이 아니다', 겹친인자찾기(['--첨부=a.pdf,b.pdf']).length === 0);
  }
  ...
  /* ⛔ 관리용 주소를 «기본» 보내는 주소로 쓰지 않는다.
     ⚠ [2026-09-21] 이것은 «기본값» 검사다. 사장님이 「admin계정으로 이메일 보내」라고
       고르신 한 통을 막는 것이 아니다 — 그때는 `--보내는곳=admin@…` 으로 그 통만 바꾼다. */
  참('기본 보내는 주소가 admin@ 이 아니다', !보내는주소.startsWith('admin@'));
  ...
  /* 🔴 [2026-09-16] 정해진 보고를 이 범용 자로 보내 자물쇠가 못 알아본 일을 시험으로 굳힌다 */
  참('표 없는 업무보고 제목은 일러 준다', 제목보고표빠졌나('[5번] 업무보고 2026-09-16 — 손님 길') === true);
  참('표 있는 제목은 가만둔다', 제목보고표빠졌나('[5번] 16시 업무보고 — 오늘 한 것') === false);
  참('보고가 아닌 편지는 가만둔다', 제목보고표빠졌나('[5번] 카카오 로그인 복구 완료') === false);
  참('22시 방송은 표가 다르다', 제목보고표빠졌나('[5번] 방문자수·체류시간 2026-09-16') === false);
  참('빈 제목도 안 죽는다', 제목보고표빠졌나('') === false && 제목보고표빠졌나(null) === false);
  참('빈 글은 없다', 오류글읽기('') === '없다');
...
if (내가실행됐다) {
  ...
  const 받는곳 = 인자('받는곳');
  const 제목 = 인자('제목');
  /* 🔴🔴 [2026-09-21 · 5번] **`--보내는곳` 을 받아 놓고 «쓰지 않고» 있었다.**
     ─────────────────────────────────────────────────────────────────────
     `아는인자` 목록에 들어 있어서 오타 검사는 통과하는데, 정작 보낼 때는 모듈 상수
     `보내는주소`(= MAIL_FROM 또는 u<자리>@klifedesign.net)를 그대로 썼다.
     ⇒ 오늘 사장님이 「admin계정으로 이메일 보내」라고 하셔서 `--보내는곳=admin@…` 을
       줬는데 **u5@klifedesign.net 으로 나갔다.** 대장(docs/보낸메일.tsv)에도 u5 로 찍혔다.
     ⛔ 2026-09-03 에 `--첨부` 가 조용히 무시돼 사장님께 첨부 없이 간 것과 «같은 병»이다.
       그때 오타 검사를 세웠는데, 「아는 인자인데 안 쓰는」 자리는 그 검사로 안 잡힌다.
     ⭐ 보내는 주소는 도메인 위임(sub)으로도 쓰이므로 `대신할주소` 까지 함께 바꾼다. */
  const 보낼주소 = 인자('보내는곳') || 보내는주소;
  const 보낼이름 = 인자('이름') || 보내는이름;
  /* ⛔ 모르는 인자면 «안 보낸다». 오타 하나로 첨부가 빠지는 것보다 서는 것이 낫다 */
  {
    ...
    /* 🔴 [2026-09-06 · 5번] 같은 인자를 두 번 쓰면 «첫 것만» 갔다. 서고 알린다 */
    const 겹침 = 겹친인자찾기(process.argv.slice(2));
    if (겹침.length) {
      console.log(`\n🔴 같은 인자를 두 번 썼다: ${겹침.map((x) => '--' + x).join(', ')} — **안 보낸다.**`);
      console.log('   ⛔ 이 자는 «첫 것»만 씁니다. 그대로 두면 나머지가 조용히 빠집니다.');
      console.log('   ✅ 여럿을 붙이려면 쉼표로 씁니다 — --첨부=A.pdf,B.pdf');
      process.exit(1);
    }
  }
  const 글파일 = 인자('글');
  ...
  /* 🔴 [2026-09-16 · 5번] 정해진 보고를 «이 범용 자»로 보내면 자물쇠가 못 알아본다.
     오늘 내가 그렇게 보냈고, 보고는 사장님께 갔는데 check-2h 는 저녁 내내
     「16시가 지났는데 아직 안 냈다」고 빨간불을 켰다.
     ⛔ 자물쇠가 빨간 채로 남으면 그 자리는 결국 안 읽히고, 그 «아래»의 진짜 빨간불까지
       같이 건너뛴다. 그러니 조심으로 두지 않고 여기서 말해 준다.
     ⚠ 막지는 않는다 — 보내는 일 자체는 맞는 일이다. 이름만 바로잡게 한다. */
  if (제목보고표빠졌나(제목)) {
    console.log('');
    console.log('   ⚠ 제목에 자물쇠가 보는 표가 없다 — check-2h ③-2 가 「안 냈다」로 읽는다.');
    console.log('      정해진 16시 보고는 node scripts/send-1600-report.mjs 로 보낸다.');
    console.log('      굳이 이 자로 보내려면 제목에 「16시 업무보고」를 그대로 넣는다.');
  }
  ...
  /* 🔴 [2026-09-21] **고른 주소로 실제로 보낼 수 있는지 그 주소로 확인한다.**
     위쪽에서 위임 토큰을 받아 「이 주소로 보낼 수 있다」고 찍었는데, 그건 «기본 주소»로
     받은 것이다. `--보내는곳` 으로 다른 주소를 고르면 그 확인이 헛것이 된다 —
     「됐다」는 말만 보고 다른 주소로 나가는 꼴이다. */
  if (보낼주소 !== 보내는주소) {
    console.log(`\n■ 보내는 주소를 바꿔 달라고 했다 — ${보낼주소}`);
    try {
      await 위임토큰받기(키, 보낼주소);
      console.log('   ✅ 그 주소로도 위임 토큰을 받았다 — 이 주소로 나간다');
    } catch (e) {
      막혔다(`${보낼주소} 로는 못 보낸다`, String(e && e.message || e));
      console.log('   ⛔ 기본 주소로 «몰래» 보내지 않는다. 안 보내고 선다.');
      process.exit(1);
    }
  }
  ...
  try {
    ...
    /* ⛔ [2026-09-21] 여기 `보내는주소`(모듈 기본값)를 적고 있었다 — `--보내는곳` 으로
       다른 주소에서 보내도 대장에는 기본값이 찍혔다. 실제로 나간 주소를 적는다.
       대장이 사실과 다르면 나중에 「어느 주소로 갔나」를 영영 못 되짚는다. */
    const 한줄 = [때, 보낼주소, 받는곳, String(제목).split(칸).join(' '), 결과.id].join(칸);
