 ...
 */
/* 🔴 [2026-09-17 · 3번] 「제114조에 따라」가 걸렸다 — 「제85조」는 안 잡는다고 문서에 적어
 *   놓고도 자가시험엔 없었다. `(?<!제\s?)` 는 매치 «시작점»이 제 바로 다음일 때만 막는다.
 *   그런데 정규식은 여러 자리 숫자(114) 안의 «뒷자리」(4)에서도 다시 시작을 시도해
 *   그 자리에는 제가 안 붙어 있어 빠져나갔다. `(?<![0-9])` 로 숫자 뭉치의 «맨 앞»에서만
 *   시작하게 막아 해결했다. */
const 단위없는금액 =
  /(?<![0-9])(?<!제\s?)[0-9][0-9,.]*\s*(?:억|조)(?=\s*(?:이|가|은|는|을|를|의|에|도|만(?!\s*[가-힣])|과|와|이다|입니|짜리|규모|대\b))/;
...
// ── 자가시험 — 일부러 틀린 값을 넣어 잡히는지 본다 ──────────────
{
  ...
  const 좋은 = [
    '평균 80.6억원이다', '2,862곳 · 35,004명', '재적 547,002명 가운데 24,756명',
    '국세징수법 제114조에 따라', '제85조에 따라', /* 2026-09-17 — 여러 자리 조문번호 회귀 */
  ];
  let 실패 = 0;
