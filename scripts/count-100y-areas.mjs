...
function 셀프테스트() {
  ...
  확인('⭐ 줄임말 시도도 같은 열쇠가 된다', 지역가르기('서울 서초구 1')?.열쇠, 지역가르기('서울특별시 서초구 1')?.열쇠);
  확인('⭐ 강원도 = 강원특별자치도', 지역가르기('강원도 원주시 1')?.열쇠, 지역가르기('강원특별자치도 원주시 1')?.열쇠);
  확인('⭐ 전라북도 = 전북특별자치도', 지역가르기('전라북도 익산시 1')?.열쇠, 지역가르기('전북특별자치도 익산시 1')?.열쇠);
  확인('모르는 시도는 그대로', 지역가르기('가나다도 라마시 1')?.시도, '가나다도');
  ...
  확인('⭐ 세종 슬러그 — 이름을 두 번 담지 않는다', 지역슬러그(지역가르기('세종특별자치시 남세종로 441')), '세종특별자치시');
  확인('⭐ 세종 왕복 — 열쇠가 맞아야 지면이 0곳이 안 된다', 슬러그풀기(지역슬러그(지역가르기('세종특별자치시 남세종로 441'))), { 시도: '세종특별자치시', 이름: '세종특별자치시' });
  확인('⭐ 세종 왕복 열쇠', (()=>{const g=지역가르기('세종특별자치시 남세종로 441'); const p=슬러그풀기(지역슬러그(g)); return `${p.시도} ${p.이름}`===g.열쇠 || p.이름===g.열쇠;})(), true);
...
function 본일() {
  ...
  for (const x of 학교들) {
    ...
    if (!묶음.has(g.열쇠)) 묶음.set(g.열쇠, { ...g, slug: 지역슬러그(g), 곳: 0, 진학잰곳: 0, 갈래: new Set(), 교육청별: new Map() });
    const v = 묶음.get(g.열쇠);
    v.곳++;
    if (진학있음.has(x.code)) v.진학잰곳++;
    if (x.고교유형) v.갈래.add(String(x.고교유형).trim());
    /**
     * 🔴 **이 단위가 어느 교육청 지역에 속하나** (2026-08-08 13:3x).
     *
     *   주소는 「전남광주통합특별시 북구」처럼 **광주와 전남을 합쳐** 적혀 있다.
     *   그런데 지역 지면(·)은 **교육청 기준**이다.
     *   글자로 견주면 그 둘이 안 맞아 **지역 지면 두 장이 빈다**(실측으로 잡았다).
     *
     *   ⛔ 내가 「북구는 광주」라고 적어 넣지 않는다. **그 구에 실제로 있는 학교의
     *     교육청**을 세어 제일 많은 것을 쓴다 — 자료가 답하게 한다.
     */
    const 교 = String(x.지역 ?? '').trim();
    if (교) v.교육청별.set(교, (v.교육청별.get(교) ?? 0) + 1);
  }
  ...
  const 목록 = [...묶음.values()]
    .map((v) => ({
      ...
      갈래수: v.갈래.size,
      갈래: [...v.갈래].sort(),
      교육청지역: [...v.교육청별.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? null,
      교육청여럿: v.교육청별.size > 1,
      한벌로팔만한가: 한벌로팔만한가(v.곳),
    }))
    .sort((a, b) => b.곳 - a.곳);
