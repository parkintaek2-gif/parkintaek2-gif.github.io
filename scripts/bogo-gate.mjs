...
/**
 * 🔴 떠들었나 — 사장님(8/15): 「**떠들면서 일하지 말고. 시작할 때, 종료할 때만 뭘 했는지 말해**」
 *
 * ⛔ 말이 길면 그만큼 토큰이고, 사장님이 읽으실 것이 늘어난다.
 * ⭐ 기준은 **시작 한 줄 · 끝 한 줄** — 한 시간에 두 줄이 넘으면 떠든 것이다.
 */
export function 떠들었나(줄수, 시간수) {
  if (!시간수 || 시간수 <= 0) return false;
  return 줄수 > 시간수 * 2;
}
...
if (process.argv.includes('--selftest')) {
  const 시험 = [
    ...
    [떠들었나(2, 1), false, '한 시간에 두 줄(시작·끝)이면 통과'],
    [떠들었나(3, 1), true, '⛔ 한 시간에 세 줄이면 떠든 것'],
    [떠들었나(6, 3), false, '세 시간에 여섯 줄이면 통과'],
    [떠들었나(0, 0), false, '아직 안 지났으면 안 센다'],
  ];
  let 틀림 = 0;
