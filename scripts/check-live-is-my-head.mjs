 ...
 * 도장 «뒤에» 쌓인 커밋을 세어 준다.
 *
 * 🔴 [2026-09-08 · 5번] 이 자가 없어서 **이 검사가 거짓 결론을 냈다.**
 *   내 제목 변경이 라이브에 «실제로 나갔는데» 마지막 판정이 이렇게 떴다 —
 *     「커밋만 하고 배포를 안 했다. 새로 낸 것은 라이브에 없다(404 다)」
 *   까닭: 배포가 도는 7~13분 동안 «옆 유닛이 커밋»해서 HEAD 가 앞서 나간 것이다.
 *   여섯 자리가 한 저장소를 쓰니 이것은 «드문 일이 아니라 보통 일»이다.
 *
 * ⛔ 거짓 빨간불이 제일 나쁘다. 이 파일이 이미 적어 둔 그대로 —
 *   「2026-09-01 밤에 5번이 그렇게 두 시간을 헛돌았습니다.」
 * ✅ 그래서 «몇 개»가 뒤에 쌓였고 «무엇»인지 보여 준다. 그러면 보는 사람이
 *   자기 것이 들었는지 한눈에 안다. 판정을 대신 내려 주지 않는다.
 *
 * ⚠ 못 세면 null 이다 — git 이 없거나 도장 커밋이 이 저장소에 없을 수 있다.
 *   그때는 「0개」가 아니라 「못 쟀다」로 낸다.
 */
export function 도장뒤의커밋들(찍힌, 지금, 몇 = 5) {
  if (!찍힌 || !지금) return null;
  try {
    const 글 = execFileSync('git', ['log', '--oneline', '--no-decorate', `${찍힌}..${지금}`],
      { encoding: 'utf8', timeout: 20000 });
    const 줄들 = 글.split(/\r?\n/).filter(Boolean);
    return { 수: 줄들.length, 앞몇개: 줄들.slice(0, 몇) };
  } catch { return null; }
}
...
export async function 견준다({ 조용히 = false } = {}) {
  ...
  if (!조용히) {
    ...
    if (다른것.length) {
      const 라이브커밋 = 다른것[0] && 다른것[0].라이브커밋;
      const 사이 = 라이브커밋 ? 도장뒤의커밋들(라이브커밋, 지금커밋()) : null;
      console.log('');
      console.log('⚠ **라이브가 내 HEAD 보다 뒤에 있다.**');
      if (!사이) {
        console.log('   ⬜ 사이에 무엇이 있는지 **못 쟀다** — 0 개로 치지 않는다.');
        console.log('   ⭐ 몇 분 기다린 뒤 다시 배포하십시오. 코드를 또 뒤지지 마십시오.');
        console.log('   ⚠ 2026-09-01 밤에 5번이 그렇게 두 시간을 헛돌았습니다.');
      } else if (사이.수 === 0) {
        console.log('   🔴 사이에 커밋이 **0개**인데 라이브가 다르다 — 구워진 이미지가 옛 것이다.');
        console.log('   ⭐ 몇 분 기다린 뒤 다시 배포하십시오. 코드를 또 뒤지지 마십시오.');
        console.log('   ⚠ 2026-09-01 밤에 5번이 그렇게 두 시간을 헛돌았습니다.');
      } else {
        console.log(`   ⭐ 라이브 뒤에 커밋 ${사이.수}개가 쌓였다. 그것들이 아직 안 나간 것이다 —`);
        for (const c of 사이.앞몇개) console.log(`      · ${c}`);
        if (사이.수 > 사이.앞몇개.length) {
          console.log(`      · … 그 밖 ${사이.수 - 사이.앞몇개.length}개`);
        }
        console.log('   ⛔ 이것을 「방금 한 배포가 실패했다」로 읽지 «마십시오».');
        console.log('      여섯 자리가 한 저장소를 씁니다 — 마지막 배포 뒤에 누가 커밋하면');
        console.log('      이 줄은 «거의 언제나» 뜹니다. 그 자체로는 흠이 아닙니다.');
        console.log('   ✅ 판정하는 법 — 위 목록에 «내 커밋»이 있으면 아직 안 나갔고,');
        console.log('      없으면 내 것은 이미 라이브에 있습니다.');
        console.log(`      한 줄로 재려면:  git merge-base --is-ancestor <내커밋> ${라이브커밋}`);
      }
    } else if (못잰것.length) {
      ...
      if (뒤.잼 && 뒤.뒤처졌나) {
        const 뒤것 = 도장뒤의커밋들(뒤.찍힌, 뒤.지금);
        console.log('');
        console.log('⚠ **라이브는 «도장»과 같은데, 그 도장이 지금 HEAD 가 아니다.**');
        console.log(`   도장 ${뒤.찍힌} · 지금 HEAD ${뒤.지금}`);
        if (!뒤것) {
          console.log('   ⬜ 사이에 몇 개가 쌓였는지 **못 쟀다** — 0 개로 치지 않는다.');
        } else {
          console.log(`   ⭐ 도장 «뒤»에 커밋 ${뒤것.수}개가 쌓였다. 그 ${뒤것.수}개는 아직 라이브에 없다 —`);
          for (const c of 뒤것.앞몇개) console.log(`      · ${c}`);
          if (뒤것.수 > 뒤것.앞몇개.length) {
            console.log(`      · … 그 밖 ${뒤것.수 - 뒤것.앞몇개.length}개`);
          }
          console.log('   ⛔ 이것을 「내 것이 안 나갔다」로 읽지 «마십시오».');
          console.log('      여섯 자리가 한 저장소를 씁니다 — 배포가 도는 7~13분 동안 옆 자리가');
          console.log('      커밋하면 HEAD 가 앞서 나가고 이 줄이 뜹니다. 보통 일입니다.');
          console.log('   ✅ 확인하는 법 — 위 목록에 «내 커밋»이 있으면 아직 안 나간 것이고,');
          console.log('      없으면 내 것은 도장 안에 들어 있어 이미 나간 것입니다.');
          console.log(`      한 줄로 재려면:  git merge-base --is-ancestor <내커밋> ${뒤.찍힌}`);
        }
      } else if (뒤.잼) {
